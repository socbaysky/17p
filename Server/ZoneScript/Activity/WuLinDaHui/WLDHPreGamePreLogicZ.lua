...
function tbPreGameMgr:PlayerKickOut(pPlayer)
    Log("WuLinDaHui PreLogicMgr PlayerKickOut", pPlayer.nMapTemplateId, pPlayer.dwOrgPlayerId);
    pPlayer.ZoneLogout()
end

--重载
function tbPreGameMgr:PlayerKickOutMsg(szMsg, pPlayer)
    Log("WuLinDaHui PreLogicMgr PlayerKickOutMsg", pPlayer.nMapTemplateId, pPlayer.dwOrgPlayerId, szMsg);
    if szMsg then
        CallZoneClientScript(pPlayer.nZoneIndex, "WuLinDaHui:SendPlayerMsg", pPlayer.dwOrgPlayerId, {szMsg = szMsg })
    end
...
function tbPreGameMgr:TrueEndGame()
    --上一步可能是传送玩家，现在zonelogout 也是下一帧执行了
    ...
    if bPreEnd then --预选赛已经完， 产生16强 ,确认需求同步的角色id
        ...
        for i = 1, tbWLDHDef.tbFinalsGame.nFrontRank do
            ...
            if v then
                ...
                local tbTeamInst = WuLinDaHui:GetFightTeamByID(nFightTeamID);
                local nTeamServerId = tbTeamInst:GetServerIdx()
                for dwRoleId,v2 in pairs(tbTeamInst:GetAllPlayerID()) do
                    table.insert(tbNeedSynRoleInfoIds, {dwRoleId, nTeamServerId} )    
                end
            end
        end
...
function tbPreGameMgr:SendWinChannaelMsg(pPlayer, bFinal)
    ...
    CallZoneClientScript(pPlayer.nZoneIndex, "WuLinDaHui:SendPlayerMsg", pPlayer.dwOrgPlayerId, {szKinMsg = szKinMsg, szFriend = szFriendMsg })
end
