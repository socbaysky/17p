...
function tbBaseLogic:DoFinalsEnd(nFightTeamID)
    ...
    for nPlayerID, _ in pairs(tbAllPlayer) do
        ...
        local pRole = Player:GetRoleStayInfo(nPlayerID, true, nServerId)
        local pAsync = Player:GetAsyncData(nPlayerID, nServerId)
        if pRole then
            ...
            tbRoleInfo.nSex = pRole.nSex;
            tbRoleInfo.szKinName = pRole.szKinName
        end
        if pAsync then
        ...
        --跨服上没有playerItems 的
        if WuLinDaHui.tbCachePlayerViewItems[nServerId] then
            tbRoleInfo.tbItems = WuLinDaHui.tbCachePlayerViewItems[nServerId][nPlayerID]    
        end
    end
...
function tbBaseLogic:SendChampionTeamMsg(nFightTeamID)
    ...
    for nPlayerID, _ in pairs(tbAllPlayer) do
        ...
        local pPlayer = KPlayer.GetPlayerObjById(nPlayerID, nServerId)
        if pPlayer then
...
function tbBaseLogic:SendPlayerMsg(pPlayer, tbInfo)
    CallZoneClientScript(pPlayer.nZoneIndex, "WuLinDaHui:SendPlayerMsg", pPlayer.dwOrgPlayerId, tbInfo)
end
