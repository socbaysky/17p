...
function WuLinDaHui:RequestSynRoleInfo(tbRequestRoleIds)
	...
	for i,v in ipairs(tbRequestRoleIds) do
		...
		SendTempRoleStayInfoToZC(nConnectIdx, nServerId, dwRoleId)
	end
end

function WuLinDaHui:CallFromZoneClinetGetData()
	local nConnectIdx = Server.nCurConnectIdx;
	CallZoneClientScript(nConnectIdx, "WuLinDaHui:CallBackFromZoneServerGetData")
end
...
function WuLinDaHui:CachePlayerViewItemInfo( dwRoleId, tbItems )
	...
	self.tbCachePlayerViewItems[nServerId] = self.tbCachePlayerViewItems[nServerId] or {};
	self.tbCachePlayerViewItems[nServerId][dwRoleId] = tbItems
end
...
function WuLinDaHui:ChangeFightTeamPlayerNum(pPlayer, tbPlayerNum, nGameType)
    ...
    CallZoneClientScript(pPlayer.nZoneIndex,  "WuLinDaHui:OnZCChangeFightTeamPlayerNum", pPlayer.dwOrgPlayerId, tbPlayerNum, nGameType)
end


WuLinDaHui.tbC2ZRequest = {
	["RequestTopPreFightTeamList"] = function (pPlayer, nGameType, nClientSynTime)
		CallZoneClientScript(pPlayer.nZoneIndex,  "WuLinDaHui:OnZCRequestTopPreFightTeamList", pPlayer.dwOrgPlayerId, nGameType, nClientSynTime)
	end;

	["RequestFightTeamShow"] = function (pPlayer, nFightTeamID)
        CallZoneClientScript(pPlayer.nZoneIndex,  "WuLinDaHui:OnZCRequestFightTeamShowInfo", pPlayer.dwOrgPlayerId, nFightTeamID)
    end;

    ["RequestFightTeam"] = function (pPlayer, nGameType)
    	CallZoneClientScript(pPlayer.nZoneIndex,  "WuLinDaHui:OnZCRequestFightTeam", pPlayer.dwOrgPlayerId, nGameType)
    end;
