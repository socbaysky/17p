...
function Server:OnStartup()
	...
	Lib:CallBack({tbGoodVoiceAct.OnServerStart, tbGoodVoiceAct});
	WuLinDaHui:OnServerStartup()
	Lib:CallBack({DomainBattle.OnServerStart, DomainBattle});

	Lib:CallBack({Lottery.Load, Lottery});
	Lib:CallBack({JingMai.OnServerStart, JingMai});
	Lib:CallBack({Map.InitSetting, Map});
	Lib:CallBack({WuLinDaShi.OnStartUp, WuLinDaShi});

	Lib:CallBack({DomainBattle.tbCross.OnServerStart, DomainBattle.tbCross});
	Lib:CallBack({Mail.CheckStartGlobalMailTimeLimit, Mail});
	Lib:CallBack({RecordStone.CheckDataSetting, RecordStone});
end
...
function Server:Activate(nTimeNow)
	...
	Lib:CallBack({Furniture.MagicBowl.Activate, Furniture.MagicBowl});
end
...
function Server.OnZoneCallScript(nZoneId, szFunc, ... )
	...
	Server.nCurZoneId = nil;
	if not bRet then
		Log("Server:OnZoneCallScript Error", nZoneId, szFunc, ...)
	end
end
...
function Server:LoadZoneGroup()
	self.tbZoneGroup = LoadTabFile("Setting/ZoneGroup.tab", "dddddddddddddd", "ServerId", {"ServerId", "Normal", "Battle", "Baihu", "TeamBattle", "Indiffer",
					 "DaXueZhang","BiWuZhaoQin", "WLDH", "BLBoss", "ZoneBossMaster", "FactionBattle", "QYHCross", "CrossDomain"});
...
function Server:GetZoneGroupServerId(szType)
	local nServerId = GetServerIdentity()
	return self.tbZoneGroup[nServerId][szType]
end

function Server:ChangeZoneGroup(szType, szActName, szCloseActName, szGroup)
	...
	if Lib:IsEmptyStr(szGroup) then
		Log(debug.traceback())
	end

	
	if self.szCurChanegZoneGroupKey and  szGroup ~= self.szCurChanegZoneGroupKey then
		Log("Server:ChangeZoneGroup Error2 can't change not same ZoneGroupKey", self.szCurChanegZoneGroupKey, szGroup, szType)
		return
	end
	...
	self.szCurZoneGroupaType = szType
	if szType == "Normal" then
		self.szCurChanegZoneGroupKey = nil;
	else
		self.szCurChanegZoneGroupKey = szGroup;
	end
end
...
function Server:IsInterConnectServer()
	return GetGatewayCount() >= 2
end
