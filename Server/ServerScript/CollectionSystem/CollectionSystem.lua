...
function CollectionSystem:AddCard(pPlayer, nCardTemplateId, nReason)
    ...
    if nReason then
        Achievement:AddCount(pPlayer, "RandomFubenCard_1", 1)
    end
...
function CollectionSystem:ActivatePosition(nCollectionId, nCardTemplateId)
    ...
    local szCardName, _, _, nQuality       = KItem.GetItemShowInfo(nCardTemplateId, me.nFaction, me.nSex)
    local nItemTemplateId  = self:GetCollectionItemId(nCollectionId)
    local szCollectionName = KItem.GetItemShowInfo(nItemTemplateId, me.nFaction, me.nSex)
    local _, _, _, _, TxtColor = Item:GetQualityColor(nQuality or 1);
    
    local szMsg = string.format("你成功收集了[%s]%s[-]（淩绝峰卡片）", TxtColor or "848484", szCardName )
    me.Msg(szMsg)
...
function CollectionSystem:SendRankAward_1()
    ...
    local tbNewInfoRank = {}
    local tbMail = {Title = "凌绝峰卡片收集", From = "", nLogReazon = Env.LogWay_CollectionSystem}
    local szMailText = "少侠，上个月的凌绝峰卡片收集活动已结束，你的排名为第%d名，奖励已随附件发放，请查收。本月，新的收集活动已经开始，依然要继续努力收集哦！"
    for nRank, tbRankInfo in ipairs(tbRankPlayer or {}) do
        local nPlayerId = tbRankInfo.dwUnitID
        ...
        Achievement:AddCount(nPlayerId, "RandomFubenCard_2", 1)
        if nRank <= 100 then
            Achievement:AddCount(nPlayerId, "RandomFubenCard_3", 1)
        end
        if nRank <= 30 then
            Achievement:AddCount(nPlayerId, "RandomFubenCard_4", 1)
        end
        if nRank == 1 then
            Achievement:AddCount(nPlayerId, "RandomFubenCard_5", 1)
        end
...
function CollectionSystem:SendRankAward_2()
    ...
    local tbRankPlayer = RankBoard:GetRankBoardWithLength("CardCollection_" .. nCollectionId, 99999, 1)
    local tbNewInfoRank = {}
    local tbMail = {Title = "锦绣山河集", From = "", nLogReazon = Env.LogWay_CollectionSystem}
    local szMailText = "大侠，双十“锦绣山河”收集活动已结束，您获得了第%d名，附件为奖励，请查收！"
    for nRank, tbRankInfo in ipairs(tbRankPlayer or {}) do
        local tbAward = self:GetRankAward(nCollectionId, nRank)
        if tbAward and next(tbAward) then
