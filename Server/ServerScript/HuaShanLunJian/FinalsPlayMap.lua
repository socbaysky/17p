...
function tbBaseLogic:SendChampionTeamMsg(nFightTeamID)
    ...
    for nPlayerID, _ in pairs(tbAllPlayer) do
        ...
        if tbStayInfo then
            ...
            if tbStayInfo.dwKinId ~= 0 then
                szShowMsg = string.format("恭喜帮派成员「%s」所在的%s战队获得了本届华山论剑冠军，站在了华山之巅！", tbStayInfo.szName, szFightName);
                ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szShowMsg, tbStayInfo.dwKinId);
            end

            szShowMsg = string.format("恭喜您的好友「%s」所在的%s战队获得了本届华山论剑冠军，站在了华山之巅", tbStayInfo.szName, szFightName);
    ...
    local szWorldMsg = string.format("恭喜%s所在的「%s」战队获得了本届华山论剑冠军，站在了华山之巅！", szPlayerTeam, szFightName);
    ...
    if tbStateInfo and tbStateInfo.szEndWorldNotify then
        ...
        for _, nFightTeamID in pairs(self.tbMatchPlayGame) do
            ...
            if tbFightTeam then
                ...
                szExtMsg = szExtMsg .. string.format("战队[FFFE0D]「%s」[-]", tbFightTeam:GetName());
            end    
        end
...
function tbBaseLogic:GetTeamWatchPlayerInfo(nFightTeamID)
    ...
    for nPlayerID, tbPlayPKInfo in pairs(tbTeamInfo.tbAllPlayerPKInfo) do
        ...
        if pPlayer and pPlayer.nMapTemplateId == tbFinalsDef.nFinalsMapTID and tbPlayPKInfo.nResult == TeamPKMgr.tbDef.nPlayNone  
           and pPlayer.nFightMode ~= Npc.FIGHT_MODE.emFightMode_Death then
...
function tbBaseLogic:PlayerPlayGameOneWin(pPlayer)
    ...
    pPlayer.CenterMsg("对方战队没有成员参加，系统判定本战队胜利", true);
end
...
function tbBaseLogic:InitPlayDuelFighTeamInfo(nFightTeamID)
    ...
    for nPlayerID, _ in pairs(tbTeamInfo.tbAllPlayer) do
        local pPlayer = KPlayer.GetPlayerObjById(nPlayerID);
        if pPlayer then
            local nTeamNum = tbFightTeam:GetPlayerNum(pPlayer.dwOrgPlayerId);
            local tbShowInfo =
            {
    ...
    if nCurResult == TeamPKMgr.tbDef.nPlayDogfall then
        ...
        if tbTeamInfo.nEnemyFightTeam then
            ...
            if tbFinalsInfo1.nRank > tbCurFinalsInfo.nRank then
                ...
                szResultMsg = string.format("本来是平局，由於本战队的预选赛排名比战队:「%s」高所以判定胜利", szFightTeamName or "-");
            else
                nCurResult = TeamPKMgr.tbDef.nPlayFail;
                szResultMsg = string.format("本来是平局，由於本战队的预选赛排名比战队:「%s」低所以判定失败", szFightTeamName or "-");     
            end
        end          
    end 
...
function tbBaseLogic:PlayDuelFightTeamPK(nIndex, tbPlayInfo, nEndTime, nNum)
    ...
    if nPlayer1 then
        if MODULE_ZONESERVER  then
            local nServerId1 = tbFightTeam1:GetServerIdx()    
            nPlayer1 = KPlayer.GetZonePlayerIdByOrgId(nPlayer1, nServerId1)
        end
    ...
    if nPlayer2 then
        if MODULE_ZONESERVER  then
            local nServerId2 = tbFightTeam2:GetServerIdx()    
            nPlayer2 = KPlayer.GetZonePlayerIdByOrgId(nPlayer2, nServerId2)
        end
        ...
        if tbAllTeamShowAccount then
            ...
            if nPlayer1 and tbPlayerPKInfo1 then
                ...
                if pPlayer1 then
                    ...
                    pPlayer1.CenterMsg("本场比赛无战斗发生，直接产生结果", true);
            ...
            if nPlayer2 and tbPlayerPKInfo2 then
                ...
                if pPlayer2 then
                    ...
                    pPlayer2.CenterMsg("本场比赛无战斗发生，直接产生结果", true);
...
function tbBaseLogic:ApplyFightTeamPlayer(pPlayer)
    ...
    if not tbFightTeam:HavePlayerID(pPlayer.dwOrgPlayerId) then
...
function tbFinalsPKLogic:OnCreate(tbMapLogic, nMapLogicId, nIndex, tbPlayInfo, nEndTime)
    ...
    self:ForeachAllPlayer({self.ShowInfoRMsg, self, "准备开始"});
...
function tbFinalsPKLogic:PlayGameEnd()
    self:ForeachAllPlayer({self.ShowInfoRMsg, self, "结算"});
...
function tbFinalsPKLogic:OnEnterMap(pPlayer)
    ...
    self:ShowInfoRMsg("准备开始", pPlayer);
    local tbFinalsDef = tbBaseLogic:GetFinalDef();
...
function tbFinalsPKLogic:WatchTeamPlayerNpcID(tbWatchTeamNpcID, pPlayer)
    if pPlayer.nFightMode == Npc.FIGHT_MODE.emFightMode_Death then --已死亡的不观战
        return
    end
    local pNpc = pPlayer.GetNpc();
    if not pNpc then
...
function HuaShanLunJian:CheckPlayerEnterFinalsMap(pPlayer)
    ...
    if pPlayer.nLevel < tbFinalsDef.nAudienceMinLevel then
        return false, string.format("「%s」等级不足%s无法参加", pPlayer.szName, tbFinalsDef.nAudienceMinLevel);
    end    

    local bRet = Map:CheckEnterOtherMap(pPlayer);
    if not bRet then
        return false, "您非安全区不能进入";
    end   
    ...
    if nTeamId > 0 then
        ...
        for _, nPlayerID in pairs(tbMember) do
            ...
            if nMemberTeamID ~= nFightTeamID then
                return false, string.format("%s不是同一个战队", pMember.szName), 0, true;
            end

            bRet = Map:CheckEnterOtherMap(pMember);
            if not bRet then
                return false, string.format("「%s」非安全区不能进入", pMember.szName), 0, true;
            end
...
function HuaShanLunJian:PlayerEnterFinalsMap(pPlayer)
    local tbFinalsDef = tbDef.tbFinalsGame
    local bRet, szMsg, nEnterPreMapID, bTeamMsg = self:CheckPlayerEnterFinalsMap(pPlayer);
    if not bRet then        
        pPlayer.CenterMsg(szMsg, true);
