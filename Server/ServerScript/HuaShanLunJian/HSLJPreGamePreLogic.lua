...
function tbPreGameMgr:StartPreGame()
    ...
    self.szShowUIMsg = string.format("等待第[FFFE0D]%s/%s[-]场比赛开始", 1, tbPreDef.nPlayerGameCount);
...
function tbPreGameMgr:OnEnterMatchTime()
    ...
    if bRetOpen then
        local tbNotPreData = HuaShanLunJian:GetPrepareGameNotSave();
        self.szShowUIMsg = string.format("等待第[FFFE0D]%s/%s[-]场比赛开始", tbPreDef.nPlayerGameCount - tbNotPreData.nPlayGameCount, tbPreDef.nPlayerGameCount);
        if tbPreDef.bShowMSgWhenEnterMatchTime then
            self:SendWorldMsg(tbPreDef.szPreOpenWorldMsg);
        end
    else
        self.szShowUIMsg = "比赛结束";
        self:CloseOnceTimer();
...
function tbPreGameMgr:PlayerMatchGameEmpty(pPlayer)
    ...
    local tbPreDef = self:GetPreDef()   
    pPlayer.CenterMsg(string.format("战队轮空，判定获胜，比赛时间增加%s", Lib:TimeDesc(tbPreDef.nMatchEmptyTime)));
    pPlayer.Msg(string.format("您的战队轮空了，系统判定阁下胜利，战队比赛时间增加%s。请等待下一场比赛开始！", Lib:TimeDesc(tbPreDef.nMatchEmptyTime)));
...
--重载
 ...
 function tbPreGameMgr:SetPlayerTeamTitle(pPlayer, nPlayTeam)
    ...
    local szText = string.format("战队·%s", szName);
