...
function HuaShanLunJian:PreEndChampionGuessing()
    ...
    KPlayer.BoardcastScript(tbGuessingDef.nMinLevel, "Ui:SynNotifyMsg", tbMsgData);
    Log("HuaShanLunJian PreEndChampionGuessing");
end
...
function HuaShanLunJian:PreStartFinalsPlayGame()
    ...
    KPlayer.SendWorldNotify(1, 999, "本届华山论剑冠军争夺战将在[FFFE0D]3分钟[-]後开始进场，请各位参赛选手提前准备！", 1, 1);
end
...
function HuaShanLunJian:ChampionFightTeamStatue()
    for nMapTID, _ in pairs(tbDef.tbStatueMapID) do
        Map:ClearMapStatue(nMapTID, "HuaShanLunJian");
    end
...
function HuaShanLunJian:GetPlayerStatueInfo(nPlayerID)
    ...
    local tbSexStatue = tbDef.tbFactionStatueNpc[pRole.nSex];
    if not tbSexStatue then
        return;
    end

    local nNpcTID = tbSexStatue[pRole.nFaction];
...
function HuaShanLunJian:SendTeamPreGameMailAward(nFightTeamID, tbAllAward, szContent)
    ...
    local nJoinCount = tbFightTeam:GetJoinCount();
    local nMaxCount = HuaShanLunJian:GetPreGameJoinCount(tbSaveData.nWeekDay);
    local nWinCount = tbFightTeam:GetWinCount();
    local nFailCount = tbFightTeam:GetFailCount();
    local nDogfallCount = tbFightTeam:GetDogfallCount(tbDef.tbPrepareGame.nWinJiFen, tbDef.tbPrepareGame.nDogfallJiFen);

    local szExtMsg = "(尚未获得赛季奖励资格)";
    if nJoinCount >= 4 then
        szExtMsg = "(已获得赛季奖励资格)";
    end

    local szExtMailContent = string.format("\n阁下当前所在战队比赛情况如下：\n[FFFE0D]完成比赛[-]：%s/%s%s \n[FFFE0D]比赛结果[-]：\n    胜：%s场；\n    平：%s场；\n    负：%s场", nJoinCount, nMaxCount, szExtMsg, nWinCount, nDogfallCount, nFailCount);
    local tbAllPlayer = tbFightTeam:GetAllPlayerID();
    szContent = szContent or "华山论剑预选赛奖励";
    szContent = szContent .. szExtMailContent;
    for nPlayerID, _ in pairs(tbAllPlayer) do
        local tbMail =
        {
            To = nPlayerID,
