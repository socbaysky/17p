            ...
            for nPos = nFrom, v.nRankEnd do
                ...
                if tbInfo then
                    Mail:SendSystemMail({
                        To = tbInfo.dwUnitID,
                        Title = "普天同庆排名奖励",
                        Text = string.format("大侠，本轮祝福活动你获得了来自好友的%s祝福值，排行%d名，人气真不错啊！这是您的奖励，请查收。", tbInfo.szValue, nPos),
                        tbAttach = { v.tbAward },
                    });
...
function SendBless.SendBlessCallBack(dwRoleId1, bSucceed, szBillNo, dwRoleId2, bUseGold, szWord)
    ...
    if not pPlayer2 then
        pPlayer.CenterMsg("对方不线上，不能送出祝福！")
        return false
    end

    if pPlayer2.nLevel < SendBless.nMinLevel then
        pPlayer.CenterMsg("对方等级不足" .. SendBless.nMinLevel)
        return false
    end
    ...
    if bUseGold then
        ...
        if tbActSetting.szColorSendMsg then
            ...
            ChatMgr:SendPlayerMsg(ChatMgr.ChannelType.Color, dwRoleId1, pPlayer.szName, pPlayer.nFaction, pPlayer.nPortrait, pPlayer.nSex, pPlayer.nLevel, string.format(tbActSetting.szColorSendMsg, pPlayer.szName, pPlayer2.szName, szWord) )
        end
