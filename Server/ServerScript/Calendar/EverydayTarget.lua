...
function EverydayTarget:AddCount(pPlayer, szKey, nAdd)
	SummerGift:OnJoinAct(pPlayer, szKey, nAdd)
	Activity:OnPlayerEvent(pPlayer, "Act_OnJoinAct", szKey, nAdd)
...
function EverydayTarget:TryGainAward(pPlayer, nAwardIdx)
	...
	if kinData then
		...
		pPlayer.CenterMsg(string.format("帮派增加了%d建设资金", nAddFound))
	end
