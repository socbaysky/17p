...
function Calendar:CheckData(pPlayer)
	...
	if #tbAllAward > 0 then
		local tbMail = {
            ...
            Title = "武林荣誉",
            From = "独孤剑",
            Text = "      尊敬的侠士，恭喜你在本月出类拔萃，成为武林荣誉榜上有名的人物。特备薄礼聊表敬意，还请侠士此後多加努力，更上一层楼！",
            tbAttach = tbAllAward,
...
function Calendar:OnCompleteActDirect(dwID, szAct, ...)
	...
	if nNewDiv > nLastDiv then
        ...
        pPlayer.TLog("WuLinHonorFlow", szAct, nAddHonor, nNewDiv, nLastDiv)
...
function Calendar:SendNotifyMsg(pPlayer, szKey, nDivision, _, nParam2)
    ...
    local szNotifyMsg    = string.format("恭喜侠士「%s」参加%s%s，成功晋级该活动本月%s段武林荣誉。", pPlayer.szName, szActName, szRank, self.tbDivisionName[nDivision])
...
function Calendar:OnCombineServer(tbOldData)
    local tbData = ScriptData:GetValue("HonorMonthRank")
    if not tbOldData.nMonth or tbOldData.nMonth ~= tbData.nMonth then
        return
    end
    tbData.tbRankInfo = tbData.tbRankInfo or {}
    for nPlayerId, tbInfo in pairs(tbOldData.tbRankInfo or {}) do
        tbData.tbRankInfo[nPlayerId] = tbInfo
    end
    ScriptData:AddModifyFlag("HonorMonthRank")
    Log("Calendar OnCombineServer Complete")
end
...
Calendar.MONTH_RANK_PLAYER_COUNT = 250
function Calendar:CheckMonthRank()
    ...
    local tbRankList = {tbKinName = {}, tbFullList = {}, tbList = {}}
    for nPlayerId, tbInfo in pairs(tbData.tbRankInfo) do
        ...
        if pPlayer then
            ...
            if not tbRankList.tbKinName[pPlayer.dwKinId] then
                local pKinData = Kin:GetKinById(pPlayer.dwKinId or 0)
                if pKinData then
                    tbRankList.tbKinName[pPlayer.dwKinId] = pKinData.szName 
                end
            end
            table.insert(tbRankList.tbFullList, tbCompleteInfo)
        end
    end
    if #tbRankList.tbFullList > 0 then
        table.sort(tbRankList.tbFullList, function (a, b)
        ...
        local nLen = math.min(#tbRankList.tbFullList, self.MONTH_RANK_PLAYER_COUNT)
        for i = 1, nLen do
            local tbInfo = tbRankList.tbFullList[i]
            local nPlatnumAct = self:Act2Number(tbInfo.tbPlatnumAct)
            tbInfo.tbPlatnumAct = nil
            tbInfo.nPlatnumAct = nPlatnumAct
            tbInfo.nChangeTime = nil
            table.insert(tbRankList.tbList, tbInfo)
        end
        tbRankList.tbFullList = nil
        NewInformation:AddInfomation("HonorMonthRank", GetTime() + 5*24*3600, tbRankList)
    end
    tbData.nMonth = nLocalMonth
    tbData.tbRankInfo = {}
    ScriptData:AddModifyFlag("HonorMonthRank")
    Log("Calendar SendMonthRank NewInformation", nDataMonth, #tbRankList.tbList)
end
