...
function ScheduleTask:OpenIndifferNotify(nMinute)
	KPlayer.SendWorldNotify(InDifferBattle.tbDefine.nMinLevel, 999, string.format("心魔幻境将在%d分钟後开始报名，届时报名时间将持续%d分钟，请各位侠士提前做好入场准备！", nMinute, math.floor(InDifferBattle.tbDefine.MATCH_SIGNUP_TIME / 60)), 1, 1)
end
