...
function c2s:OnFinishTaskDialog(nTaskId, nState, nNpcId)
	...
	if nState == Task.STATE_CAN_ACCEPT then
		if not Task:CheckCanClientAccetp(nTaskId) and (not me.nCurAcceptTaskId or me.nCurAcceptTaskId ~= nTaskId) then
			me.CenterMsg(string.format("任务%s无法通过用户端接取", nTaskId));
			return;
		end
...
function c2s:OnTeamRequest(szType, ...)
	if not me.CanTeamOpt(szType) then
		me.CenterMsg("目前地图无法进行此操作");
		return
	end
	TeamMgr:ClientRequest(szType, ...);
end
...
function c2s:OnZCBossRequest( ... )
	Boss:ZCClientRequest(...);
end
...
function c2s:UseItemBreakSkillMaxLv(nSkillId)
	FightSkill:DoBreakMaxLvByItem(me, nSkillId)
end
...
function c2s:RequestBlackHim(dwAlliedPlayerID)
	...
	if not bRet then
		me.CenterMsg(szMsg or "加入黑名单失败")
	else
		me.CenterMsg("您已将其拉入黑名单")
	end
end
...
function c2s:RequestViewRelationData(dwRoleId)
	FriendShip:RequestViewRelationData(me, dwRoleId)
end

function c2s:RequestChangeCanViewRelation(bCanNotViewFriend, bCanNotViewStrange)
	me.SetUserValue(FriendShip.SAVE_GROUP, FriendShip.SAVE_KEY_VIEW_FRIEND, bCanNotViewFriend and 1 or 0)
	me.SetUserValue(FriendShip.SAVE_GROUP, FriendShip.SAVE_KEY_VIEW_STRANGE, bCanNotViewStrange and 1 or 0)
	Log("RequestChangeCanViewRelation", me.dwID, tostring(bCanNotViewFriend), tostring(bCanNotViewStrange))
	FriendShip:UpdateCanViewRelation(me)
end
...
function c2s:SendChatBQ(nChatID, NpcType)
	if type(nChatID) ~= "number" then
		return;
	end

	ChatMgr:PlayerChatActionBQ(me, nChatID, NpcType);
end

function c2s:DoChangeFaction(nFaction, nSex)
	if type(nFaction) ~= "number" then
		return;
	end

	if type(nSex) ~= "number" then
		return;
	end

	if not Faction.tbFactionInfo[nFaction] then
		me.CenterMsg("没有当前的门派!!", true);
        return;
    end

	ChangeFaction:PlayerChangeFaction(me, nFaction, nSex);
end
...
local tbViewPlayerWndKey = {
	["ViewRolePanel"] = 26; --
	["NewYearQAAnswerPanel"] = 10;
	["ViewFight"] = 10; --装备,门派
	["ChuangGongPanel"] = 10;
}

function c2s:ViewPlayerInfo(dwRoleId, szWnd, nServerId)
	local nKey = tbViewPlayerWndKey[szWnd]
	if not nKey then
		Log("Error!! ViewPlayerInfo", dwRoleId, szWnd)
		return
	end
	if nServerId and nServerId == GetServerIdentity() then
		nServerId = nil
	end
	local bRet = me.SyncOtherPlayerAsyncData(dwRoleId, nKey, nServerId);
	if not bRet then
		me.CenterMsg("同步资料失败！");
		Log("[Helper] ERR ?? ViewPlayerInfo bRet is false !", me.szName, me.dwID, dwRoleId);
		return;
	end
	local tbRole = Player:GetRoleStayInfo(dwRoleId, true, nServerId)
	tbRole.nServerId = nServerId --查看别的服的nServerId才非空
...
RegisterZoneC2S("RequestMailData", WS_EXECUTE);
RegisterZoneC2S("RecordReadMails", WS_EXECUTE);
RegisterZoneC2S("TakeMailAttach", WS_EXECUTE);
...
function c2s:TakeDaysCardAward(nIndex)
	Recharge:TakeDaysCardAward(me, nIndex)
end
...
function c2s:RequestHaveCrossServer()
	me.CallClientScript("BossLeader:SyncCrossTimeFrame", BossLeader:GetCrossBossTimeFrame());
...
function c2s:SendActInteract(nPassivityID, nInteractID)
    ActionInteract:DoActionInteract(me.dwID, nPassivityID, nInteractID, true);
end

function c2s:UnbindLinkInteract()
	ActionInteract:UnbindLinkInteract(me);
	me.CallClientScript("Ui:CloseWindow", "QYHLeavePanel");
end
...
function c2s:MonsterNianReq(szReq, ...)
	local bOk, szErr = Kin.MonsterNian:ClientReq(me, szReq, ...)
...
function c2s:CallMarketStallFunc(szFunc, ...)
	...
	if not MarketStall:CheckOpen() then
		me.CenterMsg("摆摊正在维护中！");
		return;
	end

	local bRet, szMsg = MarketStall:IsMarketOpen(me)
	if not bRet then
		me.CenterMsg(szMsg or "未知错误")
		return;
	end
...
function c2s:TryJoinKinSecret(nIdx)
	Fuben.KinSecretMgr:TryEnterMap(me, nIdx)
end
...
RegisterZoneC2S("DoKinRedBagReq", WS_EXECUTE);
...
function c2s:DomainBattleRequestAttackInfo()
	DomainBattle:RequestAttackInfo(me)
end
...
function c2s:DoChangeWaiyiBg(nBgId)
	Item.tbChangeColor:ChangeWaiyiBg(me, nBgId)
end
...
function c2s:ImperialTombLeave()
	if me.nFightMode == 1 then
		GeneralProcess:StartProcess(me, 5 * Env.GAME_FPS, "传送中...", ImperialTomb.LeaveRequest, ImperialTomb, me)
	else
...
function c2s:QingRenJieRespon(szEvent, ...)
	local szSafeEvent = {["Act_QingRenJie_AgreeInvite"] 	 = 1,
						 ["Act_QingRenJie_AgreeInviteDazuo"] = 1,
						 ["Act_QingRenJie_TryDazuo"] 		 = 1,
						 ["Act_QingRenJie_ChooseTitle"] 	 = 1}
	if not szSafeEvent[szEvent] then
		Log("QingRenJieRespon UnsafeEvent", me.dwID, szEvent)
		return
	end
...
function c2s:MagicBowlReq(szReqType, ...)
	Furniture.MagicBowl:OnReq(me, szReqType, ...)
end
...
function c2s:ChangeHousePosWaiYi(nPosTypeId, nWaiYiId)
	House:ChangePosWaiYi(me, nPosTypeId, nWaiYiId);
end
...
function c2s:RequestBuyDressMoney(nIndex)
	Recharge:RequestBuyDressMoney(me, nIndex)
end
...
function c2s:GotoMagicBowl()
	if not Env:CheckSystemSwitch(me, Env.SW_SwitchMap) then
		me.CenterMsg("目前状态不允许切换地图")
		return
	end

	House:GotoMagicBowl(me);
end
...
function c2s:NewYearChrisReq(szReqType, ...)
	if not Activity:__IsActInProcessByType("NewYearChris") then
		me.CenterMsg("活动已经结束", true)
		return 
	end
	Activity:OnPlayerEvent(me, "Act_NewYearChrisReq", szReqType, ...)
end
...
function c2s:Act_DaiYanRenAct(szEvent, ...)
	local szSafeEvent = {["DYRAct_TryAgreeEnterFuben"] = 1,
						 ["DYRAct_TryAgreeApply"] 	   = 1}
	if not szSafeEvent[szEvent] then
		Log("Act_DaiYanRenAct UnsafeEvent", me.dwID, szEvent)
		return
	end
	Activity:OnPlayerEvent(me, szEvent, ...)
end
...
function c2s:ReportOperate(nTime)
	if not nTime then
		return;
	end

	AddictionTip:OnReportOperate(me, nTime);
end

function c2s:DoRequesActCollectAndRobClue(szType, ... )
    local FunCall = Activity:GetClass("CollectAndRobClue").tbC2SRequest[szType];
    if not FunCall then
        return;
    end
...
function c2s:UseDiscountCoupon(nId, nCount)
	...
	if szClassName ~= "DiscountCoupon" then
		return
	end
	local tbClass = Item:GetClass(szClassName)
	if tbClass and tbClass.OnUse then
		tbClass:OnUse(pItem, nCount)
	end
end

function c2s:OnClientCallWuLinDaShiFunc(...)
	WuLinDaShi:OnClientCall(...)
end

function c2s:SendPandoraSeekHelpMsg(nSendChannel, szSendContent, szJumpTarget, szExtends)
	...
	if not szSendContent or type(szSendContent) ~= "string" or szSendContent == "" then
		return
	end

	if not szJumpTarget or type(szJumpTarget) ~= "string" or szJumpTarget == "" then
		return
	end

	if nSendChannel == ChatMgr.ChannelType.Public then
		if not ChatMgr:ReducePublicChatCount(me, 1) then
			return
		end
	end

	local szMsg = string.format("<%s>", szSendContent)
	local tbLinkData = {nLinkType = ChatMgr.LinkType.HyperText, linkParam={szHyperText = string.format("[url=pandoraSeekHelp:Msg, %s, %s][-]", szJumpTarget, szExtends or "")}}

	ChatMgr:SendPlayerMsg(nSendChannel, me.dwID, me.szName,
					 me.nFaction, me.nPortrait, me.nSex,
					 me.nLevel, szMsg, tbLinkData)
end

function c2s:SendZhenFaTaskAnwser(nTaskId, nSelect)
	ZhenFaTask:OnSendAnswer(me, nTaskId, nSelect);
end

function c2s:AcceptZhenFaTask()
	local nIdx = ZhenFaTask.nMaxTaskCount - DegreeCtrl:GetDegree(me, "ZhenFaTask");
	Npc:GetClass("ZhenFaTaskNpc"):Accept();
end

function c2s:ZhenFaOnClientCall(...)
	ZhenFa:OnClientCall(...)
end

function c2s:QYHCrossClientCall(szFun, ...)
	QunYingHuiCross:OnClientCall(szFun, me, ...)
end

function c2s:RequestOpenPandoraPlayerSpace(dwRoleId)
	...
	local tbParam = {
		dwRoleId = dwRoleId,
		szOpenId = pPlayer.szAccount,
	};
	me.CallClientScript("Pandora:OnOpenPlayerSpace", tbParam)
end

function c2s:CrossDomainSyncAidBriefReq(nVersion)
	DomainBattle.tbCross:OnSyncAidBriefInfo(me, nVersion)
end

function c2s:CrossDomainSyncAidListReq(nVersion)
	DomainBattle.tbCross:OnSyncAidList(me, nVersion)
end

function c2s:CrossDomainAidSignUpReq(nKinId)
	DomainBattle.tbCross:OnAidRequest(me, nKinId)
end

function c2s:CrossDomainEnterReq()
	DomainBattle.tbCross:EnterRequest(me)
end

function c2s:CrossDomainCityMiniMapStatueReq()
	DomainBattle.tbCross:CityMiniMapStatueReq(me)
end

function c2s:SuperVipReq(...)
	SuperVip:OnClientReq(me, ...)
end

function c2s:LabaActClientCall(szCall, ...)
	if not Activity:__IsActInProcessByType("LabaAct") then
		me.CenterMsg("活动已经结束", true)
		return
	end
	Activity:OnPlayerEvent(me, "Act_OnLabaActClientCall", szCall, ...);
end

function c2s:TryAddZoneExtPoint(nTaskId)
	Task:TryAddZoneExtPoint(me, nTaskId)
end

function c2s:PetReq(szType, ...)
	local bOk, szErr = Pet:ClientReq(me, szType, ...)
	if not bOk and szErr then
		me.CenterMsg(szErr)
		return
	end
...
function c2s:NewYearQAClientCall(szFunc, ...)
	Activity:OnPlayerEvent(me, "Act_NewYearQA_ClientCall", szFunc, ...);
end

function c2s:ChouJiangClientCall(szFunc, ...)
	Activity:OnPlayerEvent(me, "Act_ClientCall", szFunc, ...);
end

function c2s:RequestUnComposeItem(nItemId)
	Compose.UnCompose:UncomposeItem(me, nItemId)
end

function c2s:KinSecretReq(szType, ...)
	Fuben.KinSecretMgr:OnClientReq(me, szType, ...)
end

function c2s:JingMaiReq(szFun, ...)
	JingMai:OnClientCall(szFun, me, ...)
end

function c2s:AutoPlayChatEquipActionBQ()
	ChatMgr:AutoPlayChatEquipActionBQ(me);
end

function c2s:SendGoodVoiceChannelMsg(nType, nParam)
	if not Activity:__IsActInProcessByType("GoodVoice") then
		me.CenterMsg("活动已经结束", true)
		return
	end
	Activity:OnPlayerEvent(me, "Act_SendGoodVoiceChannelMsg", nType, nParam);
end

function c2s:GoodVoiceVotedAwardReq(nIndex)
	if not Activity:__IsActInProcessByType("GoodVoice") then
		me.CenterMsg("活动已经结束", true)
		return
	end
	Activity:OnPlayerEvent(me, "Act_GoodVoiceVotedAwardReq", nIndex);
end

function c2s:GoodVoiceSignUpFriendReq()
	if not Activity:__IsActInProcessByType("GoodVoice") then
		me.CenterMsg("活动已经结束", true)
		return
	end
	Activity:OnPlayerEvent(me, "Act_GoodVoiceRequestSignUpFriend");
end

function c2s:GoodVoiceUnSignUpFriendReq()
	if not Activity:__IsActInProcessByType("GoodVoice") then
		me.CenterMsg("活动已经结束", true)
		return
	end
	Activity:OnPlayerEvent(me, "Act_GoodVoiceRequestUnSignUpFriend");
end

function c2s:GoodVoiceRecommondReq(nPlayerId)
	if not Activity:__IsActInProcessByType("GoodVoice") then
		me.CenterMsg("活动已经结束", true)
		return
	end
	Activity:OnPlayerEvent(me, "Act_GoodVoiceRecommendFriend", nPlayerId);
end

function c2s:MedalFightReq(szType, ...)
	Activity:OnPlayerEvent(me, "Act_MedalFightReq", szType, ...)
end

function c2s:DanceActRequest(szFunc, ...)
	Activity.DanceMatch:DanceActRequest(me, szFunc, ...)
end

function c2s:WorldCupReq(szType, ...)
	Activity:OnPlayerEvent(me, "Act_WorldCupReq", szType, ...)
end
