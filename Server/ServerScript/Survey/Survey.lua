...
function Survey:LoadCfg()
	self.tbSortedGrpIds = {}
	self.tbCfgs = {}
	local tbMainCfg = self:LoadTabFileOutside("Setting/Survey/Survey.tab", {GroupID=1, Priority=1, Type=1, RewardGold=1, MinLevel=1, 
	...
	for _,v in ipairs(tbMainCfg) do
		...
		self.tbCfgs[v.GroupID].nOpenTime = Lib:ParseDateTime(v.OpenTime)
		self.tbCfgs[v.GroupID].nCloseTime = Lib:ParseDateTime(v.CloseTime)
		local tb = {}
...
function Survey:IsConditionValid(pPlayer, nGroupId)
	...
	local nNow = GetTime()
	if tbCfg.nOpenTime and tbCfg.nOpenTime>0 and nNow<tbCfg.nOpenTime then
		return false
	end
	if tbCfg.nCloseTime and tbCfg.nCloseTime>0 and nNow>tbCfg.nCloseTime then
		return false
	end
