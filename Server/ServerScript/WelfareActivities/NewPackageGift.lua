...
NewPackageGift.MAIL_CONTENT    = "    %d年%d月%d号前创建且等级在%d级以上的角色，若前去[c8ff00] [url=openNewPackageUrl:下载最新版本, %s, %s][-]，就将获得[FFFE0D]%s[-]的奖励。天下武功，唯快不破。此刻不去下载，更待何时！"
...
function NewPackageGift:TryGainGift(nVersion)
    ...
    if not bCanGain then
        me.CenterMsg("活动已过期或已领取")
        return
    end

    local tbData = ScriptData:GetValue(self.MAIN_KEY)
    if tbData.nVersion ~= nVersion then
        me.CenterMsg("该版本无法参加活动")
        return
    end
