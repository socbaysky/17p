...
function FriendRecall:AddRenownAward(nPlayerId, szType)
	...
	if nSaveKey then
		...
		if nAwardCount >= self.Def.MAX_RENOWN_COUNT_WEEKLY then
			pPlayer.CenterMsg(XT("本周通过此活动获得的玩家召回额外名望已达上限"));
			return
		end
	...
	if nAddValue < nValue then
		pPlayer.CenterMsg(string.format(XT("本周通过玩家召回获得的额外名望已达%d上限"), self.Def.MAX_RENOWN_WEEKLY));
	end
end
...
function FriendRecall:SendRecalledMail(pPlayer, tbList)
	...
	local szContent = string.format(XT([[侠士 %s 已重新回到精彩纷呈的江湖，阁下与他相识已久，默契仍在，只需两人组队参加特定活动，将获得[FFFE0D]亲密度获取增加100%%\n野外打怪经验加成+5%%、攻击力+70、全抗+70、生命+1500[-]的增益状态，[FFFE0D]组队秘境、淩绝峰、山贼秘窟、惩恶任务、通天塔、心魔幻境[-]阁下还将额外获得名望奖励，有效期[FFFE0D]30[-]天]]), pPlayer.szName)

	local tbMail = {Title = "重聚江湖", From = "独孤剑", Text = szContent}
