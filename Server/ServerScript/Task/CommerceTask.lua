...
function CommerceTask:CommitGather(pPlayer, nIndex)
	...
	self:SyncCommerceData(pPlayer);
	pPlayer.CenterMsg("装箱成功");
	Achievement:AddCount(pPlayer, "CommerceTask_1", 1)
	Activity:OnPlayerEvent(pPlayer, "Act_OnCommitItem")
...
function CommerceTask:CommitItem(pPlayer, nIndex)
	...
	local szTip = szInfo ~= "" and string.format("装箱成功，获得奖励 %s", szInfo) or string.format("装箱成功")
...
function CommerceTask:FinishTask(pPlayer, bConfirm)
	...
	if nFinishCount < self.COMPLETE_COUNT then
		pPlayer.CenterMsg(string.format("需收集%d个以上完成任务", self.COMPLETE_COUNT))
		return;
	end

	if not bConfirm and nFinishCount < self.ALL_COMPLETE_COUNT then
		pPlayer.MsgBox(CommerceTask.szNotAllCompleteMsg, {{"确认", CommerceTask.FinishTask, CommerceTask, pPlayer.dwID, true}, {"取消"}})
		return
	end
	...
	if nFinishCount >= self.ALL_COMPLETE_COUNT then
		Activity:OnPlayerEvent(pPlayer, "Act_FinishCommerceTask", Env.LogWay_FinishCommerceTask)
	end
...
function CommerceTask:AskHelp(pPlayer, nRequestTaskId)
	...
	pPlayer.CenterMsg("成功发起帮派求助");
	self:SyncCommerceData(pPlayer);

	local szKinMsg = string.format("<协助完成>「%s」发布了商会任务的求助，正在寻找【%s】×%d", pPlayer.szName, szItemName, tbTaskSetting.nCount);
...
function CommerceTask:CheckHelpEnable(pPlayer, nTarPlayerId, nTaskId, bCoin)
	...
	if tbTaskSetting.szType == "Item" then
		if not bCoin then
			local nHas = pPlayer.GetItemCountInAllPos(tbTaskSetting.nTemplateId);
			if nHas < tbTaskSetting.nCount then
				return false, string.format("你没有足够的%s", tbBaseInfo.szName or "")
			end
		end
	else
		return false, "此任务无法协助完成"
	end
...
function CommerceTask:OnHelpSuccess(pPlayer, nTarPlayerId, nDataPos, nTaskId)
	...
	local tbBaseInfo = KItem.GetItemBaseProp(tbTaskSetting.nTemplateId);
	local szItemName = tbBaseInfo.szName;
	local szKinMsg = string.format("「%s」协助「%s」完成了商会需求：【%s】", pPlayer.szName, pRoleStayInfo.szName, szItemName)
	...
	if pPlayer.dwKinId ~= 0 then
		...
		pPlayer.CallClientScript("CommerceTask:OnRespondHelp", string.format("获得%d贡献", nAddCount))
	end
...
function CommerceTask:RegisterActAward(fnGetAward, tbActInst)
	self.tbActAward = {fnGetAward, tbActInst}
end

function CommerceTask:UnRegisterActAward()
	self.tbActAward = nil
end

function CommerceTask:SendNotifyMail(szItemName, nCount, szPlayerName, nTarPlayerId)
	local szMailContent = string.format("你发布的商会求助【%s】×%d，已经被「%s」协助完成了。", szItemName, nCount, szPlayerName);
	-- local tbMail =
	-- {
	-- 	To = nTarPlayerId,
	-- 	Title = "商会协助",
	...
	-- 	From = "「商会老板」万金财",
	-- };
	local pTarPlayer = KPlayer.GetPlayerObjById(nTarPlayerId)
	if pTarPlayer then
		pTarPlayer.CenterMsg(szMailContent)
		Activity:OnPlayerEvent(nTarPlayerId, "Act_OnCommitItem")
	elseif self.tbActAward then
		local _, tbAward = Lib:CallBack(self.tbActAward)
