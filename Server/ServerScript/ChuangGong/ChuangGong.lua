...
function ChuangGong:RequestGetChuangGong(pPlayer, dwSendId)
	...
	if not ChuangGong.tbWithoutCDVip[pPlayer2.GetVipLevel()] and nCDTime > 0 then
		pPlayer.CenterMsg(string.format("对方还需要再等%d分钟才能再次传功", math.ceil(nCDTime / 60)))
		return
	end  
	...
	pPlayer.CenterMsg(string.format("你向「%s」发起了传功请求，等待对方确认", pPlayer2.szName))
end
...
function ChuangGong:RequestSendChuangGong(pPlayer, dwGetId, bConFirm)
	...
	if not ChuangGong.tbWithoutCDVip[pPlayer.GetVipLevel()] and nCDTime > 0 then
		pPlayer.CenterMsg(string.format("您还需要再等%d分钟才能再次传功", math.ceil(nCDTime / 60)))
		return
	end  
	...
	if nGeterCount == 0 and DegreeCtrl:GetDegree(pGeter, "ChuangGongBuy") == 0 then --todo BUY 最好就别判断了
		pPlayer.CenterMsg("对方的接受传功次数不足，无法进行传功")
		return
	end
	...
	pPlayer.CenterMsg(string.format("你向「%s」发起了传功，等待对方确认", pGeter.szName))
end
...
function ChuangGong:IsAreaValid(pGeter, pSender)
	if not ChuangGong:IsWhiteMap(pGeter.nMapTemplateId) and (Map:GetClassDesc(pGeter.nMapTemplateId) ~= "fight" or pGeter.nFightMode ~= 0) then
		return false, string.format("「%s」接受了你的传功请求，但你所在区域不可传功", pSender.szName), string.format("「%s」所在区域不可传功，传功取消", pGeter.szName)
	elseif not ChuangGong:IsWhiteMap(pSender.nMapTemplateId) and (Map:GetClassDesc(pSender.nMapTemplateId) ~= "fight" or pSender.nFightMode ~= 0) then
		return false, string.format("「%s」所在区域不可传功，传功取消", pSender.szName), string.format("「%s」接受了你的传功请求，但你所在区域不可传功", pGeter.szName)
	...
	elseif ActionInteract:IsInteract(pGeter) then
		return false, string.format("「%s」接受了你的传功请求，但你动作交互不可传功", pSender.szName), string.format("「%s」动作交互不可传功，传功取消", pGeter.szName)
	elseif ActionInteract:IsInteract(pSender) then
		return false, string.format("「%s」动作交互不可传功，传功取消", pSender.szName), string.format("「%s」接受了你的传功请求，但你动作交不可传功", pGeter.szName)	
	end
	return true
end
...
function ChuangGong:IsCDValid(pGeter, pSender, bDisableCD)
	...
	if not ChuangGong.tbWithoutCDVip[pSender.GetVipLevel()] and nCDTime > 0 then
		return false, string.format("「%s」还需要再等%d分钟才能再次传功", pSender.szName, math.ceil(nCDTime / 60))
	end
	return true
end
...
function ChuangGong:IsStateValid(pGeter, pSender)
	if pGeter.nMapTemplateId == Kin.Def.nKinMapTemplateId and pGeter.nFightMode ~= 0 then
		return false, string.format("「%s」接受了你的传功请求，但你处於战斗状态", pSender.szName), string.format("「%s」处於战斗状态不可传功，传功取消", pGeter.szName)
	elseif pSender.nMapTemplateId == Kin.Def.nKinMapTemplateId and pSender.nFightMode ~= 0 then
		return false, string.format("「%s」处於战斗状态不可传功，传功取消", pSender.szName), string.format("「%s」接受了你的传功请求，但你处於战斗状态不可传功", pGeter.szName)
	elseif not Env:CheckSystemSwitch(pGeter, Env.SW_ChuangGong)	then
		return false, string.format("「%s」接受了你的传功请求，但你当前状态不允许传功", pSender.szName), string.format("「%s」目前状态不允许传功，传功取消", pGeter.szName)
	elseif not Env:CheckSystemSwitch(pSender, Env.SW_ChuangGong) then
		return false, string.format("「%s」目前状态不允许传功，传功取消", pSender.szName), string.format("「%s」接受了你的传功请求，但你当前状态不允许传功", pGeter.szName)
	elseif pGeter.bWeddingDressOn then
		return false, "你当前穿着婚服不允许传功", string.format("「%s」当前穿着婚服不允许传功，传功取消", pGeter.szName)
	elseif pSender.bWeddingDressOn then
		return false, string.format("「%s」当前穿着婚服不允许传功，传功取消", pSender.szName), "你当前穿着婚服不允许传功"
	elseif pGeter.nState ~= Player.emPLAYER_STATE_NORMAL then
		return false, "你当前状态不允许传功", string.format("「%s」目前状态不允许传功，传功取消", pGeter.szName)
	elseif pSender.nState ~= Player.emPLAYER_STATE_NORMAL then
		return false, string.format("「%s」目前状态不允许传功，传功取消", pSender.szName), "你当前状态不允许传功"
	end
	return true
end
...
function ChuangGong:CheckBeforeBegin(pGeter, pSender, bCheckDegree)
	...
	if nPlayerCount < 1 then
		return false, string.format("%s接受传功次数不足", pGeter.szName)
	end

	local nPlayer2Count = ChuangGong:GetDegree(pSender, "ChuangGongSend")
	if nPlayer2Count < 1 then
		return false, string.format("%s传功次数不足", pSender.szName)
	end
	return true
end
...
function ChuangGong:AddPlayerExp(dwGetId, dwSendId, nGetAdd, nSendAdd, szType)
	...
	if not pGeter then
		if pSender then
			self:RemoveState(pSender, tbSenderEffect, "对方已下线,传功结束")
		end
		return false
	end
	if not pSender then
		self:RemoveState(pGeter, tbGeterEffect, "对方已下线,传功结束")
		return false
	end
	...
	if not nTimes then
		self:RemoveState(pGeter, tbGeterEffect, "数据错乱，传功结束")
		self:RemoveState(pSender, tbSenderEffect, "数据错乱，传功结束")
		return false
	end
	...
	if pGeter.nMapTemplateId ~= pSender.nMapTemplateId or pGeter.nMapId ~= pSender.nMapId then
		self:RemoveState(pSender, tbSenderEffect, "对方切换了地图，传功结束")
		self:RemoveState(pGeter, tbGeterEffect, "对方切换了地图，传功结束")
	...
	if nDis > self.MaxDistance then
		self:RemoveState(pSender, tbSenderEffect, "距离太远传功结束")
		self:RemoveState(pGeter, tbGeterEffect, "距离太远传功结束")
...
function ChuangGong:DoReduceDegree(pGeter, pSender, tbSetting)
	...
	if not nGeterRet then
		return false, string.format("%s接受传功次数扣除失败", pGeter.szName)
	end

	local nSenderRet = ChuangGong:ReduceDegree(pSender, "ChuangGongSend")
	if not nSenderRet then
		return false, string.format("%s传功次数扣除失败", pSender.szName)
	end
	return true
end
...
function ChuangGong:SelfChuanGong(nPlayerID)
    ...
    if pPlayer.nMapTemplateId ~= Kin.Def.nKinMapTemplateId then
    	pPlayer.CenterMsg("请回到帮派再进行修炼",true)
		return
    end

    local nPlayerCount = ChuangGong:GetDegree(pPlayer, "ChuangGong")
	if nPlayerCount < 1 then
		pPlayer.CenterMsg("少侠被传功次数不足，无法进行修炼",true)
		return
	end

	local nRet = ChuangGong:ReduceDegree(pPlayer, "ChuangGong")
	if not nRet then
		pPlayer.CenterMsg("被传功次数扣除失败", true)
		return false
	end
