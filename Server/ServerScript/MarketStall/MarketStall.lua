...
function MarketStall:DoCheckSendWarningMail()
	if Lib:GetMonthDay()~=self.tbWarningSettings.nMailMonthDay or version_kor then
		return
	end

	local szKey = "MarketStallWarningMailTime"
	local nLastSend = tonumber(ScriptData:GetValue(szKey)) or 0
	local nTimeNow = GetTime()
	if not Lib:IsDiffMonth(nTimeNow, nLastSend) then
		return
	end
	ScriptData:SaveAtOnce(szKey, nTimeNow)

	Mail:SendGlobalSystemMail({
		Title = "交易秩序温馨提醒",
		Text = "亲爱的少侠：\n    官方始终严厉打击倒卖及定向转移元宝等非正常游戏行为，请各位自觉维护正常交易环境，以免造成扣款、封号等损失。",
		From = "剑侠情缘手游项目组",
		LevelLimit = self.tbWarningSettings.nMailLevelMin,
	})
	Log("MarketStall:DoCheckSendWarningMail", nTimeNow, nLastSend)
end
...
function MarketStall:CostGold_UpdateMyItemPrice(nPlayerId, bSuccess, nStallId, nCurPrice)
	...
	local tbTmpInfo = self:GetStallItemInfo(nStallId, true);
	pPlayer.CallClientScript("MarketStall:OnEvent", self.emEvent_OnCancelSellItem, true, tbTmpInfo);
	if self.tbStallInfoByPlayer and self.tbStallInfoByPlayer[pPlayer.dwID] then
		self.tbStallInfoByPlayer[pPlayer.dwID][nStallId] = nil;
	end
	self.tbAllStall[nStallId] = nil;
	MarketMgr.CancelStallItem(nStallId);

	local nTime = GetTime();
	local nNewStallId = MarketMgr.NewStallItem(pPlayer.dwID, tbStallInfo.szMainType, tbStallInfo.nSubType, nCurPrice, tbStallInfo.nCount, nTime, self.nCurMaxStallId);
	if nNewStallId <= 0 then
		Log("[MarketStall] ERR ?? NewStallItem Error ", nNewStallId, pPlayer.dwID, szMainType, nSubType, nPrice, nCount, nTime);
		return false, "摆摊系统异常";
	end

	self:LoadData(nNewStallId, tbStallInfo.nPlayerId, tbStallInfo.szMainType, tbStallInfo.nSubType, nCurPrice, tbStallInfo.nCount, tbStallInfo.nCacheMoney, tbStallInfo.nMoneyType, nTime);
	pPlayer.CallClientScript("MarketStall:OnEvent", self.emEvent_OnNewSellItem, true, self:GetStallItemInfo(nNewStallId, true));
...
-- 下架物品 bResult, {...}
function MarketStall:CancelSellItem(pPlayer, nStallId, bNotNotify, bSendSellCost)
	local nPlayerId;
	...
	if tbStallInfo.nCount > 0 then
		...
		local tbAllAward = {tbAward};
		if bSendSellCost then
			local nSellCost = MarketStall:GetSellCost(tbStallInfo.nCount * tbStallInfo.nPrice);
			table.insert(tbAllAward, {"Gold", nSellCost});
		end
		if pPlayer then
			pPlayer.SendAward(tbAllAward, nil, nil, Env.LogWay_MarketStallCancelItem);
		else
			local tbMail = {
				To = tbStallInfo.nPlayerId;
				Title = "物品下架返还";
				From = "系统";
				Text = "物品下架返还物品，请在附件查收！";
				tbAttach = tbAllAward;
...
function MarketStall:GetMySellItemInfo(pPlayer)
	...
	for _, nId in pairs(tbClearStallId) do
		tbPlayerStallInfo[nId] = nil;
	end
...
function MarketStall:CancelAttention(pPlayer, szMainType, nSubType)
	nSubType = nSubType or 0;
	...
	local tbAttention = pPlayer.GetScriptTable("MSAttention");
	local tbData = tbAttention.tbData or {}
	if not tbData[szMainType] or not tbData[szMainType][nSubType] then
		return;
	end

	tbData[szMainType][nSubType] = nil;
	tbAttention.tbData = tbData

	self:_NotifyAttentionChange(pPlayer, szMainType, nSubType, "cancel")
end

function MarketStall:_NotifyAttentionChange(pPlayer, szMainType, nSubType, szChangeType)
	pPlayer.CallClientScript("MarketStall:OnEvent", self.emEvent_OnAttentionChange, szMainType, nSubType, szChangeType)
end

function MarketStall:SetAttention(pPlayer, szMainType, nSubType)
	nSubType = nSubType or 0;
	...
	local tbAttention = pPlayer.GetScriptTable("MSAttention");
	local tbData = tbAttention.tbData or {}
	if tbData[szMainType] and tbData[szMainType][nSubType] then
		return;
	end

	local nAttentionCount = 0;
	for szMainType, tbInfo in pairs(tbData) do
		for nSubType in pairs(tbInfo) do
			nAttentionCount = nAttentionCount + 1;
		end
	end

	if nAttentionCount >= self.nMaxAttentionCount then
		return;
	end

	tbData[szMainType] = tbData[szMainType] or {};
	tbData[szMainType][nSubType] = true;
	tbAttention.tbData = tbData

	self:_NotifyAttentionChange(pPlayer, szMainType, nSubType, "set")
end

function MarketStall:GetAttentionList(pPlayer)
	local tbAttention = pPlayer.GetScriptTable("MSAttention");
	local tbData = tbAttention.tbData or {}
	...
	for szMainType, tbInfo in pairs(tbData) do
		for nSubType in pairs(tbInfo) do
			local nNormalPrice = self:GetPriceInfo(szMainType, nSubType);
			if not nNormalPrice then
				tbRemoveInfo[szMainType] = tbRemoveInfo[szMainType] or {};
				tbRemoveInfo[szMainType][nSubType] = true;
			end
		end
	end

	for szMainType, tbInfo in pairs(tbRemoveInfo) do
		for nSubType in pairs(tbInfo) do
			tbData[szMainType][nSubType] = nil;
		end
		if Lib:CountTB(tbData) <= 0 then
			tbData[szMainType] = nil;
		end
	end
	tbAttention.tbData = tbData

	pPlayer.CallClientScript("MarketStall:OnEvent", self.emEvent_OnSyncAttentionList, tbData);
end

function MarketStall:GetAttentionListItem(pPlayer)
	...
	local tbResult = {};
	for nType, tbTypeInfo in ipairs(tbPlayerShowInfo) do
		for _, nStallId in pairs(tbTypeInfo) do
			local tbSInfo = self:GetStallInfoByStallId(nStallId);
			local tbStallInfo = self:GetStallItemInfo(nStallId);
			if self:CheckIsAvaliableStallInfo(nStallId, tbSInfo) and
				self:CheckStallCanShow(nil, tbStallInfo, 0, {}) and
				tbData[tbStallInfo.szMainType] and tbData[tbStallInfo.szMainType][tbStallInfo.nSubType or 0] then

				local bHas = false;
				for _, tbInfo in pairs(tbResult) do
					if tbInfo.szMainType == tbStallInfo.szMainType
						and (tbInfo.nSubType or 0) == (tbStallInfo.nSubType or 0)
						and tbInfo.nPrice == tbStallInfo.nPrice then

						tbInfo.nCount = tbInfo.nCount + tbStallInfo.nCount;
						bHas = true;
						break;
					end
				end
				if not bHas then
					table.insert(tbResult, tbStallInfo);
				end
	...
	tbResult = self:FormatShowListWhithBuyFromSystem(tbResult, tbPlayerShowInfo);
	pPlayer.CallClientScript("MarketStall:OnEvent", self.emEvent_OnSyncAttentionListItem, tbResult, tbPlayerShowInfo.nLastUpdateTime);
end
...
function MarketStall:GetItemList(pPlayer, nType, nStartIdx, nEndIdx)
	...
	for i = #tbCurTypeInfo, 1, -1 do
		...
		local tbStallInfo = self:GetStallInfoByStallId(nStallId);
		if not self:CheckIsAvaliableStallInfo(nStallId, tbStallInfo) then
...
function MarketStall:CheckIsAvaliableStallInfo(nStallId, tbStallInfo)
	if not tbStallInfo then
		return false;
	end

	if nStallId < self.MIN_EXT_STALL_ID and not tbStallInfo.tbActiveTable then
		return false;
	end

	if tbStallInfo.bTimeout or tbStallInfo.nCount <= 0 then
		return false;
	end
...
function MarketStall:FormatShowListWhithBuyFromSystem(tbResult, tbPlayerShowInfo)
	local tbToRemove = {};
...
function MarketStall:UpdateAllStall(pPlayer, bCostMoney, nLastUpdateTime)
	...
	if pPlayer.GetVipLevel()<self.nManualRefreshVipMin then
		pPlayer.CenterMsg(string.format("需要达到剑侠尊享%d级，才可刷新", MarketStall.nManualRefreshVipMin))
		return
	end
	...
	if self.tbUpdateTimes[pPlayer.dwID] >= 100 then
		pPlayer.CenterMsg("今日已达摆摊刷新次数上限！");
		return;
	end
...
function MarketStall:CostGold_BuyItemOld(nPlayerId, bSuccess, szBillNo, szMainType, nSubType, nCount, nPrice)
	local pPlayer = KPlayer.GetPlayerObjById(nPlayerId);
	if not pPlayer then
		return false, "扣除手续费中途, 您离线了";
	end
	...
	for nSellerId, nSellCount in pairs(tbSellerInfo) do
		...
		local szAwardName = self:GetStallAwardName(szMainType, nSubType, pRoleSeller.nFaction, pRoleSeller.nSex);
		local tbMail = {
			To = nSellerId;
			Title = "摆摊所得";
			From = "系统";
			Text = string.format("出售成功：%s\n单价：%s 元宝\n数量：%s\n收益：%s 元宝", szAwardName, nPrice, nSellCount, nPrice * nSellCount);
	...
	if nCurCount > 0 then
		local tbAward = self:GetSoldStallAward(szMainType, nSubType, nCurCount);
	...
	if nCurCount < nCount then
		if nCurCount > 0 then
			pPlayer.CenterMsg(string.format("现货不足，成功购买 %s 个！", nCurCount), true);
		else
			pPlayer.CenterMsg("现货不足，元宝返还！", true);
		end
...
function MarketStall:CostGold_BuyItem(nPlayerId, bSuccess, szBillNo, szMainType, nSubType, nCount, nPrice)
	...
	local nAvgCount = math.max(1, math.floor(nCount/#tbStallList))
	local bEnough = false
	for i=1, 2 do
		local fnRandSel = Lib:GetRandomSelect(#tbStallList)
		for _ in pairs(tbStallList) do
			local nIdx = fnRandSel()
			local tbInfo = tbStallList[nIdx]
			local nBuyCount = i==1 and nAvgCount or (nCount-nCurCount)
			local nC, nSellerId = self:BuyItemByStallId(pPlayer, tbInfo[1], tbInfo[2], nBuyCount, szBillNo);
			if nC > 0 then
				self:OnBuyItem(pPlayer, nSellerId, nC * nPrice);
...
function MarketStall:BuyItemByStallId(pPlayer, tbStallInfo, nStallId, nCount, szBillNo)
	...
	TLog("SecAuctionCompleteFlow", szGameAppid, nPlat, nServerIdentity, nAreaId,
		...
		pPlayer.GetShouldHaveMoney("Gold"),  nSellerFightPower, nSellerVipLevel, pPlayer.dwKinId, "", 0, 0)
...
function MarketStall:CostGold_BuyFromSystem(nPlayerId, bSuccess, szBillNo, szMainType, nSubType, nCount, nPrice)
	...
	TLog("SecAuctionCompleteFlow", szGameAppid, nPlat, nServerIdentity, nAreaId,
		...
		pPlayer.GetShouldHaveMoney("Gold"), 0, 0, pPlayer.dwKinId, "", 0, 0);
