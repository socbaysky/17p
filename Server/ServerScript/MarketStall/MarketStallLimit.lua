...
if version_vn then
	MarketStall.nMinPricePercent = 6;			-- 当购买物品价格低于此(1/10倍数)的基准价格时，不作为冷门物品
end
...
function MarketStall:LoadLimit()
	...
	local tbLimitInfo = ScriptData:GetValue("MarketStallLimit");
	self.tbForbidden = tbLimitInfo.tbForbidden or {};
	self.bAllForbidden = tbLimitInfo.bAllForbidden;
end

function MarketStall:SetForbidden(tbItemList, bLimit)
	local tbLimitInfo = ScriptData:GetValue("MarketStallLimit");
	tbLimitInfo.tbForbidden = tbLimitInfo.tbForbidden or {};

	local bAll = false;
	if (type(tbItemList) == "number") and tbItemList == 0 then
		tbItemList = {};
		bAll = true;
	end
	...
	if not bLimit then
		if bAll then
			tbLimitInfo.tbForbidden = {};
			tbLimitInfo.bAllForbidden = nil;
		else
			for _, nItemId in pairs(tbItemList) do
				tbLimitInfo.tbForbidden[nItemId] = nil;
			end
		end
	else
		if bAll then
			tbLimitInfo.bAllForbidden = true;
		end
		for _, nItemId in pairs(tbItemList) do
			tbLimitInfo.tbForbidden[nItemId] = true;
		end

		local tbNeedCancelInfo = {};
		for nStallId, tbStallInfo in pairs(self.tbAllStall) do
			if tbLimitInfo.bAllForbidden or 

				(Player.AwardType[tbStallInfo.szMainType] == Player.award_type_item and
				tbStallInfo.nSubType and tbLimitInfo.tbForbidden[tbStallInfo.nSubType]) then

				table.insert(tbNeedCancelInfo, {nStallId, tbStallInfo.nPlayerId, tbStallInfo.bTimeout});
			end
		end

		local nCount = 0;
		local tbList = {};
		for _, tbInfo in pairs(tbNeedCancelInfo) do
			table.insert(tbList, tbInfo);
			if #tbList >= 20 then
				nCount = nCount + 1;
				local tbNewTB = Lib:CopyTB(tbList);
				Timer:Register(nCount + 1, function ()
					for _, tb in pairs(tbNewTB) do
						local nStallId, nPlayerId, bTimeout = unpack(tb);
						MarketStall:CancelSellItem(nPlayerId, nStallId, false, (not bTimeout) and true or false);
					end
				end)
				tbList = {};
			end
	...
	self.tbForbidden = tbLimitInfo.tbForbidden;
	self.bAllForbidden = tbLimitInfo.bAllForbidden;
	ScriptData:SaveValue("MarketStallLimit", tbLimitInfo);
	Log("[MarketStall] SetForbidden", bLimit and "true" or "false");
	Lib:LogTB(tbItemList);
end
...
function MarketStall:OnBuyItem(pPlayer, nSellerId, nGoldCount)
	if not nSellerId or not nGoldCount or nGoldCount <= 0 then
		return
	end
