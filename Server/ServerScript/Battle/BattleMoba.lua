...
function tbBase:StopFight()
	...
	for dwRoleId,tbInfo in pairs(self.tbPlayerInfos) do
		if not tbInfo.bNotJoin then
			...
			local szMsg = string.format(tbAwardMsg[nResult], tbTeamNames[tbInfo.nTeamIndex], tbTeamNames[tbInfo.nTeamIndex])
			Battle:OnGetAwardEvent(dwRoleId, {},  tbAwardSet[nResult] , tbBattleInfo, szMsg, self.nReadyMapIndex)
		end
	end
...
function tbBase:CloseBattle()
	if self.nMapId then
		...
		for dwTeamID, _ in pairs(self.tbTeamIdToSide) do
...
function tbBase:OnBossNpcDeath(him, pKiller)
	...
	self:BattleFieldTips(string.format("%s成功捕获%s，即将加入战场", self.tbBattleSetting.tbTeamNames[nKillSide], him.szName))
end
...
function tbBase:OnBuildNpcDeath(him, pKiller)
	...
	if not next(self.tbAllBuildNpc[nTeam]) then
		self:BattleFieldTips(string.format("%s的大营被摧毁", self.tbBattleSetting.tbTeamNames[nTeam]) );	
	...
	self:BattleFieldTips(string.format("%s的一座防御塔被摧毁", self.tbBattleSetting.tbTeamNames[nTeam]) );	
...
function tbBase:AddBossNpc(nX, nY, nDir)
	...
	self:BlackMsg(string.format("%s现身战场，击杀的一方可获得协助", pNpc.szName))
end
