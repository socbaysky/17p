...
function tbBase:StopFight()
	...
	local szOthrAwardType = Battle:GetOthrAwardType(self.tbBattleSetting.szLogicClass) or ""
	local szOthrAwardName =  Player:GetAwardTypeName(szOthrAwardType) 
	local szMsgFormt =  "本次战场你勇夺第%d名，获得%d点%s" 
	...
	for nRank, tbRankInfo in ipairs(self.tbBattleRank) do
		...
		if not tbInfo.bNotJoin then
			...
			for i,v in ipairs(tbAward) do
				if v[1] == szOthrAwardType then
					nGetHonor = nGetHonor + v[2]
				end
			end

			tbRankInfo.nGetHonor = nGetHonor
			local szMsg = string.format(szMsgFormt, nRank, nGetHonor, szOthrAwardName);
			...
			if szOthrAwardType == "BattleHonor" then
				szMsg = szMsg .. string.format("（%s）", Battle.szBoxExchangeTip)
			end
			Battle:OnGetAwardEvent(dwRoleId, tbAchievenments, tbAward, tbBattleInfo, szMsg, self.nReadyMapIndex)
		end
	end
