...
-- Battle.tbRoleIdToOrgRoleId = Battle.tbRoleIdToOrgRoleId or {};
...
function Battle:OnPlayedBattleEvnet(pPlayer, bCostTicket)
	...
	CallZoneClientScript(pPlayer.nZoneIndex, "Battle:OnZoneOnPlayedBattle", pPlayer.dwOrgPlayerId, bCostTicket)
end

--重载
function Battle:OnGetAwardEvent(dwRoleId, tbAchievenments, tbAward, tbBattleInfo, szMsg, nReadyMapIndex)
	...
	local wServerId, dwOrgPlayerId = KPlayer.GetOrgPlayerIdByZoneId(dwRoleId)

	CallZoneClientScript(nZoneIndex, "Battle:OnGetAwardEvent", dwOrgPlayerId, tbAchievenments, tbAward, tbBattleInfo, szMsg, nReadyMapIndex)
end
