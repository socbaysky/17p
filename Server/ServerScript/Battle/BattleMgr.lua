...
function Battle:CheckPlayerCanInvite(dwRoleId)
	...
	if not self.LegalMap[pPlayer.nMapTemplateId] then
		pPlayer.CenterMsg("您目前无法被传入战场地图，已经退出等待伫列")
		return
	end
...
function Battle:SignUp(pPlayer, bSkipZone)
	...
	if pPlayer.nLevel < tbCurBattleSetting.nMinLevel then
		pPlayer.CenterMsg(string.format("需要%d级才能参加", tbCurBattleSetting.nMinLevel))
		return
	end
...
function Battle:OnAllReadyMapCreateEvent()
	local tbSetting = self.tbCurBattleSetting
    assert(tbSetting)
   	local szBattleName = "宋金大战";
   	if tbSetting.bShowName then
   		szBattleName = tbSetting.szName
   	end
    local szMsg = string.format("%s一触即发，众侠士请火速报名加入！", szBattleName) 
        ...
        for nSettingIndex, _ in pairs(self.tbNowOpenZoneSetting) do
            ...
            if tbBattleSetting.bZone then
                if tbBattleSetting.nShowLevel > nLastLevel and ( not tbBattleSetting.nRealNeedLevel or  GetMaxLevel() >= tbBattleSetting.nRealNeedLevel)  then
                    szMsg = string.format("%s即将开启，众侠士请火速报名参战！", tbBattleSetting.szName) 
                end
            end
    ...
    KPlayer.SendWorldNotify(tbSetting.nMinLevel, 999, szMsg, 1, 1)
...
function Battle:OnGetAwardEvent(dwRoleId, tbAchievenments, tbAwardNeedCopy, tbBattleInfo, szMsg, nReadyMapIndex)
	...
	if pPlayer then
		...
		if tbBattleInfo then
			...
			pPlayer.TLogRoundFlow(Env.LogWay_Battle, tbBattleSetting.nIndex, tbBattleInfo.nScore, tbBattleInfo.nMatchTime
					...
					if tbQUalifySetting and GetTimeFrameState(tbQUalifySetting.OpenTimeFrame) == 1 and tbQUalifySetting.ChechConditionFunc then
						...
						if not bNowCanInNext then
							...
							Mail:SendSystemMail({
								...
								tbAttach 	= { {"AddTimeTitle", tbQUalifySetting.nQualifyTitleId, nNextMothBattleOpenTime, true} };
								nLogReazon 	= Env.LogWay_GetBattleQualify;
								tbParams 	= {LogReason2 = tbQUalifySetting.szQualifyType};
								})

							local szNotifyMsg = string.format("恭喜侠士「%s」参加%s进入前%d名，成功获得武林角逐赛%s资格。", pPlayer.szName, tbBattleSetting.szName, tbBattleSetting.nQualifyMinRank, tbQUalifySetting.szName)
	...
	if tbBattleInfo then
		if tbBattleInfo.nRank == 1 then
			Battle:OnGetBattleFirstEnven(dwRoleId, tbBattleInfo.szLogicClass, nReadyMapIndex)	
		end
		local tbBattleSetting = Battle.tbAllBattleSetting[tbBattleInfo.szLogicClass] 
...
function Battle:OnGetBattleFirstEnven(dwRoleId, szLogicClass, nReadyMapIndex)
	...
	if pRole and tbBattleSetting then
		...
		if dwKinId ~= 0 then
			...
			if tbKin then
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("恭喜本帮派的「%s」获得了%s第一名！", pRole.szName, tbBattleSetting.szName), dwKinId);
		...
		if tbBattleSetting.bZone then
			...
			if tbZoneLevelSetting then
				if tbZoneLevelSetting.nLevelEnd >= GetMaxLevel() then
					KPlayer.SendWorldNotify(1, 999,  string.format("恭喜「%s」勇夺%s第一名！", pRole.szName, tbBattleSetting.szName), 1, 1)
				end
			end
		end

		local tbRedBagType = Kin.tbRedBagBattleTypes[tbBattleSetting.szLogicClass]
		if tbRedBagType then
			for _, nRedBagType in ipairs(tbRedBagType) do
				Kin:RedBagOnEvent(dwRoleId, nRedBagType, 1)
			end
		end
	end
end
