...
function CardPicker:SelectRandomItems(tbItems, szPickerType)
	...
	assert(false, szPickerType .. "数量:" .. #tbItems .. "机率:" .. nMyProb .. "/" .. nAllProbVip);
end
...
function CardPicker:GoldTenPick(nPrice)
	...
	me.CostGold(nRealCost, Env.LogWay_GoldTenPick, nil, function (nPlayerId, bSuccess, szBillNo)
		...
		if nPrice ~= nRealCost then
			return false, "抽卡价格有变, 还请侠士刷新介面後再次尝试";
		end
	...
	for nIdx = CardPicker.Def.SAVE_GOLD_PICK_CACHE_BEGIN, CardPicker.Def.SAVE_GOLD_PICK_CACHE_END, 2 do
		...
		if nTyp ~= 0 and nItemId ~= 0 then
			table.insert(tbGoldPickCache, {
				szItemType = Player.AwardType2Name[nTyp][1];
				nItemId    = nItemId;
				nCount     = 1;
			});
		end
...
function CardPicker:SendSpecialPartnerNewInformation()
	local szTitle = "绝世名侠现江湖";
	local tbContent = {"    如今江湖风起云涌，诸多握有一技之长的侠士、武者、名姬现身江湖，据惜花老板娘的情报，他们会在以下时日出现于隐香楼，还望诸位侠士把握时机，前去进行[00FF00][url=openwnd:元宝招募, Partner, \"CardPickingPanel\"][-]，将之纳入麾下。\n    [FFFE0D]注：以下特定日期进行[url=openwnd:元宝招募, Partner, \"CardPickingPanel\"]将有概率获得特定SS级同伴[-]"};
	local szPartnerFormate = "\n    [FFFE0D]%s[-]    SS级同伴[FFFE0D][url=openwnd:%s, PartnerDetail, nil, nil, nil, %d][-] "

	if version_tx then
		tbContent = {"    如今江湖风起云涌，诸多握有一技之长的侠士、武者、名姬现身江湖，据惜花老板娘的情报，他们会在以下时日出现于隐香楼，还望诸位侠士把握时机，前去进行[00FF00][url=openwnd:元宝招募, Partner, \"CardPickingPanel\"][-]，将之纳入麾下。\n    [FFFE0D]注：以下特定日期进行[url=openwnd:元宝招募, Partner, \"CardPickingPanel\"]将有机率获得特定SS级同伴[-]"};
		szPartnerFormate = "\n    [FFFE0D]%s[-]    SS级同伴[FFFE0D][url=openwnd:%s, PartnerDetail, nil, nil, nil, %d][-] "	
	end
...
function CardPicker:ClientRequest(szRequestType, ...)
	if not CardPicker:CheckCardPickCount(me.dwID, szRequestType) then
		me.CallClientScript("CardPicker:OnCardPickFail", "今日已达招募次数上限，请少侠明日再来！");
		return;
	end
