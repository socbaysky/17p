...
function SwornFriends:_GetRelativeName(pMe, nPid)
	...
	if nPid~=pMe.dwID then
		...
		szName = pStay and pStay.szName or "队友"
	end
	return szName
end
...
function SwornFriends:CheckBeforeConnect(pPlayer)
	...
	if nMemberCount<self.Def.nMinPlayer or nMemberCount>self.Def.nMaxPlayer then
		return false, string.format("结拜需%d~%d人", self.Def.nMinPlayer, self.Def.nMaxPlayer)
	end

	for _, nPid in ipairs(tbMembers) do
		if self:_GetConnectInfo(nPid) then
			return false, string.format("%s已有结拜关系，不能再次结拜", self:_GetRelativeName(pPlayer, nPid))
		end
	end
	...
	if nItemCount<=0 then
		local pStay = KPlayer.GetRoleStayInfo(nPid)
		return false, string.format("%s当前没有金兰谱", self:_GetRelativeName(pPlayer, nPid))
	end
	
	local nMinLevel, nPid = self:_GetPlayersMinLevel(tbMembers)
	if nMinLevel<self.Def.nMinLevel then
		return false, string.format("%s的等级不足%d级", self:_GetRelativeName(pPlayer, nPid), self.Def.nMinLevel)
	end
	...
	if nMinImitity<self.Def.nMinImitity then
		...
		return false, (pStay1 and pStay2) and 
			string.format("%s与%s的亲密度不足%d级", pStay1.szName, pStay2.szName, self.Def.nMinImitity) or
			string.format("亲密度不足%d级", self.Def.nMinImitity)
	end
...
function SwornFriends:_DoConnect(tbPids, szMainTitle)
	...
	for _, nPid in ipairs(tbPids) do
		local pPlayer = KPlayer.GetPlayerObjById(nPid)
		FriendShip:AddViewRelationModifyFlag(nPid)
		if pPlayer then
...
function SwornFriends:_GenMainTitle(szHead, szTail, nMemberCount)
	...
	if nHeadLen>self.Def.nTitleHeadMax or nHeadLen<self.Def.nTitleHeadMin or nTailLen>self.Def.nTitleTailMax or nTailLen<self.Def.nTitleTailMin then
		return false, string.format("首码%d-%d字元，尾码%d~%d字元", self.Def.nTitleHeadMin, self.Def.nTitleHeadMax, self.Def.nTitleTailMin, self.Def.nTitleTailMax)
	end

	if not CheckNameAvailable(szHead) or not CheckNameAvailable(szTail) then
		return false, "称号中含有敏感词"
	end
...
function SwornFriends:DisconnectReq()
	...
	for nPid in pairs(tbInfo[2] or {}) do
		...
		if pPlayer then
			self:_PushConnectIds(pPlayer)
			pPlayer.Msg(string.format("%s已与你结束结拜关系", me.szName))
		end
...
function SwornFriends:ChangePersonalTitle(szTitle, bConsumeItem)
	...
	if nLen<=0 then
		return false, "请填写结拜称号"
	end
	if nLen>self.Def.nPersonalTitleMax or nLen<self.Def.nPersonalTitleMin then
		return false, string.format("个人称号%d-%d字元", self.Def.nPersonalTitleMin, self.Def.nPersonalTitleMax)
	end
	...
	for nPid, sz in pairs(tbInfo[2]) do
		if sz==szTitle then
			return false, nPid==nMyId and "你已经是这个称号了" or "这个称号已被其他结拜成员使用"
		end
	end
...
function SwornFriends:_FinishConnectionAction(tbIds)
	...
	for _, nId in ipairs(tbIds) do
		...
		if pPlayer then
			...
			pPlayer.CallClientScript("SwornFriends:OnFinishConnection")
		end
		pPlayer = pPlayer or KPlayer.GetRoleStayInfo(nId)
		if pPlayer then
			table.insert(tbNames, string.format("[FFFE0D]「%s」[-]", pPlayer.szName))
		end
	end

	local tbInfo = self:_GetConnectInfo(tbIds[1])
	local szTitle = tbInfo and tbInfo[1] or ""
	local szNames = table.concat(tbNames, "、")
	local szMsg = string.format("%s志同道合，于襄阳城望月台拜天地，结义为「%s」", szNames, szTitle)
