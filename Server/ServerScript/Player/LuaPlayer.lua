		...
		for _, tbInfo in pairs(tbAward) do
			if (tbInfo.nWorldNotify and tbInfo.nWorldNotify > 0) or (tbInfo.nKinNotify and tbInfo.nKinNotify > 0) or (tbInfo.nTeamNotify and tbInfo.nTeamNotify > 0) then
				...
				if Player.AwardType[tbInfo[1]] == Player.award_type_item then
					local szName = KItem.GetItemShowInfo(tbInfo[2], self.nFaction, self.nSex);
				...
				if tbInfo.nWorldNotify and tbInfo.nWorldNotify > 0 then
					...
					if nItemTemplateId then
						...
						ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.System, szMsg, nil, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemTemplateId, nFaction = self.nFaction, nSex = self.nSex});
					else
				...
				if self.dwKinId > 0 and tbInfo.nKinNotify and tbInfo.nKinNotify > 0 then
					...
					if nItemTemplateId then
						ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, self.dwKinId, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemTemplateId, nFaction = self.nFaction, nSex = self.nSex});
					else
				...
				if self.dwTeamID > 0 and tbInfo.nTeamNotify and tbInfo.nTeamNotify > 0 then
					...
					if nItemTemplateId then
						ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, szMsg, self.dwTeamID, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemTemplateId, nFaction = self.nFaction, nSex = self.nSex})
					else
...
function _LuaPlayer.AddTimeTitle(nTitleID, nEndTime, bActive, bShowInfo)
    ...
    if nTimeout <= 0 and nEndTime ~= 0 then
    	...
    	if tbTitleTemp then
    		self.CenterMsg(string.format("%s称号已经过期了！", tbTitleTemp.Name));
    	end
...
function _LuaPlayer.DoChangeActionMode(nActMode, bBroadcast)
	self.ChangeActionMode(nActMode, bBroadcast and true or false);
		...
		AssistClient:DoPresent(self, nPoint, nLogReazon, nLogReazon2, function (nPlayerId, bSucceed)
			if not bSucceed then
				local tbMail = {
					...
					Title = "元宝赠送";
					From = "支付系统";
					Text = "很抱歉，在获取系统赠送的元宝时, 系统发生异常。 故将元宝以信件附件形式发送给您, 请手动领取。";
...
function _LuaPlayer.PushToPrison(nExpireTime)
	self.SetUserValue(Player.PRISON_EXPIRE_SAVE_GROUP, Player.PRISON_EXPIRE_SAVE_KEY, nExpireTime);
	return true;
end
