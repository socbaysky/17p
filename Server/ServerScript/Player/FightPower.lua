...
function FightPower:OnLogin(pPlayer)
	...
	if not tbFightPowerData.nJingMaiFightPower then
		...
		tbFightPowerData.nJingMaiFightPower = 0;
		self:ChangeFightPower("JingMai", pPlayer, true);
	end

	if not tbFightPowerData.nZhenFaFightPower then
		tbFightPowerData.nZhenFaFightPower = 0;
		self:ChangeFightPower("ZhenFa", pPlayer, true);
	end

	if not tbFightPowerData.nMagicBowlFightPower then
		tbFightPowerData.nMagicBowlFightPower = 0
		self:ChangeFightPower("MagicBowl", pPlayer, true)
	end
	...
	pPlayer.CallClientScript("Ui.Stronger:SyncFightPower", pPlayer.GetFightPower(),
		{
			...
			tbFightPowerData.nZhenFaFightPower,
		});
...
function FightPower:GetTotalFightPower(pPlayer)
	...
	nTotal = nTotal + (tbFightPowerData.nZhenFaFightPower or 0);
	nTotal = nTotal + (tbFightPowerData.nMagicBowlFightPower or 0);
	return nTotal;
end
...
function FightPower:ChangeFightPower(szType, pPlayer, bSlient)
	...
	if MODULE_GAMESERVER and szType == "JingMai" then
		RankBoard:UpdateRankVal("JingMai", pPlayer.dwID, nCurFightPower);
	end
...
function FightPower:GetHonorFPSetting(pPlayer )
	return Player.tbHonorLevelSetting
end
...
function FightPower:CalcHonorFightPower(pPlayer)
	...
	local tbHonorFPSetting = self:GetHonorFPSetting(pPlayer)
	if nHonorLevel > 0 then
		for nI = 1, nHonorLevel do
			local tbHonor = tbHonorFPSetting[nI];
...
function FightPower:CalcZhenFaFightPower(pPlayer)
	return ZhenFa:GetFightPower(pPlayer)
end
...
function FightPower:CalcMagicBowlFightPower(pPlayer)
	return Furniture.MagicBowl:GetFightPower(pPlayer.dwID)
end
