...
function PlayerTitle:CheckAddTile(pPlayer, nTitleID, nTimeout)
    ...
    for _, tbTitleData in pairs(tbData.tbAllTitle) do
        ...
        if tbTitleData.nTitleID==nTitleID then
            if not (tbTitleData.nEndTime ~= - 1 and nTimeout and nTimeout ~= -1)  then
                pPlayer.CenterMsg("你已经拥有该称号，请过期後再使用")
                return false    
            end
            
        end
        local tbCurTitleTemp = self:GetTitleTemplate(tbTitleData.nTitleID);
        if tbCurTitleTemp and tbCurTitleTemp.Rank > 0 and tbTitleTemp.Type == tbCurTitleTemp.Type then
...
function PlayerTitle:AddTitle(pPlayer, nTitleID, nTimeout, szText, bNoTip)
    local bRet, nReplaceTitleID = self:CheckAddTile(pPlayer, nTitleID, nTimeout);
    if not bRet then
        return false;
    end    
    ...
    if not bNoTip then
        pPlayer.CenterMsg(string.format("您获得了「%s」称号", tbTitleTemp.Name));
    end
...
function PlayerTitle:ActiveTitle(pPlayer, nTitleID, bShowInfo)
    local tbTitleTemp = self:GetTitleTemplate(nTitleID);
    if nTitleID ~= 0 and not tbTitleTemp then
        PlayerTitle:DeleteTitle(pPlayer, nTitleID, true)
        return
    end
    ...
    if bShowInfo then
        if nTitleID > 0 then
            local tbTitleTemp = self:GetTitleTemplate(nTitleID);
            pPlayer.CenterMsg("更换称号成功");
        else
            pPlayer.CenterMsg("隐藏称号成功");    
        end
    end        
...
function PlayerTitle:DeleteTitle(pPlayer, nTitleID, bReplace)
    ...
    if not bReplace then
        pPlayer.CenterMsg(string.format("您的称号「%s」已过期", tbTitleTemp.Name), true);
    end       
