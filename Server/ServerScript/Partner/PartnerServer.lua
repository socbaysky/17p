...
function Partner:BatchUseProtentialItem(pPlayer, nPartnerId, nProtentialType, nUseCount)
	...
	if not szShowMsg then
		szShowMsg = string.format("消耗了[FFFE0D]%s[-]个资质丹，[FFFE0D]%s[-]增加了[FFFE0D]%s[-]", nCurUseCount, self.tbProtentialName[nProtentialType], nAllAddValue);
	end
...
function Partner:ReInitPartner(pPlayer, nPartnerId)
	...
	pPlayer.CenterMsg(string.format("洗髓成功，消耗了%s个洗髓丹", nCost));
...
function Partner:DecomposePartner(pPlayer, tbPartnerList)
	...
	if bHasLimitPartner and Lib:CountTB(tbAward) <= 0 then
		pPlayer.CenterMsg("初始明星同伴遣散後没有奖励！");
	end
...
function Partner:UseSkillBook(pPlayer, nPartnerId, nItemId, nPos)
	...
	pPlayer.CenterMsg(string.format("「%s」学会了%s", pPartner.szName, szSkillName));
...
function Partner:DoUseExpItem(pPlayer, nPartnerId, nItemTemplateId, nUseCount)
	...
	if not pPartner or pPartner.nLevel >= pPlayer.nLevel then
		pPlayer.CenterMsg("目前同伴等级已达上限，无法使用！");
		return;
	end

	if pPartner.nLevel >= self.MAX_LEVEL then
		pPlayer.CenterMsg("目前同伴已满级");
		return;
	end
...
function Partner:DoAwareness(pPlayer, nPartnerId, tbUsePartnerInfo)
	...
	if not bRet then
		pPlayer.CenterMsg(szMsg or "无法觉醒");
		return;
	end
	...
	pPlayer.SendBlackBoardMsg("觉醒成功，潜能得到了全面提升！");
	...
	if nQualityLevel <= 2 then
		KPlayer.SendWorldNotify(0, 1000, string.format("恭喜「%s」的%s级同伴%s觉醒成功，潜能得到了全面提升！", pPlayer.szName, Partner.tbQualityLevelDes[nQualityLevel], szName), 1, 1);
	end

	if nQualityLevel <= 3 then
		if pPlayer.dwKinId > 0 then
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("恭喜帮派成员「%s」的%s级同伴%s觉醒成功，潜能得到了全面提升！", pPlayer.szName, Partner.tbQualityLevelDes[nQualityLevel], szName), pPlayer.dwKinId);
		end
	end
...
function Partner:UseWeapon(pPlayer, nPartnerId)
	...
	if tbPartner.nWeaponState ~= 0 then
		pPlayer.CenterMsg(string.format("【%s】已经拥有此武器", tbPartner.szName));
		return;
	end
	...
	pPlayer.MsgBox(string.format("确定装备 [FFFE0D]%s[-] 本命武器 [FFFE0D]%s[-] ？\n[FFFE0D]（装备後不可取下，遣散後将会返还）[-]", tbPartner.szName, szItemName),
		{
			{"确定", tbPartnerWeapon.UseWeapon, tbPartnerWeapon, pPlayer.dwID, nPartnerId, pItem.dwId},
			{"取消"}
		});
end
...
function Partner:OnAddPartner(nPartnerId, nQualityLevel, nLogReazon)
	...
	if nQualityLevel <= 3 then
		...
		if szPartnerName then
			local szWorldNotifyMsg = string.format("恭喜「%s」获得%s级同伴%s", me.szName, Partner.tbQualityLevelDes[nQualityLevel], szPartnerName);
...
function Partner:CallPartner(pPlayer, nPartnerTemplateId)
	...
	if nHasPartner == 0 then
		pPlayer.CenterMsg("必须拥有过此同伴才可以招募");
		return;
	end
	...
	if nCost ~= nNeedCount then
		pPlayer.CenterMsg("扣除道具失败，请联系客服人员！");
