...
function Achievement:AddCount(player, szSubKind, nAddCount)
    ...
    if type(player) == "number" then
        ...
        if not pPlayer then
            local pStayInfo = KPlayer.GetRoleStayInfo(player)
            if pStayInfo then
...
function Achievement:OnCountChange(pPlayer, szSubKind, nCurCount, nNewCount)
    ...
    for _, tbCount in ipairs(tbCountList) do
        ...
        if nCurCount < nCount and nNewCount >= nCount then
            ...
            table.insert(tbCompleteLv, nCompletedLevel)
    ...
    if next(tbCompleteLv) then
        pPlayer.CallClientScript("Achievement:OnCompleteLv", szMainKind, tbCompleteLv)
    end
end
