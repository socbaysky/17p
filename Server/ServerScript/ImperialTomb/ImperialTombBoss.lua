...
function ImperialTomb:OnBossTimeOut(nNpcId, nMapId)
	...
	if tbDamage then
		...
		for i,v in ipairs(tbSortedList) do
			TLog("KinActFlow", szGameAppid, nPlat, nServerIdentity, Env.LogWay_ImperialTomb_Emperor_Attend, v.nId, nNpcTemplate, math.floor(v.nPercent * 100));
		end
	end
...
function ImperialTomb:OnBossDeath(pNpc, pKiller)
	...
	if tbDamage then
		...
		local szGameAppid, nPlat, nServerIdentity = GetWorldConfifParam()
		for i,v in ipairs(tbSortedList) do
			TLog("KinActFlow", szGameAppid, nPlat, nServerIdentity, Env.LogWay_ImperialTomb_Emperor_Attend, v.nId, pNpc.nTemplateId, math.floor(v.nPercent * 100));
		end
	end
	...
	if szKinName and szPlayerName then
		szMsg = string.format(XT("恭喜「%s」帮派的「%s」给予%s最後一击，使其得以安息", szKinName, szPlayerName, szNpcName))
	elseif szPlayerName then
		szMsg = string.format(XT("恭喜「%s」给予%s最後一击，使其得以安息", szPlayerName, szNpcName))
	end
	...
	for nMapType,tbMapInfo in pairs(self.tbNormalMapList) do
		...
		if tbTrapList then
			for _,tbPos in pairs(tbTrapList) do
				self:_AddBossTrapNpc(tbMapInfo.nMapTemplateId, tbMapInfo.nMapId, tbPos[1], tbPos[2], nTrapId, nTrapIndex, XT("通往护主殿"))
				nTrapId = nTrapId + 1;
