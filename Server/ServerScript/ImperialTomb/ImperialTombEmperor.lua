...
function ImperialTomb:CloseEmperor()
	...
	for nNpcId,tbNpcInfo in pairs(self.tbEmperorList) do
		...
		if tbNpcInfo.nMirrorWarnningTimer and Timer:GetRestTime(tbNpcInfo.nMirrorWarnningTimer) >= 0 then
			Timer:Close(tbNpcInfo.nMirrorWarnningTimer)
		end
		tbNpcInfo.nMirrorWarnningTimer = nil
	end
...
function ImperialTomb:OnWarnningEmperorMirrorTrans(nNpcId)
	local tbNpcInfo = self.tbEmperorList[nNpcId];
	if not tbNpcInfo then
		Log("[Error]", "ImperialTomb", "OnWarnningEmperorMirrorTrans Failed Not Find Npc Info", nNpcId)
		return true
	end

	if tbNpcInfo.nMirrorWarnningTimer then
		Log("[Error]", "ImperialTomb", "OnWarnningEmperorMirrorTrans Failed Already Start nMirrorWarnningTimer", nNpcId, tbNpcInfo.nMirrorWarnningTimer)
		return true
	end
	...
	tbNpcInfo.nMirrorWarnningTimer = Timer:Register(Env.GAME_FPS * 3, self.OnEmperorMirrorTrans, self, nNpcId);
...
function ImperialTomb:OnEmperorDeath(pNpc, pKiller)
	...
	if tbDamage then
		...
		if tbMirrorDmgList then
			for _,tbMirrorDmg in pairs(tbMirrorDmgList) do
		...
		local tbSortedList = self:GetSortedDmgInfo(tbDamageList, {tbNpcInfo},self.FIRST_LAST_DMG_BONUS, self.DMG_PERCENT_FACTOR);
		for i,v in ipairs(tbSortedList) do
			TLog("KinActFlow", szGameAppid, nPlat, nServerIdentity, Env.LogWay_ImperialTomb_Emperor_Attend, v.nId, nTemplateId, math.floor(v.nPercent * 100));
		end
	...
	if szKinName and szPlayerName then
		szMsg = string.format(XT("恭喜「%s」帮派的「%s」给予%s最後一击，使其得以安息", szKinName, szPlayerName, szTargetName))
	elseif szPlayerName then
		szMsg = string.format(XT("恭喜「%s」给予%s最後一击，使其得以安息", szPlayerName, szTargetName))
	end
...
function ImperialTomb:OnEmperorTimeOut(nMapId)
	...
	if tbDamage then
		...
		local szGameAppid, nPlat, nServerIdentity = GetWorldConfifParam()
		local nTemplateId = pNpc.nTemplateId
		local tbSortedList = self:GetSortedDmgInfo(tbDamage, {tbNpcInfo},self.FIRST_LAST_DMG_BONUS, self.DMG_PERCENT_FACTOR);
		for i,v in ipairs(tbSortedList) do
			TLog("KinActFlow", szGameAppid, nPlat, nServerIdentity, Env.LogWay_ImperialTomb_Emperor_Attend, v.nId, nTemplateId, math.floor(v.nPercent * 100));
		end
...
function ImperialTomb:EmperorAward()
	...
	for _,tbMirrorDmgList in pairs(self.tbEmperorMirrorDmg) do
		for _,tbMirrorDmg in pairs(tbMirrorDmgList) do
			Lib:MergeTable(tbDamageList, tbMirrorDmg);
		end
	end
	...
	--输出第一家族参与成就
	local tbFirstKinDmgInfo = tbSortedList[1];
	if tbFirstKinDmgInfo then
		local nKinId  = tbFirstKinDmgInfo.nId;
		local tbPlayerList = self.tbEmperorJoinList[nKinId];
		if tbPlayerList then
			for nPlayerId, _ in pairs( tbPlayerList ) do
				Achievement:AddCount(nPlayerId, "ImperialTombEmperor_2", 1);
			end
		end
	end
