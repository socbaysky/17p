...
function ImperialTomb:Open()
	...
	if nOpenServerDay == nToday then
		...
		{
			Title = '宝珠赠侠士',
			Text = [[    秦始皇陵重现于世，古秦士卒四起伤人，各位於皇陵出现时等级已达60级，特赠[FFFE0D]「夜明珠」[-]一颗，此物有避毒功效，始皇降世时，唯有携此物方可前往皇陵。还望诸位能够除此祸害，还江湖平静。若仍有不解之处，可通过下方连结前往查看説明。
                                     查看活动：[47f005][url=openwnd:秦始皇陵, CalendarPanel, 1, 37][-]
                                     查看活动：[47f005][url=openwnd:始皇降世, CalendarPanel, 1, 38][-] ]],
			From = '独孤剑',
...
function ImperialTomb:OnEnterMap(nMapTemplateId, nMapId, nType, nParam)
	...
	if self.bOpenEmperor or self.bOpenFemaleEmperor then
...
function ImperialTomb:EnterTomb()
	...
	if bEmperor and not self.tbEmperorTikectList[me.dwID] then
		...
		Achievement:AddCount(me, "ImperialTombEmperor_1", 1)
	end
...
function ImperialTomb:CheckStayTime(pPlayer)
	...
	if nLeftTime and nLeftTime <= 0 then
		pPlayer.Msg(XT("皇陵毒雾弥漫，侠士已无法承受其中剧毒，唯有将侠士送出皇陵"))
		pPlayer.Revive(1);
		pPlayer.GotoEntryPoint();
	end
end
...
function ImperialTomb:OnPlayerDeath(pKiller)
	if self:IsEmperorMirrorMapByTemplate(me.nMapTemplateId) or self:IsFemaleEmperorMirrorMapByTemplate(me.nMapTemplateId) then
		...
		me.Msg(string.format(XT("%d秒後复活", nTime)))
	end
end
...
function ImperialTomb:OnCreateChatRoom(dwKinId, uRoomHighId, uRoomLowId)
	if not self.bOpenEmperor and not self.bOpenFemaleEmperor then
		return false
	end
