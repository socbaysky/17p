...
function tbTeamBattleMgr:AssignTeam()
	...
	for _, pPlayer in pairs(tbAllMapPlayer) do
		local nPlayerId = TeamBattle:GetUniqIdByPlayer(pPlayer)
		tbAllPlayer[nPlayerId] = pPlayer;
		if not self.tbPlayerTeamId[nPlayerId] then
	...
	Lib:CallBack({TeamBattle.LogTeamInfo, TeamBattle, self.tbAllTeamInfo})
...
function tbTeamBattleMgr:CostAllPlayerTimes()
	...
	for _, tbMember in pairs(self.tbAllTeamInfo) do
		for _, nPlayerId in ipairs(tbMember) do
			-- 这里nPlayerId不用判断，因为这里的逻辑肯定是本服通天塔
...
function tbTeamBattleMgr:KickoutSpacePlayer()
	...
	for _, pPlayer in pairs(tbAllMapPlayer) do
		...
		if not self.tbPlayerTeamId[nUniqId] then
			pPlayer.CenterMsg("由於未能给你匹配到队友，无法参加活动失败！");

			if MODULE_ZONESERVER then
				local nConnectIdx = Server:GetConnectIdx(pPlayer.nZoneServerId);
				if nConnectIdx then
					CallZoneClientScript(nConnectIdx, "TeamBattle:SendSpaceAward", pPlayer.dwOrgPlayerId, self.nType);
				end
			end
...
function tbTeamBattleMgr:AssignFloor()
	...
	for nTeamId, tbMember in pairs(self.tbAllTeamInfo) do
		...
		for _, nPlayerId in ipairs(tbMember) do
			local pMember = TeamBattle:GetPlayerById(nPlayerId);
			if pMember then
		...
		if #tbMember >= 2 then
			...
			for _, nPlayerId in ipairs(tbMember) do
				...
				if pPlayer and pPlayer.dwTeamID > 0 then
					...
					local _, dwID = TeamBattle:GetPlayerById(nPlayerId)
					if dwID then
						table.insert(tbOther, dwID);
					else
						Log("[TeamBattle] MakeTeam ERR ?? No Find PlayerId !", nPlayerId);
					end
				end
			end
...
function tbTeamBattleMgr:StartFail()
	...
	for _, pPlayer in pairs(tbAllMapPlayer) do
		if MODULE_ZONESERVER then
			local nConnectIdx = Server:GetConnectIdx(pPlayer.nZoneServerId);
			if nConnectIdx then
				CallZoneClientScript(nConnectIdx, "TeamBattle:SendStartFailAward", pPlayer.dwOrgPlayerId, self.nType);
			end
		end
		pPlayer.CenterMsg("由於报名人数不足，活动未开启！", true);
		TeamBattle:GoBack(pPlayer);
	end
...
function tbTeamBattleMgr:DoStartFight()
	...
	Lib:CallBack({Lib.LogTB, Lib, self.tbAllFloorInfo})
	-- 普通跨服通天塔的最后一层
...
function tbTeamBattleMgr:SendPlayerAward(pPlayer, nFloor)
	...
	local szMsg = "";
	if nAwardItemCount > 0 then
		szMsg = string.format("恭喜你在本次通天塔中登上第%s层，获得%s点荣誉。\n自动兑换%s个通天塔宝箱。剩余%s点荣誉\n（每满%s点荣誉，自动兑换1个通天塔宝箱）",
	...
	if GetTimeFrameState(TeamBattle.szLeagueOpenTimeFrame) == 1 and nFloor >= 7 then
		...
		if nCurInfo ~= nNextOpenMonth - 1 then
			...
			local tbMonthlyMail = {
				To = pPlayer.dwID,
				Title = "月度通天塔资格通知",
				Text = string.format("      恭喜您在本场通天塔竞技中成绩优异，登上[FFFE0D]%s[-]层，现已获得月度通天塔参赛资格。月度通天塔开启时间为[FFFE0D]%s[-]，期待您的参与！更高的荣誉等着您！", nFloor, szTimeInfo),
				From = "通天塔",
			...
			local szFriendMsg = string.format("恭喜侠士「%s」在通天塔竞技中英勇无敌，一口气登上%s层，成功获得月度通天塔参赛资格！#49", pPlayer.szName, nFloor);
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Friend, szFriendMsg, pPlayer.dwID);
			if pPlayer.dwKinId > 0 then
				local szKinMsg = string.format("恭喜本帮派成员「%s」在通天塔竞技中英勇无敌，一口气登上%s层，现已获得月度通天塔参赛资格！#49", pPlayer.szName, nFloor);
...
function tbTeamBattleMgr:SendTeamAward(nTeamId, nOtherTeamId)
	if MODULE_ZONESERVER then
			...
			for nIdx, nCurTeamId in pairs(tbFloorInfo) do
				if self.tbAllTeamInfo[nCurTeamId] and not self.tbAllTeamInfo[nCurTeamId].bAward and (nCurTeamId == nTeamId or nCurTeamId == nOtherTeamId) then
					...
					local function fnGetServerInfo(self, pPlayer, tbMember, tbInfo)
						tbInfo[pPlayer.nZoneServerId] = tbInfo[pPlayer.nZoneServerId] or {};
						tbInfo[pPlayer.nZoneServerId][nFloor] = tbInfo[pPlayer.nZoneServerId][nFloor] or {};
						table.insert(tbInfo[pPlayer.nZoneServerId][nFloor], pPlayer.dwOrgPlayerId);
		...
		for nServerId, tbPlayerInfo in pairs(tbAllServerInfo) do
			...
			if nConnectIdx then
				...
				Log("[TeamBattle] GetConnectIdx Fail !!", nServerId);
...
function tbTeamBattleMgr:OnLoginPreMap()
...
function tbTeamBattleMgr:OnEnterPreMap()
	...
	local nMyUniqId = TeamBattle:GetUniqIdByPlayer(me)
	if not self.tbPlayerTeamId[nMyUniqId] then
		TeamMgr:QuiteTeam(me.dwTeamID, me.dwID);
	end

	if MODULE_ZONESERVER then
		local bCountErr = false;
		local nCacheTeamId = self.tbPlayerTeamId[nMyUniqId];
		if nCacheTeamId and self.tbAllTeamInfo[nCacheTeamId] then
			...
			for _, nPlayerId in ipairs(tbMembers) do
				if nPlayerId ~= nMyUniqId then
					local pOther = TeamBattle:GetPlayerById(nPlayerId);
					if pOther then
						if pOther.dwTeamID and pOther.dwTeamID > 0 then
							...
							local _, dwID = TeamBattle:GetPlayerById(nPlayerId)
							local bResult = TeamMgr:Create(me.dwID, dwID or 0, true);
							if not bResult then
								Log("[TeamBattle] Create Team Fail !!", me.dwID, nPlayerId, dwID);
							end
						end
						break;
					end
		...
		if bCountErr then
			self.tbPlayerTeamId[nMyUniqId] = nil;
			self:UpdatePlayerTeamInfo(nMyUniqId);
		end
	end
