TeamBattle.tbUniqId2ZoneId = TeamBattle.tbUniqId2ZoneId or {}
...
function TeamBattle:TryJoinPreMap(pPlayer, bIsSignle)
	...
	if not bRet then
		if bIsSignle or nCode == self.emMsgNotTeamCaptain then
			...
			if pPlayer.dwTeamID and pPlayer.dwTeamID > 0 then
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format(TeamBattle.tbMsg[nCode], unpack(tbParam or {})), pPlayer.dwTeamID);
			end
		end
		return;
	end
	local nServerId = GetServerIdentity();
	local nPlayerUniqId = self:CombineUniqId(nServerId, pPlayer.dwID)
	if bIsSignle then
		if nType == self.TYPE_NORMAL then
			self.tbCurBattleMgr:UpdatePlayerTeamInfo(pPlayer.dwID);
		else
			CallZoneServerScript("TeamBattle.tbCurBattleMgr:UpdatePlayerTeamInfo", nPlayerUniqId);
		end
		...
		if nType == self.TYPE_NORMAL then
			...
			CallZoneServerScript("TeamBattle.tbCurBattleMgr:NotifyTeamEnter", nPlayerUniqId);
			local tbMember = TeamMgr:GetMembers(pPlayer.dwTeamID);
			for _, nPlayerId in pairs(tbMember) do
				local pMember = KPlayer.GetPlayerObjById(nPlayerId);
				local nMemberUniqId = self:CombineUniqId(nServerId, nPlayerId)
				CallZoneServerScript("TeamBattle.tbCurBattleMgr:UpdateLeaguePlayerTeamInfo", nMemberUniqId, nPlayerUniqId);
				self:SwitchToPreMap(pMember, nType);
			end
		end
	end
...
function TeamBattle:CombineUniqId(nServerId, dwID)
	return Lib:CombineUniqId(nServerId, dwID)
end

function TeamBattle:RestoreUniqId(nUniqId)
	return Lib:RestoreUniqId(nUniqId)
end

function TeamBattle:GetPlayerById(dwID)
	local nPlayerId = dwID
	if MODULE_ZONESERVER then
		nPlayerId = self.tbUniqId2ZoneId[dwID]
	end
	-- if not nPlayerId then
	-- 	Log("TeamBattle fnGetPlayerById no player id", dwID)
	-- end
	return KPlayer.GetPlayerObjById(nPlayerId or -1), nPlayerId
end

function TeamBattle:GetUniqIdByPlayer(pPlayer)
	return MODULE_ZONESERVER and TeamBattle:CombineUniqId(pPlayer.nZoneServerId, pPlayer.dwOrgPlayerId) or pPlayer.dwID
end

function TeamBattle:CachePlayerId(pPlayer)
	if MODULE_ZONESERVER then
		local nUniqId = QunYingHuiCross:CombineUniqId(pPlayer.nZoneServerId, pPlayer.dwOrgPlayerId)
		self.tbUniqId2ZoneId[nUniqId] = pPlayer.dwID
	end
end

function TeamBattle:LogTeamInfo(tbAllTeamInfo)
	for nTeamId, tbMember in pairs(tbAllTeamInfo) do
		Log("nTeamId >", nTeamId)
		for _, nUniqId in ipairs(tbMember) do
			if MODULE_ZONESERVER then
				local dwID, nServerId = TeamBattle:RestoreUniqId(nUniqId)
				Log(dwID, nServerId)
			else
				Log(nUniqId)
			end
