...
function tbFightMap:UpdatePlayerDmg(pPlayer)
	local nTeamId = self:GetTeamId(pPlayer);
	if not nTeamId then
		return;
	end
	local nUniqId = TeamBattle:GetUniqIdByPlayer(pPlayer)
	self.tbDmgInfo[nTeamId].tbPlayerDmg = self.tbDmgInfo[nTeamId].tbPlayerDmg or {};
	self.tbDmgInfo[nTeamId].tbPlayerDmg[nUniqId] = self.tbDmgInfo[nTeamId].tbPlayerDmg[nUniqId] or {nDamage = 0};

	local tbLastCounter = self.tbDmgInfo[nTeamId].tbPlayerDmg[nUniqId];
	local tbCounter = pPlayer.GetNpc().GetDamageCounter();
	self.tbDmgInfo[nTeamId].tbPlayerDmg[nUniqId] = tbCounter;
...
function tbFightMap:SyncPlayerLeftInfo(pPlayer)
	local tbDmgInfo = {
		szStateInfo = self.szStateInfo or "等待开始",
	...
	pPlayer.CallClientScript("TeamBattle:SyncPlayerLeftInfo", self:GetTeamId(pPlayer), tbDmgInfo);
end
...
function tbFightMap:GetTeamId(pPlayer)
	local nUniqId = TeamBattle:GetUniqIdByPlayer(pPlayer)
	return self.tbCurTeamId[nUniqId]
end
...
function tbFightMap:ShowBeyInfo()
	...
	local function fnShowBeyInfo(self, pPlayer)
		pPlayer.SendBlackBoardMsg("本轮轮空，直接晋级！", true);
		...
		if MODULE_ZONESERVER and self.nType == TeamBattle.TYPE_NORMAL then
			CallZoneClientScript(pPlayer.nZoneIndex, "TeamBattle:OnSyncWin", pPlayer.dwOrgPlayerId);
...
function tbFightMap:PreStart()
	local function fnPreStart(self, pPlayer)
		...
		if tbBeginFightPos then
			...
			pPlayer.CenterMsg("资料异常，请联系客服！");
			TeamBattle:GoBack(pPlayer);
			Log("[TeamBattle] fnPreStart Err", pPlayer.dwID, pPlayer.dwOrgPlayerId, pPlayer.szName, nTeamId)
		end
	end
...
function tbFightMap:GotoRest(bIsMid)
	local function fnGotoRest(self, pPlayer)
		...
		if tbRestPos then
			if bIsMid then
				pPlayer.SendBlackBoardMsg("上半场结束，中场休息", true);
			end
			...
			pPlayer.CenterMsg("资料异常，请联系客服！");
			TeamBattle:GoBack(pPlayer);
			Log("[TeamBattle] fnGotoRest Err", pPlayer.dwID, pPlayer.dwOrgPlayerId, pPlayer.szName, nTeamId)
		end
	end
...
function tbFightMap:ClcResult()
	...
	if self.tbDmgInfo[2].nKillCount == self.tbDmgInfo[2].nKillCount and self.tbDmgInfo[2].nTotalDmg == self.tbDmgInfo[1].nTotalDmg then
		local tbCount = {0,0};
		local function fnCountPlayer(self, pPlayer, nTeamId)
			tbCount[nTeamId] = tbCount[nTeamId] or 0;
			tbCount[nTeamId] = tbCount[nTeamId] + 1;
		end
		self:ForeachPlayer(1, fnCountPlayer, 1);
		self:ForeachPlayer(2, fnCountPlayer, 2);

		if tbCount[1] ~= tbCount[2] then
			nWinTeamId = tbCount[1] > tbCount[2] and 1 or 2;
		end
	end
	...
	local function fnSetShowInfo(self, pPlayer)
		...
		if self.tbDmgInfo[nTeamId] then
			if self.tbDmgInfo[nTeamId].tbPlayerDmg[nUniqId] then
				nDamage = self.tbDmgInfo[nTeamId].tbPlayerDmg[nUniqId].nDamage;
			end

			if self.tbDmgInfo[nTeamId].tbPlayerKillCount then
				nKillCount = self.tbDmgInfo[nTeamId].tbPlayerKillCount[nUniqId] or 0;
			end
		end
		tbShowInfo[nTeamId] = tbShowInfo[nTeamId] or {};
		tbShowInfo[nTeamId][nUniqId] = {
			szName		= pPlayer.szName,
			nPortrait	= pPlayer.nPortrait,
			nLevel		= pPlayer.nLevel,
			nHonorLevel	= pPlayer.nHonorLevel,
	...
	local function fnNotifyResult(self, pPlayer)
		...
		Lib:CallBack({self.TLogRoundFlow, self, pPlayer, nCostTime, nResult})
		...
		local nKillCount = self.tbQQReportKillCountInfo[nUniqId] or 0;
		TeamBattle:ReportQQScore(pPlayer, Env.QQReport_TeamBattle_KillCount, nKillCount);

		local nKillCount2 = self.tbQQReportKillCount2Info[nUniqId] or 0;
		TeamBattle:ReportQQScore(pPlayer, Env.QQReport_TeamBattle_KillCount2, math.max(nKillCount2, nKillCount - nKillCount2));
		TeamBattle:ReportQQScore(pPlayer, Env.QQReport_TeamBattle_DeathCount, self.tbQQReportDeathCountInfo[nUniqId] or 0);
	end
...
function tbFightMap:TLogRoundFlow(pPlayer, nCostTime, nResult)
	if MODULE_ZONESERVER then
		CallZoneClientScript(pPlayer.nZoneIndex, "TeamBattle:TLogRoundFlow", pPlayer.dwOrgPlayerId, nCostTime, nResult);
		return;
	end
	TeamBattle:TLogRoundFlow(pPlayer.dwID, nCostTime, nResult)
end
...
function tbFightMap:OnPlayerDeath(pKiller)
	local nUniqId = TeamBattle:GetUniqIdByPlayer(me)
	local nMyTeamId = self:GetTeamId(me);
	local nKillerTeamId = nMyTeamId == 1 and 2 or 1;

	if pKiller then
		local pKillPlayer = pKiller.GetPlayer();
		if pKillPlayer then
			local nKillUniqId = TeamBattle:GetUniqIdByPlayer(pKillPlayer)
			self.tbDmgInfo[nKillerTeamId].tbPlayerKillCount = self.tbDmgInfo[nKillerTeamId].tbPlayerKillCount or {};
			self.tbDmgInfo[nKillerTeamId].tbPlayerKillCount[nKillUniqId] = self.tbDmgInfo[nKillerTeamId].tbPlayerKillCount[nKillUniqId] or 0;
			self.tbDmgInfo[nKillerTeamId].tbPlayerKillCount[nKillUniqId] = self.tbDmgInfo[nKillerTeamId].tbPlayerKillCount[nKillUniqId] + 1;
			...
			self.tbQQReportKillCountInfo[nKillUniqId] = self.tbQQReportKillCountInfo[nKillUniqId] or 0;
			self.tbQQReportKillCountInfo[nKillUniqId] = self.tbQQReportKillCountInfo[nKillUniqId] + 1;

			if not self.tbQQReportDeathCountInfo[nKillUniqId] or self.tbQQReportDeathCountInfo[nKillUniqId] <= 0 then
				self.tbQQReportKillCount2Info[nKillUniqId] = self.tbQQReportKillCount2Info[nKillUniqId] or 0;
				self.tbQQReportKillCount2Info[nKillUniqId] = self.tbQQReportKillCount2Info[nKillUniqId] + 1;
			end
	...
	self.tbQQReportDeathCountInfo[nUniqId] = self.tbQQReportDeathCountInfo[nUniqId] or 0;
	self.tbQQReportDeathCountInfo[nUniqId] = self.tbQQReportDeathCountInfo[nUniqId] + 1;
...
function tbFightMap:OnEnter()
	TeamBattle:CachePlayerId(me)
	local nUniqId = TeamBattle:GetUniqIdByPlayer(me)
	if MODULE_ZONESERVER and self.nType == TeamBattle.TYPE_NORMAL then
		local tbCache = TeamBattle.tbAllCacheData[TeamBattle:GetUniqIdByPlayer(me)];
		if tbCache then
	...
	self.tbRegisterInfo[nUniqId] = {};
	self.tbRegisterInfo[nUniqId].nOnDeathRegID = PlayerEvent:Register(me, "OnDeath", function(pKiller) self:OnPlayerDeath(pKiller); end);
	self.tbRegisterInfo[nUniqId].nOnReviveRegID = PlayerEvent:Register(me, "OnRevive", function () end);
...
function tbFightMap:OnLogin()
	Timer:Register(Env.GAME_FPS * 3, function (self, nPlayerId)
		...
		TeamBattle:CachePlayerId(pPlayer)
	end, self, me.dwID);
end
...
function tbFightMap:OnLeave()
	...
	PlayerEvent:UnRegister(me, "OnDeath", self.tbRegisterInfo[nUniqId].nOnDeathRegID);
	PlayerEvent:UnRegister(me, "OnRevive", self.tbRegisterInfo[nUniqId].nOnReviveRegID);
	self.tbRegisterInfo[nUniqId] = nil;
	self:ClearState(me);
end
