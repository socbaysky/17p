...
local tbRespondTips = {
	[RSP_CODE_OK]            = "成功领取礼包码, 请到信箱当中查看";
	[RSP_CODE_USED]          = "该礼包已被使用";
	[RSP_CODE_CHANNEL_WRONG] = "该礼包码不可在本渠道使用";
	[RSP_CODE_SERVER_WRONG]  = "该礼包码不可在本服使用";
	[RSP_CODE_NOT_EXIST]     = "该礼包码不存在";
	[RSP_CODE_DB_WRONG]      = "验证礼包码的时候, 伺服器发生了一个错误";
}
...
function CodeAward:SendXGCodeGift(nPlayerId, szGiftType)
	...
	if pPlayer then
		pPlayer.CenterMsg("成功领取礼包码, 请到信箱当中查看");
	end
