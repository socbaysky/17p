...
function RankBattle:SyncRankEnemy(pPlayer, nVersion, bRefresh)
	...
	if bRefresh and self.tbEnemy[nPlayerId] and self.tbEnemy[nPlayerId].nFrashEnemyCD >= nCurTime then
		pPlayer.CenterMsg("您刷新对手过快，请稍後再试")
		return
	end
...
function RankBattle:StartBattle(pPlayer, nFightType, nRankNo, nId)
	if Env:CheckSystemSwitch(pPlayer, Env.SW_ChuangGong) then
	...
	if not self.tbEnemy[nPlayerId] then
		pPlayer.CenterMsg("您的名次已经发生了变化，重新为您更新挑战列表");
		self:SyncRankEnemy(pPlayer, 0)
		return;
	end

	if pPlayer.nLevel < 12 then
		pPlayer.CenterMsg("武神殿12级开放");
		return;
	end

	if not AsyncBattle:CanStartAsyncBattle(pPlayer) then
		pPlayer.CenterMsg("目前地图无法挑战，请先返回[FFFE0D]「襄阳城」[-]再尝试")
		return;
	end

	if pPlayer.dwTeamID > 0 then
		pPlayer.MsgBox("组队状态无法进行挑战，是否退出目前队伍？",
			{
				{"退出队伍", TeamMgr.QuiteTeam, TeamMgr, pPlayer.dwTeamID, pPlayer.dwID},
				{"取消"},
			})
		return;
	end
	...
	if tbEnemy then
		if tbEnemy.nFightType ~= nFightType or (nFightType == self.FIGHT_TYPE_NPC and self.tbRank[nRankNo].nPlayerId) or
			...
			-- 该名次已经易主
			pPlayer.CenterMsg("您要挑战的玩家名次发生了变化")
		...
		if self.tbLockNo[nRankNo] then
			if self.tbLockNo[nRankNo].nTimeOut > nCurTime then	-- 木有超时
				pPlayer.CenterMsg("对方正在进行挑战，请稍後再试")
				return;
			else
		...
		if tbMyRankInfo and self.tbLockNo[tbMyRankInfo.nRankNo] then
			if self.tbLockNo[tbMyRankInfo.nRankNo].nTimeOut > nCurTime then	-- 木有超时
				pPlayer.CenterMsg("您正在被其他玩家挑战，请稍後再试")
				return;
			else
...
function RankBattle:OnRankBattleResult(pPlayer, nResult, tbBattleObj, nEnemyId, nRankNo, nBeginTime)
	if nResult == 1 then
		if Forbid:IsBanning(pPlayer,Forbid.BanType.WuShenRank) then
	        ...
	        pPlayer.MsgBox(string.format("您由於%s被禁止上榜，解禁时间%s",Forbid:BanTips(pPlayer,Forbid.BanType.WuShenRank), szTime or ""), {{"确定"}, {"取消"}})
		end
		local nPlayerId = pPlayer.dwID;
		...
		pPlayer.CenterMsg(string.format("获得[FFFE0D] %d [-]点经验", pPlayer.TrueChangeExp(RankBattle.WIN_AWARD * pPlayer.GetBaseAwardExp())))
	else
		self:UnLockRankNo(nRankNo);

		pPlayer.CenterMsg(string.format("获得[FFFE0D] %d [-]点经验", pPlayer.TrueChangeExp(RankBattle.LOST_AWARD* pPlayer.GetBaseAwardExp())))
	end
...
function RankBattle:InitFakeAsyncData(nNpcTeamId)
	...
	function (nMapId, nX, nY)
		...
		local pNpc = KPlayer.AddFakePlayer(tbSetting.Faction, nSex, tbSetting.Level, nMapId, nX, nY)
		if pNpc then
	...
	function ()
		local nSex = Player:Faction2Sex(tbSetting.nFaction);
		return tbSetting.HeroName, tbSetting.Faction, tbSetting.Level, nSex;
	end;
