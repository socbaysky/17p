...
function Lottery:Load()
	...
	-- 由于 bIsOpen 是后期补上的，外网旧数据没有此值，因此此处对旧数据初始化其值
	local tbRecord = self:GetRecordData();
	if tbRecord.bIsOpen == nil and tbRecord.nDrawWeek then
		tbRecord.bIsOpen = true;
		ScriptData:SaveAtOnce(KEY_RECORD, tbRecord);

		Log("[Lottery] init old record", tbRecord.nDrawWeek);
	end
...
function Lottery:RecordData(tbLucky)
	local tbData = self:GetRecordData();
	local nRecordWeek = tbData.nDrawWeek;
	tbData.bIsNotify = nil;
	tbData.tbLucky = tbLucky;
	tbData.nDrawWeek = tbData.nDrawWeek + 1;

	local nDrawWeek = self:GetDrawWeek() + 1;
	if tbData.nDrawWeek < nDrawWeek then
		tbData.nDrawWeek = nDrawWeek;
		Log("[WARNING][Lottery] draw week not match!!", nRecordWeek, nDrawWeek);
	end
	ScriptData:SaveAtOnce(KEY_RECORD, tbData);

	KPlayer.BoardcastScript(1, "Lottery:OnSyncDrawWeek", tbData.nDrawWeek);

	Log("[Lottery] record data! week:", nRecordWeek, tbData.nDrawWeek);
end
...
function Lottery:SendAward(dwPlayerId, nRank)
	...
	local tbMail = 
	{
        ...
        Title = "盟主的赠礼";
        From = "武林盟主";
        Text = "少侠为维护武林秩序做出的杰出贡献，吾与其他侠士尽皆看在眼里，今奉薄礼一份，以助少侠一臂之力。还望少侠在江湖中继续努力，闯荡出自己的一番天地。";
        tbAttach = tbAward;
...
function Lottery:IsAvailable()
	if not Lottery:IsOpen() then
		return false, "活动尚未开启，稍安勿躁！";
	end

	if Lottery.IsForbid then
		return false, "系统繁忙，请稍候再试！";
	end
...
function Lottery:ExchangeTicket(pPlayer)
	...
	pPlayer.CenterMsg(string.format("使用成功，本周已使用盟主的赠礼%d张", nCount, 1));
...
function Lottery:Notify()
	...
	local tbSetting = { szTitle = "盟主的赠礼", szUiName = "LotteryNotify" }; 
...
function Lottery:IsOpen()
	local tbData = self:GetRecordData();
	return tbData.bIsOpen;
end

function Lottery:Open()
	local tbData = self:GetRecordData();
	if tbData.bIsOpen then
		return;
	end
	...
	KPlayer.BoardcastScript(1, "Lottery:OnSyncState", true, tbData.nDrawWeek);
end

function Lottery:Close()
	local tbData = self:GetRecordData();
	if not tbData.bIsOpen then
		return;
	end
	...
	Log("[Lottery] lottery close!", tbData.nDrawWeek);

	KPlayer.BoardcastScript(1, "Lottery:OnSyncState", false, tbData.nDrawWeek);
end
...
function Lottery:SendWorldMsg(nMinute)
	...
	local szMsg = string.format("武林盟主将在%d分钟之後从本周使用过<盟主的馈赠>的少侠中挑选幸运儿赠送惊喜大礼，敬请期待！", nMinute);
...
function Lottery:SyncLotteryState(pPlayer)
	local tbData = self:GetRecordData();
	pPlayer.CallClientScript("Lottery:OnSyncState", self:IsOpen(), tbData.nDrawWeek);
end