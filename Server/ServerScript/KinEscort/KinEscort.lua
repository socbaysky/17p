KinEscort.szKinEscortOpenMessage 		= "帮派运镖活动开启，各帮派管理可前往夜郎废墟的委托人处接取押镖任务";    --家族运镖活动开启系统通告
KinEscort.szKinEscortCloseMessage 		= "帮派运镖活动结束";    --家族运镖活动关闭系统通告

KinEscort.szNpcTips = "一大堆的货物要运出去，真是件头疼的事情啊……";               --劫镖NPC提示语
KinEscort.szTitle   = "%s 帮派护送";                                              --镖车称号

KinEscort.tbKinChannelMsg = {                                                     --家族频道公告
	["szStart"] = "帮派镖车已经发动！本次镖车为%s,帮派成员尽快前去护送%s",        --镖车启动时，%s:颜色，%s坐标链接
	["szComplete"] = "帮派运镖任务完成！",                                        --镖车完成时
	["szUnderAttack"] = "帮派镖车正遭受攻击, 请火速前往救援！%s",                 --镖车收到攻击时，%s坐标链接
	["szCrossHPStage"] = "帮派镖车血量低於%d%%，掉落镖物！",                      --镖车打到百分比时，%d：血量百分比
	["szUpgrade"] = "%s 花费%d帮派建设资金对帮派镖车进行升阶！目前品质为%s",      --镖车升阶时，%s：玩家名字，%s：品质，即颜色
	["szDeath"] = "帮派镖车被%s击破，本次帮派运镖失败！",                         --%s:killer
}
...
KinEscort.tbNpcColor = {"白色", "绿色", "蓝色", "紫色", "橙色"};            --对应镖车升级到品质1~5的NpcId所需建设资金
...
KinEscort.szDropRewardWrodMsg = "镖车被击毁，掉落部分物品";                       --劫镖过程掉落物品世界通告
	...
	for nId in pairs(pCarriageNpc.tbJoinPlayer) do
		if kinMembers[nId] then
			...
			Mail:SendSystemMail({
				To       = nId,
				Title    = "帮派运镖奖励",
				Text     = bArrive and "本次帮派运镖顺利完成，特发奖励以资鼓励" or "本次帮派运镖失败，望不要气馁，再接再厉",
				From     = "帮派总管",
				tbAttach = tbAttach,
		...
		if nOldHpPercent>tbThisHPStage.nHpStage and nNewHpPercent<=tbThisHPStage.nHpStage then
		    ...
		    if tbSortDamage and #tbSortDamage>0 then
			    ...
			    if tbSortDamage[1].nTotalDamage > 0 then
			    	...
			    	if pNpc and pNpc.dwPlayerID > 0 then
			    		self:AddRewardToAroundPlayerBySameTeam(him.nId, pNpc.dwPlayerID, 0, 1, tbThisHPStage);
			    	end
				end
		    end
...
function KinEscort:CheckUpgrade(nPlayerId, nCarriageNpcId, nFoundCost)
	...
	if nFoundCost > tbKin:GetFound() then
		pPlayer.CenterMsg("帮派建设资金不足")
		return false
	end

	local tbKinState = self:GetKinEscortData(pCarriageNpc.dwKinId);
	if tbKinState.nState~=self.States.prepare then
		pPlayer.CenterMsg("镖车不在待发状态，升阶失败!")
		return false
	end
...
function KinEscort:OnUpgrade(nPlayerId, nKinId, bConfirm)
	...
	if not bConfirm  then
		pPlayer.MsgBox(string.format("花费%d帮派资金对镖车进行升阶吗？", nFoundCost),
					   {{"确认", self.OnUpgrade, self, nPlayerId, nKinId, true}, {"取消"}});
		return ;
	end
