...
function tbNpc:SendPlayerAward(nPlayerID, tbNpcInfo, tbPunishTaskInfo, nCaptainID)
    ...
    if pPlayer.nLevel < PunishTask.nMinPlayerLevel then
        pPlayer.CenterMsg(string.format("等级不足%s级", PunishTask.nMinPlayerLevel), true);
        return;
    end  
    ...
    pPlayer.Msg(string.format("今日惩恶任务奖励剩余%s次", nShowCount));
    Dialog:SendBlackBoardMsg(pPlayer, string.format("今日惩恶任务奖励剩余%s次", nShowCount));
