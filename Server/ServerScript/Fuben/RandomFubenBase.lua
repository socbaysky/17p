...
function tbFuben:OnFirstJoin(pPlayer)
	Timer:Register(Env.GAME_FPS * 5, function (nPlayerId)
		...
		if pP then
			pP.SendBlackBoardMsg("秘境即将开启，原地等待队友集合後进行探险！");
		end
	end, pPlayer.dwID);
...
function tbFuben:OnPlayerDeath()
	...
	me.SendBlackBoardMsg(string.format("您已身受重伤，将於%s秒後复活", nReviveTime));
...
function tbFuben:FinishGame()
	...
	local fnExcute = function (pPlayer)
		...
		local szMsg = self.nWinCount >= RandomFuben.RANDOM_COUNT and "完美闯关" or "闯关成功 离开凌绝峰";
		pPlayer.CallClientScript("Fuben:SetFubenProgress", -1, szMsg);

		szMsg = self.nWinCount >= RandomFuben.RANDOM_COUNT and "恭喜您闯关成功！篝火已刷出可持续获得经验！" or "恭喜您闯关成功，现在可以离开了！";
...
function tbFuben:GameWin()
	...
	if self.nRandomCount >= RandomFuben.RANDOM_COUNT then
		...
		local fnExcute = function (pPlayer)
			...
			pPlayer.CallClientScript("Fuben:SetFubenProgress", -1, "继续探索凌绝峰");
		end
		self:AllPlayerExcute(fnExcute);
...
function tbFuben:GameLost()
	...
	if self.nRandomCount >= RandomFuben.RANDOM_COUNT then
		...
		local fnExcute = function (pPlayer)
			pPlayer.CallClientScript("Fuben:SetEndTime", GetTime() + RandomFuben.AFTER_END_TIME);
			pPlayer.CallClientScript("Fuben:SetFubenProgress", -1, "另寻道路上山");
		end
		self:AllPlayerExcute(fnExcute);
...
function tbFuben:ConfirmFubenMission(pPlayer, bIsLeaveMap, bNotShowMissionAward)
	...
	if self.nTeamLeader and self.nTeamLeader == pPlayer.dwID then
		...
		if pPlayer.dwTeamID > 0 then
			local szMsg = string.format("本次评价为「%s」，队长额外获得%s点贡献", MissionAward.tbGradeDesc[szType], nLeaderExtAward);
...
function tbFuben:OnCopyPlayer(nLockId, szGroup, szPointName, bRevive, nDir, nDealyTime, nEffectId, nEffectTime)
	...
	for _, pPlayer in pairs(tbAllPlayer) do
		...
		if tbNpcInfo then
			self:_AddNpc(tbNpcInfo.nNpcIndex, 1, nLockId, szGroup, string.format("_____copy_pos__%s_%s", szPointName, nCurIdx), bRevive, nDir, nDealyTime, nEffectId, nEffectTime, "CopyPlayer", string.format(tbNpcInfo.szName, pPlayer.szName), pPlayer.dwID, pPlayer.nFaction);
...
function tbFuben:OnAddNpc(pNpc, szType, szName, nPlayerId, nFaction)
	...
	if nPlayerId then
		local nResId, tbPartRes = KPlayer.GetNpcResId(nFaction, nSex);
		...
		local nHead = tbPartRes[Npc.NpcResPartsDef.npc_part_head] or 0;
		...
		if tbEquipInfo then
			...
			local nArmorResId = tbArmor and tbArmor.nShowResId or 0;

			nWeaponResId = tbWeapon and tbWeapon.nShowResId or 0;

			if tbWaiWeapon then
				nWeaponResId = tbWaiWeapon.nShowResId;
			end

			if tbWaiArmor then
				nArmorResId = tbWaiArmor.nShowResId;
			end

			pNpc.ChangeFeature(nResId, Npc.NpcResPartsDef.npc_part_body, nArmorResId == 0 and nBodyResId or nArmorResId);
			pNpc.ChangeFeature(nResId, Npc.NpcResPartsDef.npc_part_weapon, nWeaponResId);
			pNpc.ChangeFeature(nResId, Npc.NpcResPartsDef.npc_part_head, nHead);
		end
	end
end
