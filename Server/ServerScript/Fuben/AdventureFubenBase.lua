...
function tbFuben:OnPlayerDeath()
    ...
    Dialog:SendBlackBoardMsg(me, string.format("您已身受重伤，将於%s秒後复活", nReviveTime or 1))
end
...
function tbFuben:GameWin()
    ...
    if pCaptain then
        ...
        if pCaptain.dwTeamID > 0 then
            ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, string.format("队长额外获得%d点贡献", nContrib), pCaptain.dwTeamID)
        end
    end
