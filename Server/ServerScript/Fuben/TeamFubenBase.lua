...
function tbFuben:OnJoin(pPlayer)
	...
	if not bCost then
		...
		pPlayer.CenterMsg("您处於助战状态，不能获得闯关奖励！", true);
...
function tbFuben:ShowMissionAward(pPlayer)
	if not self.tbPlayerState[pPlayer.dwID] then
		pPlayer.CenterMsg("您处於助战状态，不能获得闯关奖励！");
		return;
	end
...
function tbFuben:OnPlayerDeath()
	me.CallClientScript("Ui:OpenWindow", "CommonDeathPopup", "AutoRevive", "您将在 %d 秒後复活", TeamFuben.REVIVE_TIME);
