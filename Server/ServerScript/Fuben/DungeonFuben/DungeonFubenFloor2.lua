...
function tbFuben:DigMine(pPlayer, pNpc, nItemId)
	...
	if self.tbDigNum[pPlayer.dwID] >= self.nMaxPickNum then
		pPlayer.CenterMsg("本次地底秘藏的采集次数已耗尽")
		return
	end
 	pNpc.Delete();
 	self.nCurnMineNum = self.nCurnMineNum - 1
 	local nRet, szMsg = Item:GetClass("RandomItemByLevel"):GetAwardByLevel(pPlayer, nItemId, "发现地宫奖励")
	if szMsg then
	...
	pPlayer.CallClientScript("Fuben:SetTargetInfo", string.format("剩余采集次数：%d", (self.nMaxPickNum - self.tbDigNum[pPlayer.dwID])), self.nEndTime)
end
...
function tbFuben:OnJoin(pPlayer)
	...
	if self.tbDigNum then
		tbFubenInfo.szName = string.format("剩余采集次数：%d", self.nMaxPickNum - (self.tbDigNum[pPlayer.dwID] or 0) )
	end
...
function tbFuben:GameLost()
	self.bLost = true
	local fnExcute = function (pPlayer)
		pPlayer.SendBlackBoardMsg("神秘洞窟的存在时间已结束");
