...
function tbFuben:OnStartFight()
    ...
    Timer:Register(Env.GAME_FPS * tbFubenMgr.PREPARE4PK, function ()
        ...
        self:SetFubenProgress(-1, "等待一层头目现身")
...
function tbFuben:InitFightRoom()
    ...
    for i = 1, self.nTopRoom do
        ...
        local szTitle = (nFloor <= 2) and "等待开启PK模式" or string.format("等待%s层头目出现", tbFloorName[nFloor])
...
function tbFuben:OnOpenCrossDoor()
    ...
    CallZoneServerScript("Fuben.WhiteTigerFuben:SyncKillBossKinId", self.nWinKin, tbKinData.szName, nServerId, nServerIdx)
...
function tbFuben:OnGotoCrossFuben()
    if me.dwKinId ~= 0 and me.dwKinId == self.nWinKin then
        ...
        Timer:Register(2, function ()
            ...
            if nTeamId > 0 then
                TeamMgr:QuiteTeam(nTeamId, pPlayer.dwID)
            end
            local nX, nY = unpack(tbFubenMgr.tbMainRoomTrap[tbFubenMgr.nCrossRoomIdx])
            local bRet = pPlayer.SwitchZoneMap(tbFubenMgr.nCrossMapId, nX, nY)
            if bRet then
                local nServerId = GetServerIdentity()
                if nTeamId > 0 then
                    CallZoneServerScript("Fuben.WhiteTigerFuben:JoinZoneClientTeam", nServerId, pPlayer.dwID, nTeamId)
                end
            else
                pPlayer.CenterMsg("副本进入失败")
            end
        end)
    end
end
