    ...
    if pPlayer then
        ...
        if pPlayer.dwKinId > 0 then
            ...
            if nCombo >= 20 and nCombo <= 100 and nCombo%10 == 0 then
                local szMsg = string.format("「%s」在白虎堂中，连斩%d人！", pPlayer.szName, nCombo)
...
function tbFuben:CalcBossHpPercent(pNpc)
    ...
    local tbRoomInfo = self.tbRoomBoss[pNpc.nId]
    local nServerId, nOrgKinId = Fuben.WhiteTigerFuben:GetKinIdByMap(self.nMapId, tbRoomInfo.nRoomIdx)
    if not nServerId or not nOrgKinId then
        Log("WhiteTigerFubenCrossBase CalcBossHpPercent Err No Kin", self.nMapId, tbRoomInfo.nRoomIdx)
        return
    end
    local nKinId = KPlayer.GetZoneKinIdByOrgId(nServerId, nOrgKinId)
    if nKinId == 0 then
        Log("WhiteTigerFubenCrossBase CalcBossHpPercent Err OrgKin Tran Fail", self.nMapId, tbRoomInfo.nRoomIdx)
        return
    end
...
function tbFuben:SendRedbag(pKiller)
    ...
    if not pKiller.dwOrgPlayerId then
        return
    end

    local nWs = Fuben.WhiteTigerFuben:GetWsByKinId(pKiller)
    if nWs then
        CallZoneClientScript(nWs, "Fuben.WhiteTigerFuben:OnCrossTopBossKill", pKiller.dwOrgPlayerId)
    end
end
...
function tbFuben:OnLeaveMap(pPlayer)
    ...
    local nWs = Fuben.WhiteTigerFuben:GetWsByKinId(pPlayer)
    if nWs then
        CallZoneClientScript(nWs, "Fuben.WhiteTigerFuben:OnLeaveCross", pPlayer.dwOrgPlayerId, self.tbComboInfo[pPlayer.dwID] or 0)
    end
end