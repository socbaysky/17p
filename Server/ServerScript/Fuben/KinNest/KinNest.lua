...
function KinNest:RandomActivity(nKinID, nRound, nNpcNumber, nLevel)
    ...
    if nRandom <= self.tbOpenPro[nRound] then
        ...
        if tbKin:AddClue(1) then
            local szMsg = "从线人手中得到的奸商地窖的线索，可前往奸商地窖，获得大量食材。"
...
function KinNest:ForceOpen(nKinID, nNpcNumber, nLevel)
    ...
    local szMsg = string.format("已获得前往「奸商地窖」及地窖中大量食材的线索，「堂主」及「副堂主」可找「帮派总管」开启，记得通知帮派成员集合")
    ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szMsg, nKinID)
...
function KinNest:ApplyOpenKinNest(pPlayer)
    ...
    local szMsg = string.format("「奸商地窖」活动已经开启，各位成员可寻找「帮派总管」对话进入");
...
function KinNest:CheckEnterKinNest(pPlayer)
    ...
    if pPlayer.nLevel < Kin.NestDef.nJoinLevel then
        return false, string.format("请先将等级升到%d级！", Kin.NestDef.nJoinLevel)
    end
...
function KinNest:OnLabaActStart(nEndTime)
    self.nLabaActEndTime = nEndTime
end

function KinNest:OnLabaActEnd()
    self.nLabaActEndTime = nil
end

function KinNest:FormatActAward(tbAward)
    if not self.nLabaActEndTime then
        return tbAward
    end
    local tbMergeAward = Lib:CopyTB(tbAward or {})
    local tbAct = Activity:GetClass("LabaAct") 
    Lib:MergeTable(tbMergeAward, tbAct:FormatAward(tbAct.tbFubenAward, self.nLabaActEndTime))
    return tbMergeAward
end
...
function KinNest:SendLastReward(nKinId)
    ...
    tbRewards = self:FormatActAward(tbRewards)
    tbKin:ResetClue()
    ...
    for nPid in pairs(tbAllPlayers) do
        ...
        if bInKin then
            Mail:SendSystemMail({
                To = nPid,
                Title = "勇闯地窖奖励",
                Text = "诸位帮派弟兄根据线索，成功击败奸商手下的保镖走狗，这些是从奸商处截获的食材，请注意查收！",
                From = "帮派总管",
                tbAttach = tbRewards,
                nLogReazon = Env.LogWay_KinNest,
            })
