...
function tbFuben:GameLost()
    local fnExcute = function (pPlayer)
        pPlayer.SendBlackBoardMsg("奸商地窖存在时间已结束");
...
function tbFuben:OnKillNpc(pNpc, pKiller)
    ...
    -- 击杀盗贼者与所在队伍好友,亲密度加50
    --local killer = KPlayer.GetPlayerObjById(pKiller.dwPlayerID) or {};
    ...
    for _, nPlayerID in pairs(tbTeamMembers) do
        if nPlayerID ~= pKiller.dwPlayerID then
            FriendShip:AddImitityByKind(nPlayerID, pKiller.dwPlayerID, Env.LogWay_KinRobber);
        end
    end
