...
function DefendFuben:CheckCanEnter(pTarget)
    ...
    if pTarget.dwTeamID == 0 then
        return false, string.format("请组成%d人队伍再来",DefendFuben.JOIN_MEMBER_COUNT),tbTeamMember
    end
  
    if #tbTeamMember ~= DefendFuben.JOIN_MEMBER_COUNT then
        return false, string.format("队伍人数必须为%d人",DefendFuben.JOIN_MEMBER_COUNT),tbTeamMember
    end
    ...
    for nIdx, nPlayerId in ipairs(tbTeamMember) do
        ...
        if pPlayer.nLevel < DefendFuben.JOIN_LEVEL then
            return false, string.format("「%s」不足%d级",pPlayer.szName, DefendFuben.JOIN_LEVEL),tbTeamMember
        end
