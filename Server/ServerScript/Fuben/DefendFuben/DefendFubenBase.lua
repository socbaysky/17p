...
function tbFuben:ActiveNpc(nNpcID,dwID)
	...
	if not self.bStart then
		pPlayer.CenterMsg("敌人尚未出现，请稍後片刻",true)
		return
	end
	...
	if self.tbActive[self.nRound] then
		pPlayer.CenterMsg("你已选择了一名助战者，请下轮再重新选取",true)
		return
	end

	if not self.tbDialogNpc[nNpcID] then
		pPlayer.CenterMsg("你要启动哪个NPC??",true)
		return
	end
	local tbSetting = self.tbSetting
...
function tbFuben:SendReward(pPlayer)
	...
	local szText = string.format("少侠，在五一江湖展身手活动中，你与同伴共成功抵御了[c8ff00] %d轮 [-]敌人的进攻，这是在下的小小心意，还望勿要拒绝！",self.nRound)
	local tbMail = {
		To = pPlayer.dwID;
		Title = szTitle;
		From = "独孤剑";
...
function tbFuben:OnPlayerDeath()
	me.CallClientScript("Ui:OpenWindow", "CommonDeathPopup", "AutoRevive", "您将在 %d 秒後复活", DefendFuben.REVIVE_TIME);
