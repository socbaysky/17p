...
function WhiteTigerFuben:SetupPreMapCallback()
    ...
    tbReadyMap.OnEnter = function (tbMap, nMapId)
        ...
        me.CallClientScript("Fuben:SetFubenProgress", -1, "选择一个入口进入")
...
function WhiteTigerFuben:NotifyPlayer()
    local szMsg = "各位少侠！白虎堂已经准时开启！通过「活动」前去挑战吧！"
...
function WhiteTigerFuben:TryBackToPrepareMap(pPlayer)
    ...
    elseif self.nState == self.STATE_FIGHTING then
        pPlayer.CenterMsg("战斗已开始，无法回到准备场")
        return
    end
...
function WhiteTigerFuben:TryEnterOutSideFuben(pPlayer, nRoomId)
    ...
    pPlayer.CenterMsg(string.format("你已经进入了白虎堂入口%s", tbIndex[nRoomId]), true)
...
function WhiteTigerFuben:OnKillCrossBoss(nKillKinId, tbHostileKin)
    ...
    local szDefeatMsg = string.format("本服%s帮派英勇击败了", tbKinData.szName)
    for i, tbInfo in ipairs(tbHostileKin) do
        local szDesc = Sdk:GetServerDesc(tbInfo[1])
        szDefeatMsg = string.format("%s%s的%s帮派%s", szDefeatMsg, szDesc, tbInfo[2], i == #tbHostileKin and "" or "和")
    end
    KPlayer.SendWorldNotify(1, 999, szDefeatMsg, 1, 1)

    local szMsg = string.format("恭喜%s帮派成功击杀了跨服白虎堂5层中的%s", tbKinData.szName, szBossName)
...
function WhiteTigerFuben:OnBeDefeatInCross(tbKinInfo, nKinId)
    ...
    local szDesc = Sdk:GetServerDesc(tbKinInfo[1])
    local szMsg = string.format("本帮派在跨服白虎堂中被来自%s的%s帮派击败", szDesc, tbKinInfo[2])
