...
function _TeamData:Available2Join(pPlayer)
	...
	if pPlayer.nLevel < TeamMgr.OPEN_LEVEL then
		return false, string.format("%d级以上才允许组队", TeamMgr.OPEN_LEVEL);
	end
	...
	local nCaptainID = self:GetCaptainId();
	local captain = KPlayer.GetPlayerObjById(nCaptainID or 0);
	if not captain or not captain.CanTeamOpt() or 
		( captain.nState == Player.emPLAYER_STATE_ZONE or pPlayer.nState == Player.emPLAYER_STATE_ZONE ) then
		return false, "所在地图不可进行队伍操作";
	end
...
function _TeamData:AddMember(nMemberID, bIgnoreCheck)
	...
	Lib:CallBack({ZhenFa.OnJoinTeam, ZhenFa, member, self:GetMembers()});
...
function _TeamData:RemoveMember(nMemberID)
	...
	Lib:CallBack({ZhenFa.OnLeaveTeam, ZhenFa, nMemberID, self:GetMembers()});
...
function _TeamData:UpdateCaptain()
	...
	local szTips = string.format("「%s」成为新队长", player.szName);
...
function _TeamData:ChangeCaptain(nTargetPlayerID)
	...
	if not targetPlayer or self.nTeamID ~= targetPlayer.dwTeamID then
		return false, "目标玩家不在当前队伍";
	end

	if self.nCaptainID == nTargetPlayerID then
		return false, "目标玩家已经是队长了";
	end
...
function _TeamData:ChangeTargetActivity(nActivityId)
			...
			if nActivityId then
				...
				if not bRet then
					...
					if bCanHelp then
						...
						member.CenterMsg("进入助战状态，参与活动不消耗次数也无法获得奖励！", true, ChatMgr.SystemMsgType.Team);
					else
	...
	if szActivityName then
		szTips = string.format("目前队伍目标更换为：%s", szActivityName);
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, szTips, self.nTeamID);
	end
