...
function Wedding:TryStartBookWedding(pPlayer)
	...
	if not nEngaged then
		pPlayer.CenterMsg("只有订婚关系才能开启婚礼", true)
		return
	end
	...
	if pPlayer.nState ~= Player.emPLAYER_STATE_NORMAL then
		pPlayer.CenterMsg("你处於非正常状态不能操作", true)
        return 
    end
    if pEngageder.nState ~= Player.emPLAYER_STATE_NORMAL then
    	pPlayer.CenterMsg(string.format("「%s」处於非正常状态不能操作", pEngageder.szName), true)
        return 
    end
    ...
    if fDists > (Wedding.OPEN_WEDDING_MIN_DISTANCE * Wedding.OPEN_WEDDING_MIN_DISTANCE) or nMapId1 ~= nMapId2 then
    	pPlayer.CenterMsg("此等大事还是和伴侣一起过来再进行吧", true)
        return
    end
	...
	if tbMapSetting.fnCheckBookOverdue(nOpen) then
		pPlayer.CenterMsg("预定资讯已经过期", true)
		return
	end

	if tbPlayerBookInfo.nOpenTime then
		pPlayer.CenterMsg("预定婚礼已经举办过", true)
		return
	end

	if not tbMapSetting.fnCheckBookIsOpen(nOpen) then
		pPlayer.CenterMsg("请在预定婚礼的当天来开启", true)
		return
	end
...
function Wedding:MakeLoveRelation(pPlayer, pEngageder, nWeddingLevel)
	...
	self:_SetTitle(pPlayer, string.format("%s的%s", pEngageder.szName, tbSuffix[nEngagederSex] or "爱人"), nil, true)
	self:_SetTitle(pEngageder, string.format("%s的%s", pPlayer.szName, tbSuffix[nPlayerSex] or "爱人"), nil, true)
...
function Wedding:TryBookWedding(pPlayer, nLevel, nBookTime)
	...
	if not bRet then
		pPlayer.CenterMsg(szMsg or "敬请期待")
		return
	end
...
function Wedding:DoBookWedding(nPlayerId, bSuccess, nLevel, nBookTime, nCost)
	...
	if not bSuccess then
		pPlayer.CenterMsg("支付失败请稍後再试！", true)
	...
	if tbMapSetting.bBook then
		...
		local szMsg = string.format("恭喜[FFFE0D]「%s」[-]和[FFFE0D]「%s」[-]在月老祠预定了[FFFE0D]「%s」[-]婚礼，良辰吉日定在了%s", pPlayer.szName, pEngageder.szName, tbMapSetting.szWeddingName, szDateStr)
		KPlayer.SendWorldNotify(1, 999, szMsg, 1, 1)
		if pPlayer.dwKinId ~= 0 then
            szMsg = string.format("恭喜帮派成员[FFFE0D]「%s」[-]和[FFFE0D]「%s」[-]在月老祠预定了[FFFE0D]「%s」[-]婚礼，良辰吉日定在了%s", pPlayer.szName, pEngageder.szName, tbMapSetting.szWeddingName, szDateStr)
        ...
        if pEngageder.dwKinId ~= 0 and pEngageder.dwKinId ~= pPlayer.dwKinId then
            szMsg = string.format("恭喜[FFFE0D]「%s」[-]和[FFFE0D]「%s」[-]在月老祠预定了[FFFE0D]「%s」[-]婚礼，良辰吉日定在了%s", pEngageder.szName, pPlayer.szName, tbMapSetting.szWeddingName, szDateStr)
	...
	if not bOpenMail and tbMapSetting.bBook then
		local tbPlayerMail = {
			...
			Title = "婚礼预定";
			From = "月老";
			Text = string.format("    恭喜你和「%s」成功预定了[FFFE0D]「%s」[-]婚礼，良辰吉日定在了[FFFE0D]%s[-]。你们可以在%s的[FFFE0D]10:00~24:00时间段[-]举行婚礼，届时还请你们双方组队来我这里开启婚礼。\n    [FFFE0D]提示：若婚礼没有在约定的时间内举办，则需要补交一半的费用重新预定。[-]", pEngageder.szName, szWeddingName, szDateStr, szDate);
		...
		local tbEngagedeMail = {
			...
			Title = "婚礼预定";
			From = "月老";
			Text = string.format("    恭喜你和「%s」成功预定了[FFFE0D]「%s」[-]婚礼，良辰吉日定在了[FFFE0D]%s[-]。你们可以在%s的[FFFE0D]10:00~24:00时间段[-]举行婚礼，届时还请你们双方组队来我这里开启婚礼。\n    [FFFE0D]提示：若婚礼没有在约定的时间内举办，则需要补交一半的费用重新预定。[-]", pPlayer.szName, szWeddingName, szDateStr, szDate);
			nLogReazon = Env.LogWay_WeddingBook;
		};
		Mail:SendSystemMail(tbEngagedeMail);
	end
	if tbMapSetting.bBook then
		pPlayer.CenterMsg(string.format("成功预定了[FFFE0D]「%s」[-]婚礼，良辰吉日定在了[FFFE0D]%s[-]", szWeddingName, szDateStr), true)
		pEngageder.CenterMsg(string.format("成功预定了[FFFE0D]「%s」[-]婚礼，良辰吉日定在了[FFFE0D]%s[-]", szWeddingName, szDateStr), true)
	end
...
function Wedding:CheckConsumeItem(pPlayer, szItem)
	...
	for _, szItemInfo in ipairs(tbStrItem) do
		...
		if nHave < nCount then
			return false, string.format("【%s】不足", KItem.GetItemShowInfo(nItemId, pPlayer.nFaction, pPlayer.nSex) or "")
		end
	end
	return true
end
...
function Wedding:DoConsumeItem(pPlayer, szItem)
	...
	for _, szItemInfo in ipairs(tbStrItem) do
		...
		if nConsume < nCount then
			...
			return false, string.format("消耗道具【%s】失败", KItem.GetItemShowInfo(nItemId, pPlayer.nFaction, pPlayer.nSex) or "")
		end
	end
	return true
end
...
function Wedding:CheckBeforeBook(pPlayer, nLevel, nBookTime)
	...
	if not tbMapSetting then
		return false, "不能预定该档次的婚礼"
	end
	local szDebtTip = "「%s」处於欠款状态，不能预定婚礼"
	if not tbMapSetting.bBook then
		szDebtTip = "「%s」处於欠款状态，不能举行婚礼"
	...
	if pPlayer.nState ~= Player.emPLAYER_STATE_NORMAL then
        return false, "你处於非正常状态不能操作"
    end
    if pEngageder.nState ~= Player.emPLAYER_STATE_NORMAL then
        return false, string.format("「%s」处於非正常状态不能操作", pEngageder.szName)
    end
    local bRet, szMsg = Wedding:CheckWeddingSex(pPlayer, pEngageder)
    if not bRet then
    	return false, szMsg
    ...
    if fDists > (Wedding.BOOK_WEDDING_MIN_DISTANCE * Wedding.BOOK_WEDDING_MIN_DISTANCE) or nMapId1 ~= nMapId2 then
        return false, "此等大事还是和伴侣一起过来再进行吧"
    end

    if tbMapSetting.nNeedVip and pPlayer.GetVipLevel() < tbMapSetting.nNeedVip then
    	return false, string.format("剑侠尊享%s才可预定该婚礼", tbMapSetting.nNeedVip)
    end
...
function Wedding:CheckCost(pPlayer, nLevel, nBookTime)
	...
	if nBookLevel and not tbPlayerBookInfo.nOpenTime then
		...
		if not tbMapBookSetting then
			return false, "有未知档次的预定资料？？"
		end
		-- nBook 是想要预定的时间点
		local nBook = tbMapSetting.fnGetDate(nBookTime)
		-- 选择的时间点就是自己已经预定的时间点
		if nBook == nOpen then
	 		return false, "你已经预定了时间段的婚礼"
		end
...
function Wedding:CheckBookDay(pPlayer, nLevel, nBookTime)
	...
	if nBookDay < nLocalDay then
		return false, "不能预定已经过去的时间点"
	end

	if nBookDay - nLocalDay + 1 > tbMapSetting.nPre then
		return false, string.format("不能预定%d天之後的日期", tbMapSetting.nPre)
	end
...
function Wedding:CheckBookWeek(pPlayer, nLevel, nBookTime)
	...
	if nBookWeek - nLocalWeek + 1 > tbMapSetting.nPre then
		return false, string.format("不能预定%d周之後的日期", tbMapSetting.nPre)
	end
					...
					if not tbPlayerBookInfo.nOpenTime then 								-- 还没举行婚礼
						if not tbPlayerBookInfo.nSendOverdueMailTime and bOverdue then
							...
							for dwID, nSex in pairs(tbPlayerBookInfo.tbPlayer) do
								...
								local szLoverName = pLoverStayInfo.szName or "爱人"
								local tbOverdueMail = {
									To = dwID;
									Title = "婚礼逾期通知";
									From = "月老";
									Text = string.format("    你和「%s」预定的[FFFE0D]「%s」婚礼[-]由於没有在约定时间内举办，现已逾期。\n    你们可以在[FF6464FF]60天内[-]来我这[FFFE0D]补交一半的费用[-]重新预定[FFFE0D]「%s」婚礼[-]，或者预定其他婚礼。", szLoverName, tbMapSetting.szWeddingName, tbMapSetting.szWeddingName);
								};
	...
	if not tbPlayerBookInfo.nSendMailTime then
		...
		for dwID, nSex in pairs(tbPlayerBookInfo.tbPlayer) do
			...
			local tbWarnMail = {
				...
				Title = "婚礼提醒";
				From = "月老";
				Text = string.format("    %s是你和「%s」约定的婚期，你们预定的婚礼是[FFFE0D]「%s」[-]，可以在%s之内的[FFFE0D]10:00~24:00时间段[-]组队来我这里开启婚礼。\n    [FFFE0D]提示：若婚礼没有在约定的时间内举办，则需要补交一半的费用重新预定。[-]", szCurDate, szLoverName, szWeddingName, szCurDate);
			};
