...
function WeddingTourBase:SendMsg()
	...
	local function fnMsg(self, pPlayer)
		 local tbNofityData = {
   		...
   		tbNofityData.szContent = string.format("你的好友「%s」正在%s举行花轿游城，是否前往观礼？", pPlayer.szName, szMapName)
  	 	...
  	 	if pPlayer.dwKinId > 0 then
		    ...
		    tbNofityData.szContent = string.format("帮派成员「%s」正在%s举行花轿游城，是否前往观礼？", pPlayer.szName, szMapName)
...
function WeddingTourBase:SetNpcInfo(pNpc, nPlayerId, szDefaultName)
	...
	if szKinName then
		szKinName = string.format("[FF69B4]［帮派］%s[-]", szKinName or "-")
