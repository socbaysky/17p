...
function tbFuben:OnJoin(pPlayer)
	...
	if self.nProcess == Wedding.PROCESS_MARRYCEREMONY then
        ...
        pPlayer.CenterMsg("正处於拜堂期间，稍安勿躁", true)
	end
...
function tbFuben:OnDialogYueLao(pPlayer, pYueLao)
	...
	if self.nProcess == Wedding.PROCESS_WELCOME and bRole then
		Dialog:Show(
        {
            Text = "宾客若都到齐了，可以提前进入下个环节。",
            OptList = {
                { Text = "开始下个环节", Callback = self.JumpOverWelcome, Param = {self, me.dwID} };
            },
        }, pPlayer, pYueLao)
    elseif self.nProcess == Wedding.PROCESS_STARTWEDDING and bRole then
    	Dialog:Show(
        {
            Text = "新人都准备好了吗？",
            OptList = {
                { Text = "准备好了，开始婚礼", Callback = self.BeginWedding, Param = {self, me.dwID} };
            },
        }, pPlayer, pYueLao)
    else
    	Dialog:Show(
        {
            Text = "天生才子佳人配，只羡鸳鸯不羡仙！",
            OptList = {},
        }, pPlayer, pYueLao)
	end
end
...
function tbFuben:JumpOverWelcome(dwID)
	...
	if pNotifyer then
		if not self:CheckJumpOverWelcomeTime(pNotifyer) then
			pPlayer.CenterMsg("对方正在申请跳过", true)
			return
		end
		...
		local fnRefuse = function (nPlayerId)
			...
			if pRequester then
				...
				pRequester.CenterMsg("你的伴侣似乎还没准备好，拒绝进入下个阶段", true)
		...
		local szTip = string.format("宾客似乎都到齐了，[FFFE0D]%s[-]申请了进入下个阶段，是否同意？", pPlayer.szName)
		pNotifyer.MsgBox(szTip .. "\n（%d秒後自动同意）", {{"同意", fnAgree, pPlayer.dwID}, {"拒绝", fnRefuse, pPlayer.dwID}}, nil, Wedding.nRequestJumpWelcomeTime, fnJump)
		pPlayer.MsgBox("等待伴侣确认：%d", {{"确定", fnClose}}, nil, Wedding.nRequestJumpWelcomeTime, function () 
			me.CallClientScript("Ui:CloseWindow", "MessageBox")
		 end)
...
function tbFuben:ClearApplyData(pClear)
	if not self.tbRole[pClear.dwID] then
		pClear.CenterMsg("你无权操作", true)
		return
	end
	if not next(self.tbApplyWelcome) then
		pClear.CenterMsg("没有可清理的申请资讯", true)
		return
	end
	self.tbApplyWelcome = {}
	...
	if not tbPlayer or #tbPlayer <= 0 then
  	...
  	if nWelcomeCount < #tbPlayer then
		...
		local nCost = self.tbMapSetting.nInviteCost * nLack
  		pPlayer.MsgBox(string.format("你的请柬数量不足，是否花费[FFFE0D]%d元宝[-]购买[FFFE0D]%d张请柬[-]，并且发送邀请？", nCost, nLack), 
  			{{"确定", self.DoBuyWelcome, self, pPlayer.dwID, nCost, nLack, tbPlayer, nType, nInviteId}, {"取消"}}, "WeddingSendWeldome_Once")
  		return
  	end
		...
		if self.tbWelcomeCount[pPlayer.dwID] > 0 then
			...
			if pWelcomer and not self.tbHadWelcome[pWelcomer.dwID] then
				...
				local tbMail = {
					...
					Title = "婚礼邀请";
					From = pPlayer.szName;
					Text = string.format("    我即将与「%s」举行[FFFE0D]%s[-]婚礼，快来祝福我们吧！[FFFE0D]请柬已经送到你的背包中，使用後可以前来参加我们的婚礼。[-]", szLoveName, self.tbMapSetting.szWeddingName or "婚礼");
					tbAttach = tbAward;
					nLogReazon = Env.LogWay_Wedding;
				};
				Mail:SendSystemMail(tbMail);
				if not bOneKey then
					pPlayer.CenterMsg(string.format("已经给「%s」发送请柬", pWelcomer.szName), true)
				end
...
function tbFuben:DoBuyWelcome(dwID, nCost, nAddWelcome, tbPlayer, nType, nInviteId)
	...
	if not nInviteId then
		...
		if not tbSurePlayer or #tbSurePlayer ~= #tbPlayer then
			Wedding:SynWelcome(pPlayer)
			pPlayer.CenterMsg("成员发生变动，请重新邀请", true)
			return
		end
	else
		-- 单独邀请
...
function tbFuben:ApplyWelcome(pPlayer)
	if self.tbRole[pPlayer.dwID] then
		pPlayer.CenterMsg("主人家不能申请")
		return
	end
	if self.tbHadWelcome[pPlayer.dwID] then
		pPlayer.CenterMsg("你已经收到请柬了，请查看背包")
		return
	end
	if self.tbApplyWelcome[pPlayer.dwID] then
		pPlayer.CenterMsg("你已经申请过了")
		return
	end
	local nNowTime = GetTime()
	...
	pPlayer.CenterMsg(string.format("成功申请参加「%s」和「%s」的婚礼，请等待确认", self:ManName(), self:FemaleName()), true)
...
function tbFuben:CheckApplyWelcome(pPlayer, nWelcomeId)
	...
	if self.tbRole[nWelcomeId] then
		return false, "不能邀请主人家"
	end
	if self.tbHadWelcome[nWelcomeId] then
		return false, "已经邀请过玩家"
	end
...
function tbFuben:BeginWedding(dwID)
	...
	if self.nProcess ~= Wedding.PROCESS_STARTWEDDING then
		pPlayer.CenterMsg("婚礼已经开始", true)
		return
	end

	if not self:CheckStartWeddingTime(pPlayer) then
		pPlayer.CenterMsg("你已经请求了开始", true)
		return
	end
	local nNotifyId = self:GetOtherPlayerId(dwID) or 0
	local pNotifyer = KPlayer.GetPlayerObjById(nNotifyId)
	if pNotifyer then
		...
		local szTip = string.format("[FFFE0D]%s[-]已经准备好了，你若也准备好了即可开始婚礼。", pPlayer.szName)
		pNotifyer.MsgBox(szTip .. "\n（%d秒後自动同意）", {{"开始婚礼", fnAgree, pPlayer.dwID}, {"拒绝", fnRefuse, pPlayer.dwID}}, nil, Wedding.nRequestStartWeddingTime, fnStart)
		pPlayer.MsgBox("等待伴侣确认：%d", {{"确定", fnClose}}, nil, Wedding.nRequestStartWeddingTime, function () 
			me.CallClientScript("Ui:CloseWindow", "MessageBox")
		 end)
...
function tbFuben:TryPromise(dwID, szMsg, bSystem)
	...
	if not self.tbRole[dwID] then
		if pPlayer then
			pPlayer.CenterMsg("您无权宣誓", true)
		end
		return
	end
	if self.tbPromise[dwID] then
		if pPlayer then
			pPlayer.CenterMsg("您已经宣过誓", true)
		end
		return
	end
	if self.nProcess ~= Wedding.PROCESS_PROMISE then
		if pPlayer then
			pPlayer.CenterMsg("宣誓阶段已经结束", true)
		end
		return 
	end
	if not bSystem then
		if version_kor then
			...
			if nKorLen > Wedding.nKorPromiseMax or nKorLen < Wedding.nKorPromiseMin then
				if pPlayer then
					pPlayer.CenterMsg(string.format("誓言长度需要在%d~%d个汉字内", Wedding.nVNPromiseMin, Wedding.nVNPromiseMax), true)
				end
				return 
			end
		...
		elseif version_th then
			...
			if nNameLen > Wedding.nTHPromiseMax or nNameLen < Wedding.nTHPromiseMin then
				if pPlayer then
					pPlayer.CenterMsg(string.format("誓言长度需要在%d~%d个汉字内", Wedding.nTHPromiseMin, Wedding.nTHPromiseMax), true)
				end
				return 
			end
		else
			...
			if nNameLen > Wedding.nPromiseMax or nNameLen < Wedding.nPromiseMin then
				if pPlayer then
					pPlayer.CenterMsg(string.format("誓言长度需要在%d~%d个汉字内", Wedding.nPromiseMin, Wedding.nPromiseMax), true)
				end
				return
			end
			if Lib:HasNonChineseChars(szMsg) then
				if pPlayer then
					pPlayer.CenterMsg("誓言只能使用中文字元，请修改後重试！", true)
				end
		        return
		    end
		end
		if ReplaceLimitWords(szMsg) then
			if pPlayer then
				pPlayer.CenterMsg("誓言含有敏感字元，请修改後重试！", true)
			end
			return
		end
	end
	...
	local szMeTitle = self.nBoyPlayerId == dwID and "新郎" or "新娘"
	local szMeName = self.nBoyPlayerId == dwID and self:ManName() or self:FemaleName()
	local szOtherTitle = self.nBoyPlayerId == dwID and "新娘" or "新郎"
...
function tbFuben:OnAddConcentricFruit(nX, nY)
	...
	if pNpc then
		...
		self:BlackMsg(string.format("他们能在%d秒内同心食得同心果吗？为他们加油！", Wedding.nConcentricFruitHitTime))
	else
...
function tbFuben:ConcentricFruitSuccess(pNpc, bDefault)
	...
	for dwID in pairs(self.tbRole) do
		local pPlayer = KPlayer.GetPlayerObjById(dwID)
		if pPlayer then
			pPlayer.CenterMsg("恭喜获得同心果，你们真是天造地设的一对啊！", true)
...
function tbFuben:TryBless(pPlayer, nIdx)
	...
	if not tbBlessMsg then
		pPlayer.CenterMsg("请选择系统给出的祝福语", true)
		return
	end
...
function tbFuben:OpenCashPanel(pPlayer)
	if self.nProcess == Wedding.PROCESS_END or self.bClose == 1 then
		pPlayer.CenterMsg("婚礼已经结束，无法赠送礼金")
		return
	end
