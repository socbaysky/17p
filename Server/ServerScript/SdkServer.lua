...
function Sdk:OnLogin(pPlayer)
	...
	if Sdk:IsMsdk() then
		AssistClient:QueryFirstRegistInfo(pPlayer);
	end
end
...
function Sdk:OnQQLuckyBagRsp(nPlayerId, szRetData, szBagType)
	...
	local szContent = string.format("给QQ好友分享宝箱，即可领取元宝。[url=openwnd:点击分享宝箱,RedPaperSharePanel,\"%s\"]", szUrl);
	local tbMail = {
			To = nPlayerId;
			Title = "恭喜你获得元宝宝箱";
...
Sdk.tbNotifyPlatAllSetting = {
	...
	[1] = { -- android
		message = [[{"content":"%s","title":"剑侠情缘", "vibrate":1}]];
		tbPrarams = {
...
function Sdk:OnWeixinLuckyBagRsp(nPlayerId, szRetData, szBagType)
	...
	local tbBagInfo = Sdk.Def.tbWeixinLuckBagSetting[szBagType];
	local szContent = string.format("你现在可以给微信好友发送[FFFE0D]福袋[-]了。[url=openwnd:点击发放福袋,RedPaperSharePanel,\"%s\"]", szUrl or "http://g.cn");
	local tbMail = {
			To = nPlayerId;
			Title = "微信福袋";
...
function Sdk:SendBindPhoneReward()
	if not Sdk:IsEfunHKTW() and not version_kor then
		return false, "本渠道没有绑定手机奖励";
	end

	if Sdk:IsPhoneBinded(me) then
		return false, "您已经领取过绑定手机奖励了";
	end
...
function Sdk:RequestAddQQFriend(dwFriendId)
	...
	if not pFriend then
		return false, "好友不线上，不可发起请求";
	end

	if not Sdk:IsQQAddFriendAvailable(pFriend) then
		return false, "好友设置无法添加QQ好友";
	end
	...
	if Sdk.tbCachePlayerAddQQMapList[me.dwID][dwFriendId] == nToday then
		return false, "您今日已向该好友发送申请，请择日再发";
	end
...
function Sdk:TLogQQInfo(tbPlayerInfo, nPageId, nObjId, nOperateId, szTarget, nParam1)
	local szGameAppid, nPlat, nServerIdentity, nAreaId = GetWorldConfifParam();
	--print(nAreaId ~= Sdk.eServerAreaQQ, tbPlayerInfo, nPageId, nObjId, nOperateId, szTarget, nParam1)
	if nAreaId ~= Sdk.eServerAreaQQ then
		return;
	end
	...
	TLog("QQReportFlow",
		...
		nPlat,
		nServerIdentity,
		tbPlayerInfo.szAccount or "",
		tbPlayerInfo.dwID or "",
		nPageId, nObjId, 2, nOperateId, szTarget or "", nParam1 or "");

	-- Log("QQReportFlow",
	-- 	szGameAppid,
	-- 	nPlat,
	-- 	nServerIdentity,
	-- 	tbPlayerInfo.szAccount,
	-- 	tbPlayerInfo.dwID,
	-- 	nPageId, nObjId, 2, nOperateId, szTarget or "", nParam1 or "");
end

function Sdk:TLogMyQQInfo(nPageId, nObjId, nOperateId, szTarget, nParam1)
	self:TLogQQInfo(me, nPageId, nObjId, nOperateId, szTarget, nParam1);
	return true;
end
...
function Sdk:ReprotASMInfo(tbInfo)
	Log("ReprotASMInfo", me.dwID, tbInfo["iad-attribution"]);
	...
	TLog("ASMIadInfo",
		...
		nServerIdentity,
		me.szAccount,
		Lib:IsTrue(tbInfo["iad-attribution"]) and 1 or 0,
		tbInfo["iad-campaign-id"] or 0,
		tbInfo["iad-campaign-name"] or "NULL",
		tbInfo["iad-conversion-date"] or "",
		tbInfo["iad-click-date"] or "",
		tbInfo["iad-adgroup-id"] or 0,
		tbInfo["iad-org-name"] or "NULL",
		tbInfo["iad-adgroup-name"] or "NULL",
		tbInfo["iad-keyword"] or "NULL");
end

function Sdk:AskQQFriendsForInvite()
	local bOpenAct = Sdk:IsQQInviteFriendActOn();
	if not bOpenAct then
		return false, "不在活动期间";
	end

	AssistClient:QueryQQUnRegistFriends(me);
	return true;
end

function Sdk:OnQueryQQUnregistFriendRsp(pPlayer, tbFriendsInfo)
	pPlayer.CallClientScript("Sdk:OnSyncQQUnregistFriends", tbFriendsInfo);
end

function Sdk:InviteQQFriends(tbOpenIds)
	...
	if tbMsdkInfo.nPlatform ~= Sdk.ePlatform_QQ then
		return false, "请使用手Q进行登入";
	end

	if #tbOpenIds < 1 then
		return false, "你没有邀请好友";
	end

	for _, szOpenId in ipairs(tbOpenIds) do
		AssistClient:ShareInfo(me, szOpenId, "快来玩剑侠情缘", "二十年原班人马打造，多门派会战江湖，你不来试试？", "MSG_INVITE");
	end

	local nInvitedCount = Sdk:GetQQInviteFriendPlayerInfo(me);
	nInvitedCount = nInvitedCount + #tbOpenIds;
	me.SetUserValue(Sdk.Def.SDK_INFO_SAVEGROUP, Sdk.Def.SDK_INFO_QQ_FRIEND_INVITED_COUNT, nInvitedCount);
	me.CallClientScript("Sdk:OnQQInviteStateChanged");
	return true;
end
...
function Sdk:TakeInviteQQAward(nStep)
	...
	if nStep ~= (nAwardStep + 1) then
		return false, "请先领取上一级奖励";
	end

	local tbAwardInfo = Sdk.Def.tbQQInviteFriendAward[nStep];
	if not tbAwardInfo then
		return false, "没有对应等级的奖励";
	end

	if nInvitedCount < tbAwardInfo.nCount then
		return false, "未达到领取奖励条件";
	end

	me.SendAward(tbAwardInfo.tbAward, nil, nil, Env.LogWay_QQInviteUnRegFriends);
	me.SetUserValue(Sdk.Def.SDK_INFO_SAVEGROUP, Sdk.Def.SDK_INFO_QQ_FRIEND_INVITE_AWARDED, nStep);
	me.CallClientScript("Sdk:OnQQInviteStateChanged");
	return true;
end
...
function Sdk:UpdateTXPhoneBind()
	if not version_tx then
		Log("UpdateTXPhoneBind Fail By version");
		return false;
	end

	local nCurMonth = Lib:GetLocalMonth();
	me.SetUserValue(Sdk.Def.SDK_INFO_SAVEGROUP, Sdk.Def.SDK_INFO_BINDED_PHONE_TX, nCurMonth);
	return true;
end
...
local tbInterface = {
	...
	ReprotASMInfo            = true;
	TLogMyQQInfo             = true;
	InviteQQFriends          = true;
	TakeInviteQQAward        = true;
	AskQQFriendsForInvite    = true;
	UpdateTXPhoneBind        = true;
};
