local tbNpc = Npc:GetClass("IndifferGatherBook")
...
function tbNpc:OnDialog()
	...
	local bRet, _, szConfirmMsg = tbInst:CheckGatherSkillBook(me, him.nTemplateId) 
	if not bRet then
		return
	end
...
function tbNpc:EndProcess(nPlayerId, nNpcId)
	...
	local bRet = tbInst:GatherSkillBook(pPlayer, pNpc.nTemplateId)
	if not bRet then
		return
	end
