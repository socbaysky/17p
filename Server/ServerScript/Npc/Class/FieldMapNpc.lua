...
function tbFieldNpc:CalcDeathFixExp(pNpc, tbDamageInfo, nMaxPos, pKiller, bWithoutXiuLian)
    ...
    local nKillerTeamId = nil;
    if pKillerPlayer then
        nMainAttackID = pKillerPlayer.dwID;
        nKillerTeamId = pKillerPlayer.dwTeamID;
    end
            ...
            if tbPlayer and #tbPlayer > 0 then
                ...
                for _, tbPlayerInfo in pairs(tbPlayer) do
                    ...
                    if pPlayer.dwID == nMainAttackID then
                        ...
                        if pPlayer.dwTeamID and pPlayer.dwTeamID ~= 0 and pPlayer.dwTeamID == nKillerTeamId then
                            local pNpc = pPlayer.GetNpc()
                            nTeamAddExpP = pNpc.nEnhanceExpTeamP
                        end
                    end

                    nAddExp = nAddExp * (100 + nAddExpP + nTeamAddExpP) / 100;
                    if nAddExp > 0 then
        ...
        if tbDamage then
            ...
            if tbPlayer and #tbPlayer > 0 then
                ...
                for _, tbPlayerInfo in pairs(tbPlayer) do
                    ...
                    local nAddExp = nPlayerBaseExp * nBaseExp * (100 + nAddExpP + nTeamAddExpP) / 100;
                    if nAddExp > 0 then
                        if bWithoutXiuLian then
                            pPlayer.AddExperience(nAddExp, Env.LogWay_NpcDropAward);
                        else
