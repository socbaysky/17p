local tbNpc = Npc:GetClass("IndifferHorse")
...
function tbNpc:OnDialog()
	...
	if not tbInst:CheckGatherHorseEquip(me, him.nTemplateId) then
		return
	end
...
function tbNpc:EndProcess(nPlayerId, nNpcId)
	...
	local bRet = tbInst:GatherHorseEquip(pPlayer, pNpc.nTemplateId)
	if not bRet then
		return
	end
