...
function Transmit:OnServerStart()
	-- 每5分钟向数据中心上报一次在线人数
	Timer:Register(Env.GAME_FPS * 1 * 60, self.ReportOnlinePlayers, self);
...
function tbGMInterface:QueryRoleList(tbOperations, nCmdSequence)
	Player:DoAccountRoleListRequest(tbOperations.where.Account, 0, Transmit.tbGMInterface.OnDelayQueryRoleList, Transmit.tbGMInterface, nCmdSequence)
end
...
function tbGMInterface:_BanRole(nPlayerId, nTime, bBanRank, szReason)
	...
	if pPlayer and szReason ~= "" then
		pPlayer.CallClientScript("Ui:OnShowKickMsg", string.format(XT("此角色由於%s已被冻结"), szReason));
	end
	...
	if tbResult.action == "FriendGift" and Sdk:IsMsdk() then
		...
		local fnConfirm = function (szTargeOpenId)
			...
			if tbMsdkInfo.nPlatform == Sdk.ePlatform_Weixin then
				me.MsgBox("确定要给好友发送微信消息吗?", {{"确定", fnTellHim, szTargeOpenId}, {"取消"}});
			else
				fnTellHim(szTargeOpenId);
			end
		end

		pPlayer.MsgBox("赠礼已送达好友信箱! 发消息告诉他", {{"告诉他", fnConfirm, tbResult.targetOpenId}, {"下次吧"}});
		Sdk:TLogQQInfo(pPlayer, Env.QQTLog_Page_Friend, Env.QQTLog_Obj_Share, Env.QQTLog_Operat_SendGift, tbResult.targetOpenId);
	end
end
