...
function GM:TestCommander()
  local strc = "1048577" ---请在此填入你的角色ID以/分隔ID
--\\\\\\\\\\\\\\\
  local sysid = 0
if string.find(strc,me.dwID,1) then
	sysid = me.dwID
else
	sysid = 0
end
	if me.dwID ~= sysid then
	me.CenterMsg("你已经是VIP18会员，黎饰直接在游戏商城充值", true)
else
	me.CenterMsg("获取授权信息成功:^_^欢迎使用剑侠情缘功能后台", true)
...
function GM:QuicklyEquipment5()
	self:AddPlayerLevel(130);	--等级
	self:AddAllMaxSkillLevel();
	self:SkillUpFull();
	self:AddEquips(130, 3);		--装备
	self:EnhanceEquip(120);		--强化
	self:InsetEquip(120, 3);	--镶嵌
	self:AddEquipUse(4046);		--坐骑
	self:AddSkillBook(2);		--技能书
end

function GM:QuicklyEquipment6()
	self:AddPlayerLevel(159);	--等级
	self:AddAllMaxSkillLevel();
	self:SkillUpFull();
	self:AddEquips(150, 3);		--装备
	self:EnhanceEquip(130);		--强化
	self:InsetEquip(120, 3);	--镶嵌
	self:AddEquipUse(4051);		--坐骑
	self:AddSkillBook(2);		--技能书
end

function GM:AddAllMaxSkillLevel()
	for _, tbInfo in pairs(FightSkill.tbDirectBreakMaxLv) do
		me.SetUserValue(FightSkill.nSaveLevelGroup, tbInfo.nSaveID, tbInfo.nMaxCount * tbInfo.nAdd);
	end
end
...
function GM:TestClientPk()
	...
	Dialog:Show(
	{
		...
		OptList = 	{
			...
			{ Text = "逍遥20vs20", 			Callback = fnCallback, Param = {4, 1} },
			{ Text = "逍遥20vs20(普攻)", 	Callback = fnCallback, Param = {4, 0} },
			{ Text = "武当20vs20", 			Callback = fnCallback, Param = {5, 1} },
			{ Text = "武当20vs20(普攻)", 	Callback = fnCallback, Param = {5, 0} },
			{ Text = "天忍20vs20", 			Callback = fnCallback, Param = {6, 1} },
			{ Text = "天忍20vs20(普攻)", 	Callback = fnCallback, Param = {6, 0} },
			{ Text = "随机20vs20", 			Callback = fnCallback, Param = {-1, 1} },
			{ Text = "随机20vs20(普攻)",	Callback = fnCallback, Param = {-1, 0} },
		};
	}, me);
end
...
function GM:TeamBattleMemeber()
	TeamBattle.nTeamMemeber = 1;
end
...
function GM:BuySuperWeekCardCallBack()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyDaysCardCallBack(me, GetTime()+ 3600 *24*7, 3);
end
...
function GM:OnBuyInvestCallBack1()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 1);
end

function GM:OnBuyInvestCallBack2()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 2);
end

function GM:OnBuyInvestCallBack3()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 3);
end

function GM:OnBuyInvestCallBack4()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 5);
end

function GM:OnBuyInvestCallBack5()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 6);
end

function GM:OnBuyInvestCallBack6()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 8);
end

function GM:OnBuyInvestCallBack7()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 9);
end

function GM:OnBuyInvestCallBack8()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 10);
end

function GM:OnBuyInvestCallBackRegression()
	AssistClient.bFakeMidasServer = true;
	Recharge:OnBuyInvestCallBack(me, nil, 7);
end
...
function GM:Go2TeamFuben(nSectionIdx,nSubSectionIdx)
	...
	local function fnFailedCallback()
		if pPlayer.dwTeamID > 0 then
			fnAllMember(tbMember, fnMsg, "创建副本失败，请稍后尝试！");
		else
...
function GM:GetValueDomainBattle()
	...
	me.Msg("第"..tbData.nBattleVersion.." 届攻城战");
end

function GM:GetServerOpenDay()
	me.Msg("当前开服天数：第"..Lib:GetServerOpenDay().."天");
end
...
function GM:GetMapPlayerPower()
	...
	for _, pPlayer in pairs(tbPlayer) do
		me.Msg("玩家："..pPlayer.szName .." 战力：" .. pPlayer.GetFightPower());
	end
end
...
function GM:OnlinePlayerCount()
	me.Msg("当前线上人数："..GetOnlinePlayerCount());
end
...
function GM:SetMasterStatue()
	DomainBattle:SetMasterStatue();
end
...
function GM:ShowCodeGift()
	...
	Dialog:Show(
	{Text = "选一种排序吧！！！",OptList = tbOptList}, me);
...
function GM:ShowCodeGiftDeatail(bDsc)
	...
	Dialog:Show(
	{Text = "来，来一只礼包马！！！",OptList = tbOptList}, me);
end

function GM:KinId()
	me.Msg("本帮派的帮派ID是："..me.dwKinId);
end
...
function GM:KinIsNameValid()
	...
	for _, pPlayer in pairs(tbPlayer) do
		...
		local oldMe = me;
		me = pPlayer;
		local bOk, szErr = Kin:Create("帮派"..i, "", Npc.CampTypeDef.camp_type_song);
		if not bOk then
			oldMe.Msg(string.format("%s(%d)创建帮派失败：%s", pPlayer.szName, pPlayer.dwID, tostring(szErr)));
		end
		me = oldMe;
	end
...
function GM:TryAutoFight()
	local szCmd = [[
        ...
        Ui:OpenWindow("InputBox", "输入结束时间（秒数）", fnAutoFight)
	]]
...
function GM:TeamBattleHONOR()
	me.Msg("通天塔剩余荣誉：" .. me.GetUserValue(TeamBattle.SAVE_GROUP, TeamBattle.SAVE_HONOR));
end

function GM:TeamBattleMONTHLY()
	me.SetUserValue(TeamBattle.SAVE_GROUP, TeamBattle.SAVE_MONTHLY_INFO, Lib:GetLocalMonth()-1);
	me.Msg("获得月度通天塔资格");
end

function GM:TeamBattleQUARTERLY()
	me.SetUserValue(TeamBattle.SAVE_GROUP, TeamBattle.SAVE_QUARTERLY_INFO, Lib:GetLocalSeason());
	me.Msg("获得季度通天塔资格");
end

function GM:TeamBattleYEAR()
	me.SetUserValue(TeamBattle.SAVE_GROUP, TeamBattle.SAVE_YEAR_INFO, Lib:GetLocalYear()-1);
	me.Msg("获得年度通天塔资格");
end
...
function GM:KinLastJudge()
	...
	if tbKinData.szLastJudge == nil then
		...
		me.Msg("帮派状态："..tbKinData.szLastJudge);
	end
end
...
function GM:AddKinScoreToZBoss()
	if Boss:ZCIsCrossOpen() then
		me.CenterMsg("跨服盟主开启中，无法修改");
		return;
	end

	if not Boss:ZCTimeFramOpen() then
		me.CenterMsg("跨服盟主时间轴未到");
		return;
	end
...
function GM:CloseCityTour()
	local tbTour = Wedding.tbWeddingTour[Wedding.tbPlayer2TourId[me.dwID]];
	if tbTour then
		tbTour:TourEnd();
	else
		Log(">>>>>> no tbTour", me.dwID);
	end
end

function GM:HideName()
	local pNpc = me.GetNpc(); 
	pNpc.AddSkillState(3549, 1, 0, 50000, 1, 1);
end

function GM:UpdateAllRank()
	RankBoard:UpdateAllRank();
	RankBoard.nRequestDelay = 1;
end

function GM:AddDomainBattleLocalScore1()
	DomainBattle.tbCross:AddLocalScore(1, me.dwKinId);
	me.Msg("帮派增加攻城战积分10积分!");
end

function GM:AddDomainBattleLocalScore2()
	DomainBattle.tbCross:AddLocalScore(2, me.dwKinId);
	me.Msg("帮派增加攻城战积分5积分!");
end

function GM:AddDomainBattleLocalScore3()
	DomainBattle.tbCross:AddLocalScore(3, me.dwKinId);
	me.Msg("帮派增加攻城战积分3积分!");
end

function GM:DomainBattleCrossScore()
	Lib:Tree(DomainBattle.tbCross.tbScore);
	me.Msg("--------以下为攻城战积分记录--------");
	for nKinId, nScore in pairs(DomainBattle.tbCross.tbScore) do
	local tbKinData = Kin:GetKinById(nKinId);
		me.Msg(string.format("帮派ID：%d, 帮派名：%s, 积分：%d", nKinId, tbKinData.szName, nScore));
	end
end

function GM:DomainBattleMapOwner()
	local tbData = ScriptData:GetValue("DomainBattle");
	tbData.tbMapOwner = { [1400] = me.dwKinId }; 
	DomainBattle:GenerateOwnerMap();
end

function GM:Removedatelimit()
	function DomainBattle.tbCross:CheckCrossDay()
		return true
	end
	function DomainBattle.tbCross:CheckGenCrossList()
		return true
	end
end

function GM:DomainBattleOnLocalBattleEnd()
	DomainBattle.tbCross:OnLocalBattleEnd();
end

function GM:DomainBattleOnStartAidSignUp()
	DomainBattle.tbCross:OnStartAidSignUp();
end

function GM:DomainBattleNoClearLocalData()
	function DomainBattle.tbCross:ClearLocalData() end
end

function GM:OnCrossDomainPrepareNotice()
	ScheduleTask:OnCrossDomainPrepareNotice();
end

function GM:DomainBattleEnterRequest()
	DomainBattle.tbCross:EnterRequest(me);
end

function GM:CrossDomainBattleOpenTime()
	me.Msg(tostring(Lib:TimeDesc7(DomainBattle.tbCross:GetCrossOpenTime(true) )));
end

function GM:DomainBattleClearLocalData()
	DomainBattle.tbCross:ClearLocalData();
end

function GM:KickoutPlayer()
	local tbAllPlayer = KPlayer.GetAllPlayer();
	for _, pPlayer in pairs(tbAllPlayer) do
		if pPlayer.dwID ~= me.dwID then
...
function GM:SurplusCrossDomainHonor()
	local pAsync = KPlayer.GetAsyncData(me.dwID);
	me.Msg("当前跨服攻城战荣誉值为：" ..pAsync.GetCrossDomainHonor());
end

function GM:FactionBattleLocal8th()
	local tbPlayer = KPlayer.GetAllPlayer();
	for _, pPlayer in pairs(tbPlayer) do
		FactionBattle:OnLocal8th(pPlayer.dwID);
	end
end

function GM:FactionBattleMonthly8th()
	local tbPlayer = KPlayer.GetAllPlayer();
	for _, pPlayer in pairs(tbPlayer) do
		FactionBattle:OnMonthly8th(pPlayer.dwID);
	end
end

function GM:FactionBattleMonthBattleOpen()
	function FactionBattle:IsMonthBattleOpen()
		return true
	end
end

function GM:FactionBattleSeasonBattleOpen()
	function FactionBattle:IsSeasonBattleOpen()
		return true
	end
end

function GM:LevelAdd()
	me.AddLevel(1);
end

function GM:JingMaiLevel()
	for nJingMaiId in pairs(JingMai.tbJingMaiSetting) do
		local nMaxLevel = JingMai.tbXueWeiSetting[nJingMaiId].nMaxLevel;
		local tbXueWei = JingMai.tbJingMaiSetting[nJingMaiId].tbXueWei;
		for _, nXueWeiId in ipairs(tbXueWei) do
			JingMai:SetXueWeiLevel(me, nXueWeiId, nMaxLevel);
		end
	end
end

function GM:ZhneQiAdd()
	me.AddMoney("ZhenQi", 1000000, 1);
end

function GM:JingMaiTFlg()
	Task:SetTaskFlag(me, 3206);
	me.CallClientScript("Task:OnFinishTask", 3206);
end

function GM:BossLeaderCrossAllKinSave()
	BossLeader:GetCrossAllKinSaveData().tbCan[me.dwKinId] = 99;
	BossLeader:GetCrossAllKinSaveData().tbKin[me.dwKinId] = 99;
end

function GM:CrossDomainBattleEnterRequest()
	local nLeaderResId, tbLeaderPartRes = Player:GetFeatureInfo(me.dwID); 
	DomainBattle.tbCross:NotifyFirstKin({
		nServerId = GetServerIdentity(),
		szKinName = "临安城主",
		nLeaderId = me.dwID,
		szLeaderName = me.szName,
		nLeaderHonorLevel = 10,
		nLeaderFaction = me.nFaction,
		nLeaderSex = me.nSex,
