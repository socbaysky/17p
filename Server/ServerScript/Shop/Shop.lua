...
function Shop:BuyAfterCost(pPlayer, tbWare, nCount, nPrice, bRand, szMoneyType, nRequestIndex)
	...
	pPlayer.CallClientScript("Shop:OnBuyResponse", true, "购买成功", nRequestIndex + 1, tbWare.nGoodsId);
...
function Shop:Buy(pPlayer, szShopType, nTemplateId, nCount, nClientPrice, nRequestIndex)
	if pPlayer.nShopRequestIndex and nRequestIndex and pPlayer.nShopRequestIndex == nRequestIndex then
		pPlayer.CallClientScript("Shop:OnBuyResponse", false, "该笔交易已成功", pPlayer.nShopRequestIndex + 1);
		return
	end
	...
	if bRand then
	    ...
	    if self:IsNeedRefresh(nLastDay) then
	    	...
	    	pPlayer.CenterMsg("商品已经刷新，请重进介面");
	    	return;
	    end
	end
...
function Shop:BuyWarWare(pPlayer, nTemplateId, nCount, nRequestIndex)
	...
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("%s「%s」购买了%d个<%s>", Kin.Def.Career_Name[nCareer], pPlayer.szName, nCount, szItemName), pPlayer.dwKinId, 
		{nLinkType = ChatMgr.LinkType.Item, nTemplateId = nTemplateId, nFaction = pPlayer.nFaction, nSex = pPlayer.nSex});
	...
	pPlayer.CallClientScript("Shop:OnBuyResponse", true, string.format("你购买了%d个「%s」，已被自动存入帮派领地中。", nCount, szItemName) , (nRequestIndex or 0) + 1);
end
...
function Shop:GetLimitInfo(pPlayer, nLimitType)
	...
	if nLimitType == Shop.ACT_LIMIT_TYPE then
		...
		if math.abs((tbAllLimit.nActStartTime or 0) - nShopActStartTime) < 3600  then
...
function Shop:Sell(pPlayer, tbGoods, nRequestIndex)
	if pPlayer.nShopRequestIndex and nRequestIndex and pPlayer.nShopRequestIndex == nRequestIndex then
		pPlayer.CallClientScript("Shop:OnSellResponse", false, "该笔交易已成功", pPlayer.nShopRequestIndex + 1);
		return
	end
	local tbAfterCheck = {};
	...
	for i,v in ipairs(tbGoods) do
		...
		if not nSumPrice or not szMoneyType then
			pPlayer.CallClientScript("Shop:OnSellResponse", false, "无出售价格配置");
			return
		end
	...
	for i,v in ipairs(tbAfterCheck) do
		...
		pPlayer.AddMoney(v.szMoneyType, v.nSumPrice, Env.LogWay_ShopSell, v.nTemplateId);
		local _, szMoneyEmotion = self:GetMoneyName(v.szMoneyType);
		pPlayer.CenterMsg(string.format("获得了%s:%s", szMoneyEmotion, v.nSumPrice))
    ...
    for szShopType, _ in pairs(Shop.FamilyPool) do
    	...
    	if nBuildingId then
	        ...
	        if nLevel > nSaveLevel then
                ...
                if #tbNewShopData > 0 then
                	--因为池子刷新是有替换规则的，所以这里单独加上去掉重复同类商品
                    local tbCurItemId = {};
                    for i=#tbShopData.tbWares, 1, -1 do
                        local nTemplateId = tbShopData.tbWares[i].nTemplateId
                        if tbCurItemId[nTemplateId] then
                            table.remove(tbShopData.tbWares, i)
                            Log("Shop:CheckFamilyShop Same nTemplateId Replace Remove", nTemplateId, pPlayer.dwID)
                        end
                        tbCurItemId[nTemplateId] = 1;
                    end
                end
            end
...
function Shop:MakeShopWares(pPlayer, szShopType, nLevel, nEndLevel)
    local tbShopWares = {};
    local nTotalCount = 0;
    local tbForbitLevelPool = {}
    for nLevel = nLevel, nEndLevel, -1 do
        local tbLevelWares = self:MakeLevelWares(szShopType, nLevel, tbForbitLevelPool);
...
function Shop:MakeLevelWares(szShopType, nLevel, tbForbitLevelPool)
    ...
    for nPool, v in ipairs(tbPoolSetting) do
    	if self.tbReplacePool[nLevel] and self.tbReplacePool[nLevel][nPool] then
    		local ReplaceLevel, ReplacePool = unpack(self.tbReplacePool[nLevel][nPool])
    		tbForbitLevelPool[ReplaceLevel] = tbForbitLevelPool[ReplaceLevel] or {}
    		tbForbitLevelPool[ReplaceLevel][ReplacePool] = 1;
    	end

    	if not tbForbitLevelPool[nLevel] or not tbForbitLevelPool[nLevel][nPool] then
...
function Shop:MakeEquip(pPlayer, nId)
	...
	if nIdx==3 and nKinId>0 then
		...
		if tbKinData then
			...
			local szEquipName = KItem.GetItemShowInfo(nItemId, pPlayer.nFaction, pPlayer.nSex)
			local szSysMsg = string.format("「%s」在%s成功打造出了<%s>，真是鸿运当头！", pPlayer.szName, szBuildingName, szEquipName)
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szSysMsg, nKinId, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nItemId, nFaction = pPlayer.nFaction, nSex = pPlayer.nSex})
		end
	end
