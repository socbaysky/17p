	...
	if tbPos then
		if not ArenaBattle:CheckValidMap(pPlayer) then
			if pPlayer.dwTeamID > 0 then
				self:KickOutNotice(nCamp,string.format("「%s」不在比武场，自动退出队伍",pPlayer.szName),pPlayer.dwTeamID)
...
function ArenaBattleFight:SendAutoWinTip(nCamp)
	...
	local function fnAutoWinTip(self,pPlayer)
		pPlayer.CenterMsg("对手离开擂台，自动获胜。",true)
	end
...
function ArenaBattleFight:SendResultTip(nWinCamp)
	...
	local szTip = string.format("%d号擂台",tonumber(self.nArenaId))
	local nWinnerId
	...
	if pPlayer then
		if pPlayer.dwTeamID > 0 then
			szTip = szTip ..string.format("「%s」所带领的队伍获胜！",pPlayer.szName)
		else
			szTip = szTip ..string.format("「%s」获胜！",pPlayer.szName)
		end
	end
