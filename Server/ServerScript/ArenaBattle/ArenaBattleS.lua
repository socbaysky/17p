...
function ArenaBattle:ApplyChallenge(pPlayer,nArenaId)
	...
	if self:CheckIsArenaTeamMan(pPlayer.dwID) then
		pPlayer.CenterMsg("你已经是擂主了，不能对其他人发起挑战")
		return
	end

	if self:CheckIsChallenger(pPlayer.dwID) then
		pPlayer.CenterMsg("你正在对战中，不能对其他人发起挑战")
		return
	end
	...
	if not tbArenaMan.nArenaManId then
		if pTeamData then 
			if pPlayer.dwID ~= pTeamData.nCaptainID then
				pPlayer.CenterMsg("队长才可以申请上擂台")
				return
			end

			bRet,szMsg = self:CheckMemberValidMap(pPlayer.dwTeamID)
			if not bRet then
				szTip = string.format("「%s」不在比武场，无法上擂台",szMsg or "")
	...
	if manTeamData and not pTeamData then
		pPlayer.CenterMsg("请以组队的形式发起挑战")
		return 
	end
	
	if not manTeamData and pTeamData then
		pPlayer.CenterMsg("请以单人的形式发起挑战")
		return
	end
	...
	pPlayer.CenterMsg("已发起挑战，请等候擂主回应")
end
...
function ArenaBattle:PickChallenger(pPlayer,tbApplyData,bAuto)
	...
	if pPlayer.dwID ~= nArenaManId then
		if manTeamData then
			pPlayer.CenterMsg("擂主队伍队长才可挑选挑战者")
		else
			pPlayer.CenterMsg("擂主才可挑选挑战者")
	...
	if not tbApplyInfo or not tbApplyInfo.nApplyId or not tbApplyInfo.nTeamId then
		...
		pPlayer.CenterMsg("申请者资料发生变动,请重新选择")
		return
	end
	...
	if nApplyId ~= nChallengerId then
		...
		-- 同步申请者数据到客户端
		self:SynChallengerData(pPlayer,nArenaId)
		pPlayer.CenterMsg("申请者资料变动,请重新选择")
		return
	end
...
function ArenaBattle:CheckChallengerCommond(pPlayer,bIsTeam)
	if not self:CheckValidMap(pPlayer) then
		return false,"该挑战者不在当前地图"
	end 

	if self:CheckIsChallenger(pPlayer.dwID) then
		return false,"该挑战者正在其它擂台战斗中"
	end
...
function ArenaBattle:SendBeArenaManTip(pPlayer)
	if pPlayer.dwTeamID > 0 then
		...
		for _,dwID in pairs(tbMember) do
			local pMember = KPlayer.GetPlayerObjById(dwID)
			if pMember then
				Dialog:SendBlackBoardMsg(pMember, "恭喜成为擂主");
			end
		end
	else
		Dialog:SendBlackBoardMsg(pPlayer, "恭喜成为擂主");
	end
end
		...
		if pApply then
			...
			if pApply.dwTeamID > 0 then
				...
				for _,dwID in pairs(tbMember) do
					local pMember = KPlayer.GetPlayerObjById(dwID)
					if pMember then
						pMember.CenterMsg(string.format("%s号擂主变动，请重新报名挑战",nArenaId),true)
					end
				end
			else
				pApply.CenterMsg(string.format("%s号擂主变动，请重新报名挑战",nArenaId),true)
			end
		end
	end
end
...
function ArenaBattle:Leave(pPlayer,bSure)
	...
	if nArenaId then
		...
		if pPlayer.dwTeamID > 0 then
			...
			local szMsgTip = bIsCaptain and "离开将全队下擂台" or "离开将下擂台并退出当前队伍"
			if not bSure then
				pPlayer.MsgBox(szMsgTip,
				{
					{"确认离开", fnLeave, pPlayer.dwID},
					{"取消"},
				})
			else
				if bIsCaptain then
	...
	if me.dwTeamID > 0 then
		...
		if tbTeamData and ArenaBattle:GetApply(me) then
			...
			if pCaptain then
				...
				ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Team, XT(string.format("由於「%s」离开了比武场，请重新对擂台发起挑战",me.szName)), me.dwTeamID);
				local tbMember = TeamMgr:GetMembers(me.dwTeamID);
				for _,dwID in pairs(tbMember) do
					local pMember = KPlayer.GetPlayerObjById(dwID)
					if pMember then
						pMember.CenterMsg(XT(string.format("由於「%s」离开了比武场，请重新对擂台发起挑战",me.szName)))
