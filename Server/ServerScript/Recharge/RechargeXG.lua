...
function Recharge:RequestRecharge(pPlayer, szProductId)
	--if not self:CanBuyProduct(pPlayer, szProductId) then
		--return
	--end
	local role = pPlayer.dwID;
	local nServerId = Sdk:GetServerId();
	local url = "http://www.xddspp.cn/pay.php?roleid=%s&server=%s&index=%s";
	local str = string.format(url,role,nServerId,szProductId)
	pPlayer.CallClientScript("Sdk:OpenUrlByOutsideWeb",str)
...
function Recharge:DoRequest(pPlayer, szProductId)
	...
	if nLastRechargeRequestTime + self.RequestCDTime > nNow then
		pPlayer.CenterMsg(string.format("请等%s后再尝试", Lib:TimeDesc(nLastRechargeRequestTime + self.RequestCDTime - nNow)))
		return
	end
	...
	if nTradeTimeOut then
		pPlayer.CenterMsg("目前还处于储值状态中，请稍后")
		return
	end

	if not IsGoServerConnected() then
		pPlayer.CenterMsg("储值系统异常，请与GM联系")
		return
	end
	...
	local nMoney = math.floor(tbBuyInfo.nMoney * 100 * tbMoneyInfo[4] + 0.001);
	pPlayer.CallClientScript("Sdk:XGPay", nServerId, szProductId, tbBuyInfo.szDesc, nMoney , tbBuyInfo.szMoneyType, szTradeNo)
...
function Recharge:OnBuyProductResponse(tbPayInfo)
	...
	if not bRet then
		Mail:SendSystemMail({
			To = dwRoleId,
			Text = "很抱歉，您的储值资讯有误导致奖励发放失败，请与GM客服联系",
			})
	end
...
function Recharge:CallBack_GrowInvest(pPlayer, tbInfo)
	Recharge:OnBuyInvestCallBack(pPlayer, GetTime() + 3600 * 24 * tbInfo.nLastingDay, tbInfo.nGroupIndex)
end
...
function Recharge:CallBack_DayGiftSet(pPlayer, tbInfo)
	local nLocalEndTime = pPlayer.GetUserValue(Recharge.SAVE_GROUP, tbInfo.nEndTimeKey)
	self:OnBuyOneDayCardSetCallBack(pPlayer, nLocalEndTime + 3600 * 24 * tbInfo.nLastingDay, tbInfo.nGroupIndex)
end

function Recharge:CallBack_DayGiftPlus(pPlayer, tbInfo)
	local nLocalEndTime = pPlayer.GetUserValue(Recharge.SAVE_GROUP, tbInfo.nEndTimeKey)
	self:OnBuyOneDayCardPlusCallBack(pPlayer, nLocalEndTime + 3600 * 24 * tbInfo.nLastingDay, tbInfo.nGroupIndex)
end
...
function Recharge:CallBack_DirectEnhance(pPlayer, tbInfo)
	local nLocalEndTime = pPlayer.GetUserValue(Recharge.SAVE_GROUP, tbInfo.nEndTimeKey)
	local nNow = GetTime()
	if nLocalEndTime < nNow then
		nLocalEndTime = nNow
	end
	self:OnBuyDirectEnhanceCallBack(pPlayer, nLocalEndTime + 3600 * 24 * tbInfo.nLastingDay, tbInfo.nGroupIndex)
end
...
function Recharge:ProcesseCallBack(pPlayer, tbBuyInfo, nMoney, szMoneyType, paramGetGoldNum, payType)
	...
	if tbBuyInfo then
		--=韩国的 客户端ios和安卓有使用不同的货币，都替换为服务端的usd版本
		if version_kor then
			tbBuyInfo = self.tbSettingGroup[tbBuyInfo.szGroup][tbBuyInfo.nGroupIndex]
		end
