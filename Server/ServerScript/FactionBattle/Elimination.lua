...
function tbElimination:CheckWin(nWinner, nLoser)
	...
	if not nWinner and not nLoser then
		...
		if pPlayer1 and pPlayer2 then
			...
			if pNpc1 then
				tbDamage1 = self:GetNpcDamage(pNpc1)
			end
			if pNpc2 then
				tbDamage2 = self:GetNpcDamage(pNpc2)
			end
...
function tbElimination:SyncLeftInfo(pPlayer)
	...
	if pPlayer1 then
		...
		if pNpc then
			tbDamage1 = self:GetNpcDamage(pNpc)
		end
	end
	...
	if pPlayer2 then
		...
		if pNpc then
			tbDamage2 = self:GetNpcDamage(pNpc)
		end
	end
...
function tbElimination:GetNpcDamage(pNpc)
            local tbDmgInfo = pNpc.GetDamageCounter();
            local nNpcDmg = (tbDmgInfo.nDamage - tbDmgInfo.nPlayerDamage) * 0.1;

            return {nDamage = math.floor(nNpcDmg + tbDmgInfo.nPlayerDamage)}
end
