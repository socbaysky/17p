...
function FactionMonkey:ArrangeResult(tbMonkey)
	...
	for nFaction,tbPlayers in pairs(tbMonkey) do
		if tbPlayers[1] and next(tbPlayers[1]) then
			...
			tbPlayerInfo.nSex = pPlayerStay.nSex or 0
...
function FactionMonkey:MonkeyReward(tbResultMonkey)
	...
	for nFaction,tbPlayers in pairs(tbResultMonkey) do
		if next(tbPlayers) then
			local tbTitleId = FactionBattle.MONKEY_TITLE_ID[nFaction]
			if tbTitleId then
				...
				if pStayInfo then
					local nTitleId = tbTitleId[pStayInfo.nSex]
					if nTitleId then
						...
						local szTitle = FactionBattle.tbMailSetting[nFaction].szTitle or "门派竞选"
						local szMonkey = pStayInfo.nSex == Player.SEX_MALE and "大师兄" or "大师姐"
						local szFactionName = Faction:GetName(nFaction) or ""
						local szText = FactionBattle.tbMailSetting[nFaction].szText and string.format(FactionBattle.tbMailSetting[nFaction].szText, (szFactionName ..szMonkey)) or "门派竞选称号奖励"
						local tbMail = {
							To = nMonkeyId;
							Title = szTitle;
							From = "系统";
						...
						Log("FactionMonkey:MonkeyReward no nSexTitle " , nMonkeyId, nFaction, pStayInfo.nSex, nValidTime)
					end
				else
...
function FactionMonkey:OnSynData(pPlayer)
	...
	for index,tbPlayerInfo in pairs(tbData.tbMonkey) do
		...
		tbData.tbMonkey[index].szName = pPlayerStay.szName or XT("无")
...
function FactionMonkey:VoteMonkey(pPlayer,dwMonkeyID)
	...
	if not self:IsMonkeyStarting() or GetTime() > (tbMonkeyData.nStartTime + FactionBattle.MONKEY_VOTE_TIME * 60 * 60) then
		pPlayer.CenterMsg("投票时间已过，请下次再参与！")
