...
function BiWuZhaoQin:CreatePreMapLogic(nMapId)
    local tbTargetInfo = self.tbApplyPreMapID[nMapId];
    if not tbTargetInfo then
        Log("Error BiWuZhaoQin fnCreatePreMapLogic tbTargetInfo", nMapId);
        return;
    end
    local nTargetId, nServerId = unpack(tbTargetInfo)
    if not nTargetId or not nServerId then
        Log("Error BiWuZhaoQin fnCreatePreMapLogic valid info", nMapId, nTargetId, nServerId);
        return;
    end
    ...
    if tbPrLogic then
        Log("Error BiWuZhaoQin fnCreatePreMapLogic Exist tbPrLogic", nMapId, nTargetId, nServerId);
        return;
    end

    local tbFightInfo = BiWuZhaoQin:GetFightInfo(nTargetId, nServerId)
    if not tbFightInfo then
        Log("Error BiWuZhaoQin fnCreatePreMapLogic tbFightInfo", nMapId, nTargetId, nServerId);
        return;
    end

    tbPrLogic = Lib:NewClass(tbPreMapLogic);
    self.tbAllPreMapLogic[nMapId] = tbPrLogic;
    tbPrLogic:OnCreate(nMapId, nTargetId, nServerId);
    ...
    Log("BiWuZhaoQin fnCreatePreMapLogic PrLogic", nMapId, nTargetId, nServerId, tbFightInfo.bNpcType and "true" or "false");
end
...
function BiWuZhaoQin:ClosePreMapLogic(nMapId)
    ...
    local nServerId = tbPrLogic.nServerId

    BiWuZhaoQin:ClearServerPreMapInfo(nTargetId, nServerId)

    self.tbAllPreMapLogic[nMapId] = nil;
    Log("BiWuZhaoQin ClosePreMapLogic", nMapId, nServerId, nTargetId);
end

-- >>>>>>>>>>>>>>>>>>>> 准备场逻辑
function tbPreMapLogic:OnCreate(nMapId, nTargetId, nServerId)
    local tbFightInfo = BiWuZhaoQin:GetFightInfo(nTargetId, nServerId)
    if not tbFightInfo then
    ...
    self.tbZonePlayerIdRef = {}                     -- 原服玩家id和跨服玩家id的映射(由于该活动是单服活动，不需要考虑重复)
...
function tbPreMapLogic:GetLog()
    return self.nTargetId, self.nServerId, self.nConnectIdx, self.nMapId, self.nType, self.nFightCount, self.nKinId, self.szTimeFrame
end

function tbPreMapLogic:TryFirstFight()
    self.nFirstFightTimer = nil
    local tbFightInfo = BiWuZhaoQin:GetFightInfo(self.nTargetId, self.nServerId)
    if not tbFightInfo then
...
function tbPreMapLogic:UpdatePlayerUi(pPlayer,nState)
    ...
    local tbFightInfo = BiWuZhaoQin:GetFightInfo(self.nTargetId, self.nServerId) or {}
...
function tbPreMapLogic:GetFightState(pPlayer,tbFightInfo)
    ...
    if nProcess then
        local bNoJoin = self:GetPlayerWilling(pPlayer.dwOrgPlayerId)
        if nProcess == BiWuZhaoQin.Process_Pre then
            ...
            if tbFightInfo.tbFailPlayer and tbFightInfo.tbFailPlayer[pPlayer.dwOrgPlayerId] then
                nFightState = BiWuZhaoQin.tbFightState.Out
            else
                 -- 是否参加过
                local bHadJoin = tbFightInfo.tbPlayer and tbFightInfo.tbPlayer[1] and tbFightInfo.tbPlayer[1][pPlayer.dwOrgPlayerId]
                -- 是否进入了下一轮
                local bNextRound = tbFightInfo.tbPlayer and tbFightInfo.tbPlayer[tbFightInfo.nRound] and tbFightInfo.tbPlayer[tbFightInfo.nRound][pPlayer.dwOrgPlayerId]
                ...
                if nProcess == BiWuZhaoQin.Process_Final then
                    -- 是否是8强玩家
                    local bFinalPlayer = tbFightInfo.tbFinalPlayer and tbFightInfo.tbFinalPlayer[pPlayer.dwOrgPlayerId]
                    -- 第一次进入8强
                    ...
                    if bFinalPlayer then
                        -- 是否晋级下一轮8强
                        if bFirstFinal or tbFightInfo.tbPlayer and tbFightInfo.tbPlayer[tbFightInfo.nRound - 1] and tbFightInfo.tbPlayer[tbFightInfo.nRound - 1][pPlayer.dwOrgPlayerId] then
                            nFightState = BiWuZhaoQin.tbFightState.Next
                        else
                            nFightState = BiWuZhaoQin.tbFightState.Out
                        end
...
function tbPreMapLogic:TrySynFinalData(pPlayer)
    local tbFightInfo = BiWuZhaoQin:GetFightInfo(self.nTargetId, self.nServerId)
    if not tbFightInfo then
        Log("BiWuZhaoQin fnTrySynFinalData no tbFightInfo", pPlayer.dwOrgPlayerId, self:GetLog());
        return
    end
    ...
    for nArenaId = nStartArenaId,nFightCount do
        ...
        for nPos = 1,2 do
           ...
           if tbPlayerInfo then
                ...
                local pPlayer = self:GetPlayerObjById(tbPlayerInfo.dwID)
                -- 失去资格
                ...
                if tbFightLogic then
                    if #tbArenaInfo.tbPlayer == 2 then
                        ...
                        for nCamp,tbInfo in ipairs(tbArenaInfo.tbPlayer) do
                            -- 上面的判断已经确保玩家在线
                            local pPlayer = self:GetPlayerObjById(tbInfo.dwID)
...
function tbPreMapLogic:OnClose()
     Log("BiWuZhaoQin PreMapLogic OnClose",self.nMapId, self.nTargetId, self.nServerId);
end

function tbPreMapLogic:CacheJoinPlayerInfo(pPlayer)
    local pKinData = Kin:GetKinById(pPlayer.dwOrgKinId or 0) or {};
    self.tbJoinPlayerInfo[pPlayer.dwOrgPlayerId] = {
        szName = pPlayer.szName,
        nPlayerId = pPlayer.dwOrgPlayerId,
...
function tbPreMapLogic:GetPlayerObjById(dwOrgPlayerId)
    return KPlayer.GetPlayerObjById(self:GetZonePlayerId(dwOrgPlayerId))
end

function tbPreMapLogic:GetZonePlayerId(dwOrgPlayerId)
    return self.tbZonePlayerIdRef[dwOrgPlayerId] or -1
end

function tbPreMapLogic:OnEnter(pPlayer)
    self.tbZonePlayerIdRef[pPlayer.dwOrgPlayerId] = pPlayer.dwID
    local tbFightInfo = BiWuZhaoQin:GetFightInfo(self.nTargetId, self.nServerId)
    if not tbFightInfo then
    ...
    -- 招亲的人不能参加
    if pPlayer.dwOrgPlayerId ~= self.nTargetId and tbFightInfo.nJoin < BiWuZhaoQin.nMaxJoin and tbFightInfo.nProcess == BiWuZhaoQin.Process_Pre and not bNoJoin then
        -- 全服随意参加，家族得家族ID匹配上才能参加
        if tbFightInfo.nKinId == 0 or tbFightInfo.nKinId == pPlayer.dwOrgKinId then
            tbFightInfo.tbPlayer[tbFightInfo.nRound][pPlayer.dwOrgPlayerId] = GetTime()
            ...
            Log("BiWuZhaoQin sign up ",pPlayer.dwOrgPlayerId,pPlayer.szName,tbFightInfo.nJoin,self:GetLog())
        else
            Log("BiWuZhaoQin try sign up no match???",pPlayer.dwOrgPlayerId,pPlayer.szName,self:GetLog())
        end
    end
    self:UpdatePlayerUi(pPlayer)
...
function tbPreMapLogic:OnLeave(pPlayer)
    ...
    self:RemoveFromWaitList(pPlayer.dwOrgPlayerId)

    local tbFightInfo = BiWuZhaoQin:GetFightInfo(self.nTargetId, self.nServerId)
    if not tbFightInfo then
        return
    end
    self:ClearPlayerWilling(pPlayer.dwOrgPlayerId)
     if tbFightInfo.nProcess == BiWuZhaoQin.Process_Pre and tbFightInfo.tbPlayer[tbFightInfo.nRound][pPlayer.dwOrgPlayerId] then
        tbFightInfo.tbPlayer[tbFightInfo.nRound][pPlayer.dwOrgPlayerId] = nil
        tbFightInfo.nJoin = tbFightInfo.nJoin - 1
        CallZoneClientScript(tbFightInfo.nConnectIdx,"BiWuZhaoQin:OnSynJoinS",tbFightInfo.nTargetId,tbFightInfo.nJoin)
        Log("BiWuZhaoQin give up ",pPlayer.dwOrgPlayerId,pPlayer.szName,tbFightInfo.nJoin,self:GetLog())
    end
...
function tbPreMapLogic:OnPlayerTrapIn(pPlayer,nArenaId)
    self.tbWaitWatch[nArenaId][me.dwOrgPlayerId] = true
      -- 8强赛才会创建
    local tbArenaLogic = self:GetArenaLogic(nArenaId)
    if not tbArenaLogic then
        return
    ...
    if tbArenaLogic.bStart then
        ...
        for dwID,nCamp in pairs(tbPlayer) do
            local pFightPlayer = self:GetPlayerObjById(dwID)
...
function tbPreMapLogic:OnPlayerTrapOut(pPlayer,nArenaId)
    self.tbWaitWatch[nArenaId][me.dwOrgPlayerId] = nil
      -- 8强赛才会创建
    local tbArenaLogic = self:GetArenaLogic(nArenaId)
    if not tbArenaLogic then
        return
