...
function BWZQBattle:EndFightForce(pLoser)
	local nLostCame = self.tbPlayer[pLoser.dwOrgPlayerId]
...
function BWZQBattle:OnEnter(pPlayer)
	Log("BWZQBattle fnOnEnter ", pPlayer.dwOrgPlayerId, pPlayer.szName, self:GetLog())
	if not self.tbPlayer[pPlayer.dwOrgPlayerId] then
		self:RegisterEvent(pPlayer)
	end
	pPlayer.nBWZQFightMapId = self.nMapId
	self.tbPlayer[pPlayer.dwOrgPlayerId] = Lib:CountTB(self.tbPlayer) + 1
	...
	if self.nFightType then
		...
		elseif self.nFightType == BiWuZhaoQin.FIGHT_TYPE_ARENA then
			...
			if tbPreMapLogic then
				tbPreMapLogic:RemoveFromWaitList(pPlayer.dwOrgPlayerId)
			end
		end
	end
...
function BWZQBattle:ShowTeamInfo(nWinCamp)
	...
	local function fnGetPlayerInfo(self, pPlayer)
		local nCamp = self.tbPlayer[pPlayer.dwOrgPlayerId];
		if not nCamp then
			return 
		end
		...
		if nWinCamp then
			if self.tbDmgInfo[nCamp] then
				if self.tbDmgInfo[nCamp].tbPlayerDmg[pPlayer.dwOrgPlayerId] then
					nDamage = self.tbDmgInfo[nCamp].tbPlayerDmg[pPlayer.dwOrgPlayerId].nDamage;
				end
				if self.tbDmgInfo[nCamp].tbPlayerKillCount then
					nKillCount = self.tbDmgInfo[nCamp].tbPlayerKillCount[pPlayer.dwOrgPlayerId] or 0;
				end
			end
		end
...
function BWZQBattle:PlayerAvatar()
	local function fnAvatar(self,pPlayer)
	    ...
	    if not Player:ChangePlayer2Avatar(pPlayer, pPlayer.nFaction, tbAvatarInfo.nLevel, tbAvatarInfo.szEquipKey, tbAvatarInfo.szInsetKey, tbAvatarInfo.nStrengthLevel) then
	        Log("BiWuZhaoQin Error!!!!! In ChangePlayer2Avatar" .. pPlayer.dwOrgPlayerId .. self.szTimeFrame)
...
function BWZQBattle:StartFight()
	local function fnStartFight(self,pPlayer)
		...
		if nCamp then
			...
			Log("[BWZQBattle] fnStartFight player no camp",pPlayer.dwOrgPlayerId,pPlayer.szName,pPlayer.nMapTemplateId,self:GetLog())
		end
	end
...
function BWZQBattle:UpdatePlayerDmg(pPlayer)
	...
	self.tbDmgInfo[nCamp].tbPlayerDmg[pPlayer.dwOrgPlayerId] = self.tbDmgInfo[nCamp].tbPlayerDmg[pPlayer.dwOrgPlayerId] or {nDamage = 0};

	local tbLastCounter = self.tbDmgInfo[nCamp].tbPlayerDmg[pPlayer.dwOrgPlayerId];
	local tbCounter = pPlayer.GetNpc().GetDamageCounter();
	self.tbDmgInfo[nCamp].tbPlayerDmg[pPlayer.dwOrgPlayerId] = tbCounter;
...
function BWZQBattle:SyncPlayerLeftInfo(pPlayer)
	...
	pPlayer.CallClientScript("BiWuZhaoQin:SyncPlayerLeftInfo", self.tbPlayer[pPlayer.dwOrgPlayerId], tbDmgInfo);
end
...
function BWZQBattle:RegisterEvent(pPlayer)
	...
	self.tbRegisterInfo[pPlayer.dwOrgPlayerId] = {};
	self.tbRegisterInfo[pPlayer.dwOrgPlayerId].nOnDeathRegID = PlayerEvent:Register(pPlayer, "OnDeath", function(pKiller) self:OnPlayerDeath(pKiller); end);
end

function BWZQBattle:UnRegisterEvent(pPlayer)
	self.tbRegisterInfo = self.tbRegisterInfo or {};
	if self.tbRegisterInfo[pPlayer.dwOrgPlayerId] and self.tbRegisterInfo[pPlayer.dwOrgPlayerId].nOnDeathRegID then
		PlayerEvent:UnRegister(pPlayer, "OnDeath", self.tbRegisterInfo[pPlayer.dwOrgPlayerId].nOnDeathRegID);
	end
	self.tbRegisterInfo[pPlayer.dwOrgPlayerId] = nil;
end
...
function BWZQBattle:ClcResult()
	...
	for dwOrgPlayerId,nCamp in pairs(self.tbPlayer) do
		if nCamp == nWinCamp then
			nWinPlayerId = dwOrgPlayerId
		else
			nLostPlayerId = dwOrgPlayerId
		end
	end
	...
	local function fnResult(self,pPlayer)
		...
		local nResult = self.tbPlayer[pPlayer.dwOrgPlayerId] == nWinCamp and Env.LogRound_SUCCESS or Env.LogRound_FAIL;
		pPlayer.TLogRoundFlow(Env.LogWay_BiWuZhaoQin, pPlayer.nMapTemplateId, 0, nCostTime, nResult, 0, 0);
		if pPlayer.dwOrgPlayerId == nWinPlayerId then
			local szMsg = "恭喜您赢得了比赛，晋级下阶段！"
			pPlayer.Msg(szMsg)
			Dialog:SendBlackBoardMsg(pPlayer, szMsg)
		elseif pPlayer.dwOrgPlayerId == nLostPlayerId then
	...
	local function fnLeaveArena(self,tbId)
		local tbPreMapLogic = BiWuZhaoQin:GetPreMapLogic(self.nPreMapId)
        for nPos,dwOrgPlayerId in ipairs(tbId) do
            local pPlayer = tbPreMapLogic:GetPlayerObjById(dwOrgPlayerId)
            if pPlayer then
...
function BWZQBattle:OnPlayerDeath(pKiller)
	if not self.bStart then
		Log("[BWZQBattle] fnOnPlayerDeath no start",me.dwOrgPlayerId,me.szName,me.nMapTemplateId,self:GetLog())
		return
	end
	local nCamp = self.tbPlayer[me.dwOrgPlayerId]
	if not nCamp then
		Log("[BWZQBattle] fnOnPlayerDeath no camp",me.dwOrgPlayerId,me.szName,me.nMapTemplateId,self:GetLog())
		return
	end
	...
	if pKiller then
		...
		if pKillPlayer then
			...
			self.tbDmgInfo[nKillCamp].tbPlayerKillCount[pKillPlayer.dwOrgPlayerId] = (self.tbDmgInfo[nKillCamp].tbPlayerKillCount[pKillPlayer.dwOrgPlayerId] or 0) + 1;
			
			self.tbPlayerKill[pKillPlayer.dwOrgPlayerId] = (self.tbPlayerKill[pKillPlayer.dwOrgPlayerId] or 0) + 1
			
			nKillID = pKillPlayer.dwOrgPlayerId
			nKillName = pKillPlayer.szName
		end
	end

	self.tbPlayerKill[me.dwOrgPlayerId] = 0
	...
	Log("[BWZQBattle] OnPlayerDeath ",me.dwOrgPlayerId,me.szName,nKillID or -1,nKillName or "nil",nCamp,nKillCamp,self:GetLog())
	self:ClcResult();
end
...
function BWZQBattle:ForeachPlayer(fnFunc,...)
	local tbPreMapLogic = BiWuZhaoQin:GetPreMapLogic(self.nPreMapId)
	for dwOrgPlayerId,_ in pairs(self.tbPlayer) do
		local pPlayer = tbPreMapLogic:GetPlayerObjById(dwOrgPlayerId)
		if pPlayer then
...
function BWZQBattle:UpdatePlayerUi(pPlayer)
	local nCamp = self.tbPlayer[pPlayer.dwOrgPlayerId]
	if not nCamp then
		Log("[BWZQBattle] UpdatePlayerUi can not find camp",pPlayer.dwOrgPlayerId,pPlayer.dwTeamID,pPlayer.szName,pPlayer.nMapTemplateId,self.nArenaId)
		return
	end	
