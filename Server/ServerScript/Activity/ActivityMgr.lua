	...
	for i, tbInfo in ipairs(self.tbActivityPlan) do
		if self:CheckVersion(tbInfo) then
			...
			if #tbTimeFrameLimitGE == 2 and #tbTimeFrameLimitLE == 2 then
				...
				if tbTimeFrameLimitLE[2] == - 1 then
					nOpenTimeFrameTimeLE = nOpenTimeFrameTimeLE + 24*3600 - 1; --如果直接减一天中间就少了24小时,让配置的时间轴范围时间连续
				end
...
function Activity:IsRunning(szKeyName)
	self.tbRuningActivity = self.tbRuningActivity or {}
	return self.tbRuningActivity[szKeyName] and self.tbRuningActivity[szKeyName].bRunning==1
end
	...
	for szKeyName, tbActivity in pairs(tbRuningActivity or {}) do
		if tbActivity.bRunning and tbActivity.bRunning == 1 then
			...
			if tbActivity.tbInst and tbActivity.tbInst.GetCustomInfo then
				tbInfo.tbCustomInfo = tbActivity.tbInst:GetCustomInfo(pPlayer)
			end
...
function Activity:IsBeCloseBySameTypeAct(szActType, szActKeyName, nStartTime)
    ScriptData:AddDef(szActType)
    local tbData = ScriptData:GetValue(szActType)
    tbData.tbCloseKey = tbData.tbCloseKey or {}
    local value = tbData.tbCloseKey[szActKeyName]
    if value then
        local nCloseTime = tonumber(value)
        if not nCloseTime or nStartTime <= nCloseTime then
            Timer:Register(1, function ()
                Activity:EndActivity(szActKeyName)
            end)
            Log("Activity In CloseTime", szActType, szActKeyName, Lib:TimeDesc7(nCloseTime))
            return true
        end
    end
end

function Activity:AddTime2SameTypeAct(szActType, szActKeyName)
	local tbActivityData, tbRuningActivity = self:GetActivityData()
    local tbRunningAct = tbRuningActivity[szActKeyName]
    if not tbRunningAct then
        return
    end

    if self:IsBeCloseBySameTypeAct(szActType, szActKeyName, tbRunningAct.nStartTime) then
    	return
    end

    for szKeyName, tbInfo in pairs(tbRuningActivity) do
        if tbInfo.szType == szActType and szKeyName ~= szActKeyName and not self:IsBeCloseBySameTypeAct(tbInfo.szType, szKeyName, tbInfo.nStartTime) then
            tbInfo.nEndTime = tbInfo.nEndTime + (tbRunningAct.nEndTime - tbRunningAct.nStartTime)
            local tbData = ScriptData:GetValue(szActType)
            tbData.tbCloseKey = tbData.tbCloseKey or {}
            tbData.tbCloseKey[szActKeyName] = tbRunningAct.nEndTime
            ScriptData:AddModifyFlag(szActType)
            if tbActivityData[szKeyName] then
	            tbActivityData[szKeyName].nEndTime = tbInfo.nEndTime
	            self:SaveActivityData()
            end
			--如果是Init中调用次函数，那么这里直接关掉可能是会有问题的，所以加个定时器
			Timer:Register(1, function ()
                Activity:EndActivity(szActKeyName)
            end)
            Log("Activity AddTime2SameTypeAct", szActKeyName, Lib:TimeDesc7(tbRunningAct.nStartTime), Lib:TimeDesc7(tbRunningAct.nEndTime), szKeyName, Lib:TimeDesc7(tbInfo.nEndTime))
            return
        end
    end
    return true
