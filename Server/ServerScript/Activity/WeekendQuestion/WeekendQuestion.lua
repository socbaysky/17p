...
function tbAct:OnTrigger(szTrigger)
    if szTrigger == "Start" then
        ...
        Activity:RegisterNpcDialog(self, 99,  {Text = "五一江湖动灵机", Callback = self.OnNpcDialog, Param = {self}})
        Activity:RegisterNpcDialog(self, "WeekendQuestionNpc",  {Text = "五一江湖动灵机", Callback = self.OnQuestionDialog, Param = {self}})
...
function tbAct:CheckTeamMember(pP)
    ...
    for nIdx, nPlayerId in ipairs(tbTeamMember) do
        ...
        if pPlayer.nLevel < self.JOIN_LEVEL then
            return false, string.format("「%s」等级不足%d级", pPlayer.szName, self.JOIN_LEVEL)
        end
    ...
    if me.nSex == Gift.Sex.Boy then
        ...
        if not tbBoyData.bAnswered then
            if tbBoyData.nBeginTime > 0 and GetTime() - tbBoyData.nBeginTime >= self.TIME_OUT then
                ...
                me.CenterMsg("请等待队友完成答题")
                return
            end
        end
        if tbGirlData.bAnswered then
            me.CenterMsg("已完成回答，请开始下一轮答题")
            return
        end
...
function tbAct:TryAnswer(pPlayer,bRight)
    ...
    local tbBoyData = tbData.tbPlayerData[Gift.Sex.Boy]
    local tbGirlData = tbData.tbPlayerData[Gift.Sex.Girl]
    if pPlayer.nSex == Gift.Sex.Boy then
        if tbBoyData.bAnswered then
            pPlayer.CenterMsg("你已完成这轮答题")
            return
        end
        if tbBoyData.nBeginTime == 0 then
            pPlayer.CenterMsg("请先开始答题")
            return
        end
        ...
        if bRight then
            ChatMgr:SendTeamOrSysMsg(pPlayer, string.format("%s答对了题目，真是机智过人！",pPlayer.szName))
        end
        ...
        for _, nPlayerId in ipairs(tbData.tbPlayerId) do
            if nPlayerId ~= pPlayer.dwID then
                ...
                pGirl.CenterMsg("队友已经回答完毕，轮到你进行答题了")
                bRet,szMsg = self:CheckDistance()
                if not bRet then
        ...
        if not tbBoyData.bAnswered then
            if tbBoyData.nBeginTime > 0 and GetTime() - tbBoyData.nBeginTime >= self.TIME_OUT then
                ...
                pPlayer.CenterMsg("请等待队友完成答题")
                return
            end
        end
        if tbGirlData.bAnswered then
            pPlayer.CenterMsg("已完成回答，请开始下一轮答题")
            return
        end
