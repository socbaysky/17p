...
function tbFuben:SetPlayerPKFightInfo(pPlayer)
    pPlayer.Change2Avatar(pPlayer.nFaction, pPlayer.nLevel, pPlayer.nSex, 1);
...
function tbFuben:GetPlayerStateBuff(pPlayer)
    ...
    local nBuffID = tbTeamBuff[pPlayer.nSex];
    return nBuffID;
end
...
function tbFuben:DoWinTeam(nTeamIndex)
    ...
    for nPlayerID, tbPlayerInfo in pairs(tbTeamInfo.tbAllPlayer) do
        if not self.tbFeatchAwardPlayer[nPlayerID] then
            ...
            local _, dwOrgPlayerId = KPlayer.GetOrgPlayerIdByZoneId(nPlayerID)
            CallZoneClientScript(tbPlayerInfo.nZoneIndex, "Activity.tbDaXueZhang:SendPlayerMailAwardS", dwOrgPlayerId, tbDef.nWinType, tbPlayerInfo.nRank);
...
function tbFuben:DoFailTeam(nTeamIndex)
    ...
    for nPlayerID, tbPlayerInfo in pairs(tbTeamInfo.tbAllPlayer) do
        if not self.tbFeatchAwardPlayer[nPlayerID] then
            self.tbFeatchAwardPlayer[nPlayerID] = 1;
            local _, dwOrgPlayerId = KPlayer.GetOrgPlayerIdByZoneId(nPlayerID)
            CallZoneClientScript(tbPlayerInfo.nZoneIndex, "Activity.tbDaXueZhang:SendPlayerMailAwardS", dwOrgPlayerId, tbDef.nFailType, tbPlayerInfo.nRank);
