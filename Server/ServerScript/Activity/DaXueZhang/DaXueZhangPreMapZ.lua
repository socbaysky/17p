...
function tbDaXueZhang:SendSyncPlayerTeamZ(tbSyncPlayer)
    ...
    local tbChangeSyncPlayer = {};
    for dwOrgPlayerId, v in pairs(tbSyncPlayer) do
        local nNowPlayerId = KPlayer.ForceGetZonePlayerIdByOrgId(dwOrgPlayerId, nServerId)
        tbChangeSyncPlayer[nNowPlayerId] = v;
    end
    ...
    tbPrLogic:SetPlayerEnterTeam(tbChangeSyncPlayer);
end

function tbDaXueZhang:KickOutPlayerOnZoneZ(dwOrgPlayerId)
    local nConnectIdx = Server.nCurConnectIdx;
    local nServerId = Server:GetServerId(nConnectIdx)
    ...
    pPlayer.ZoneLogout();
    Log("DaXueZhang KickOutPlayerOnZoneZ", Server.nCurConnectIdx, pPlayer.nZoneIndex, dwOrgPlayerId, nServerId);    
end

function tbDaXueZhang:ReducePlayerCount(pPlayer, nCount)
    local nConnectIdx = pPlayer.nZoneIndex;
    
    CallZoneClientScript(nConnectIdx, "Activity.tbDaXueZhang:ReducePlayerCountS", pPlayer.dwOrgPlayerId, nCount);
end
