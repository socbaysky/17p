...
function tbAct:OnTrySendLabel(pPlayer, tbParam)
	...
	if not pAcceptPlayer then
		pPlayer.CenterMsg(string.format("需要[FFFE0D]双方均线上[-]才能够添加%s哦", self.szActDes), true)
		return
	end
...
function tbAct:CheckData(pPlayer, pAcceptPlayer, nType, szLabel)
	...
	local nHimSex = pAcceptPlayer.nSex;

	if nType == self.FreeLabel then
		if tbFreeSender[pPlayer.dwID] then
			return false, string.format("您已经对目标侠士添加过该%s，请选择其他%s", self.szActDes, self.szActDes)
		end

		if not self.bTSMode and nHimSex ~= Gift.Sex.Girl then
			return false, string.format("量产印象签只能为女性好友添加%s", self.szActDes)
		end

		if not bExistLabel and nLabelCount >= self.nMaxLabel then
			return false, string.format("对方的%s已满，请另外挑选一个侠士吧", self.szActDes)
		end
	elseif nType == self.PayLabel then
		...
		if tbPayLabelPlayer[szLabel] and tbPayLabelPlayer[szLabel][pPlayer.dwID] then
			return false, string.format("不能多次添加同一%s", self.szActDes)
		end
	end
...
function tbAct:TrySendLabel(nPlayerId, bSuccess, szBillNo, nAcceptId, nType, szLabel)
	...
	if nType == self.FreeLabel then
		...
		if nConsume < self.nNeedConsumeImpressionLabel then
			szMsg = string.format("扣除道具%s失败！",KItem.GetItemShowInfo(self.nImpressionLabelItemID, pPlayer.nFaction, pPlayer.nSex) or "-")
			pPlayer.CenterMsg(szMsg, true)
			return false, szMsg
		end
	end
	...
	local szMsg = string.format("「%s」对「%s」添加了新的%s「%s」", pPlayer.szName or "-", pAcceptPlayer.szName or "-", self.szActDes, szLabel)
	...
	pPlayer.CenterMsg(string.format("已成功对「%s」添加%s", pAcceptPlayer.szName or "-", self.szActDes), true)
	pAcceptPlayer.CenterMsg(string.format("「%s」对你添加了新的%s「%s」", pPlayer.szName or "-", self.szActDes, szLabel), true)
...
function tbAct:SendGlobalMail()
	...
	Log("[WomanAct] SendGlobalMail ok ", nStartTime, nEndTime)
end
