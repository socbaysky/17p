...
function tbAct:SendAwardEmail()
    ...
    local tbMail = {
        Title = "独孤剑的腊八贺信",
        Text = "    腊八佳节将至，多谢诸位近日为江湖惩戒奸邪！此行颇有所获，我等缴获了一大批腊八粥，正好赠予诸位侠士过节！只需在服下此粥，借助帮派烤火时的热力，便可获得[FFFE0D]额外的烤火经验加成[-]。粥的效力[FFFE0D]有效期23小时[-]，不必担心误服。\n    冬日佳节，惟愿君健康安好，平安而至。",
        LevelLimit = 20,
...
function tbAct:DoWish(nPlayerId, bSuccess, szBillNo, szMsg, nWishType)
    ...
    local szTip = string.format("%s在帮派许愿树上挂上自己的愿望：「%s」", pPlayer.szName, szMsg)
    if nWishType == self.Wish_Type_Free then
        if pPlayer.dwKinId > 0 then
            ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szTip, pPlayer.dwKinId);
        end
    elseif nWishType == self.Wish_Type_Pay then
        szTip = string.format("%s在帮派许愿树许下自订愿望「%s」", pPlayer.szName, szMsg)
        if pPlayer.dwKinId > 0 then
            ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szTip, pPlayer.dwKinId);
        end
...
function tbAct:SendEndAward()
    ...
    for nKinId, szSaveKey in pairs(tbMainData.tbKinDataSaveKey) do
        ...
        for nRank, tbInfo in ipairs(tbData[nKinId] or {}) do
            ...
            local szContent = string.format("恭喜侠士在帮派许愿活动中排名第%d，获得称号奖励", nRank)
            local tbMail = {Title = "许愿排名奖励", From = "帮派总管", nLogReazon = Env.LogWay_WishAct, Text = szContent, To = nPlayerId, tbAttach = tbAward}
