...
function Furniture:SyncMapFurniture(pPlayer, tbFurnitures)
	self.nVersion = ((self.nVersion or 0)+1)%10000000

	tbFurnitures = tbFurnitures or {}
	local tbData = {}
	local nCount = 0
	local bSend = false
	for nIdx, tbFurniture in pairs(tbFurnitures) do
		tbData[nIdx] = tbFurniture
		nCount = nCount+1
		if nCount%self.nSyncMapFurnitureBatch==0 then
			bSend = true
			pPlayer.CallClientScript("House:OnSyncMapFurniture", self.nVersion, tbData)
			tbData = {}
			nCount = 0
		end
	end
	if next(tbData) or not bSend then
		pPlayer.CallClientScript("House:OnSyncMapFurniture", self.nVersion, tbData)
		tbData = {}
	end
end
...
function Furniture:DoSyncFurniture(pPlayer)
	if House:IsInNormalHouse(pPlayer) then
		...
		self:SyncMapFurniture(pPlayer, tbMapFurniture)
	end
...
function Furniture:CheckCanChangePos(pPlayer, nId, nX, nY, nRotation)
	...
	if not House:HasDecorationAccess(pPlayer) then
		return false, "只能装扮自己的家园！", tbFurniture;
	end
...
function Furniture:AddMapFurnitureByTB(pPlayer, tbSetting)
	...
	if not bRet then
		Log("[ERROR][Furniture] AddMapFurnitureByTB failed !! add furniture error", nPlayerId, pPlayer.szAccount, pPlayer.szName, nX, nY, nRotation, nTemplateId);
		return false, szMsg;
	end
	...
	if not bRet then
		Log("[ERROR][Furniture] AddMapFurnitureByTB failed !! create decoration error", nPlayerId, pPlayer.szAccount, pPlayer.szName, nX, nY, nRotation, nTemplateId);
		return false, szMsg;
	end
...
function Furniture:MakeFurniture(pPlayer, nFurnitureItemId)
	...
	if tbMakeInfo.nKinMsg > 0 and pPlayer.dwKinId > 0 then
		...
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, szKinMsg, pPlayer.dwKinId, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nFurnitureItemId, nFaction = pPlayer.nFaction, nSex = pPlayer.nSex});
	end
	...
	if tbMakeInfo.nWorldMsg > 0 then
		...
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.System, szWorldMsg, nil, {nLinkType = ChatMgr.LinkType.Item, nTemplateId = nFurnitureItemId, nFaction = pPlayer.nFaction, nSex = pPlayer.nSex});
	end

	self:OnMakeFurniture(pPlayer, nFurnitureItemId)
	Log("[Furniture] FurnitureMake Finish !", pPlayer.dwID, pPlayer.szAccount, pPlayer.szName, nFurnitureItemId);
end

function Furniture:OnMakeFurniture(pPlayer, nFurnitureItemId)
	self:Add(pPlayer, nFurnitureItemId)
	pPlayer.CallClientScript("House:OnSyncMakeFurniture", nFurnitureItemId, true)

	self:DoOnMakeSpecialTask(pPlayer, nFurnitureItemId)

	pPlayer.TLog("FurnitureFlow", nFurnitureItemId, 1)
end

function Furniture:DoOnMakeSpecialTask(pPlayer, nFurnitureItemId)
	local tbFurniture = House:GetFurnitureInfo(nFurnitureItemId)
	if not tbFurniture then
		Log("[x] Furniture:DoOnMakeSpecialTask, no info", pPlayer.dwID, nFurnitureItemId)
		return
	end

	local tbSetting = Furniture:GetTypeSetting(tbFurniture.nType)
	if not tbSetting then
		Log("[x] Furniture:DoOnMakeSpecialTask, no type", pPlayer.dwID, nFurnitureItemId, tbFurniture.nType)
		return
	end

	local szKey = tbSetting.szKey
	if szKey == "TYPE_MAGIC_BOWL" then
		self:DoOnMakeMagicBowlSpecialTask(pPlayer, nFurnitureItemId)
		return
	end
end

function Furniture:DoOnMakeMagicBowlSpecialTask(pPlayer, nFurnitureItemId)
	local nPlayerId = pPlayer.dwID
	...
	if next(tbHouse.tbMagicBowl or {}) then
		Log("[x] Furniture:DoOnMakeMagicBowlSpecialTask, exists", nPlayerId, nFurnitureItemId, tbHouse.tbMagicBowl.nLevel)
		return
	end

	tbHouse.tbMagicBowl = {
		nVersion = MathRandom(1000, 99999),
		nLevel = 1,
		tbAttrs = {},
		tbInscription = {
			nStage = 0,
	...
	Log("Furniture:DoOnMakeMagicBowlSpecialTask", nPlayerId, nFurnitureItemId)
end

function Furniture:OnLogin()
	self:DoSyncFurniture(me);
	self.MagicBowl:OnLogin(me)
end

function Furniture:OnReConnect()
	self:DoSyncFurniture(me);
	self.MagicBowl:UpdateAttrs(me, true)
end
...
function Furniture:SellFurniture(pPlayer, nTemplateId, nCount)
	if not Env:CheckSystemSwitch(pPlayer, Env.SW_SellFurniture) then
		pPlayer.CenterMsg("家俱出售暂时关闭");
		return;
	end

	local nCount = math.floor(nCount);
	if nCount <= 0 then
		pPlayer.CenterMsg("家俱数量不正确！");
		return;
	end

	if not Furniture:CanSell(nTemplateId) then
		pPlayer.CenterMsg("此家俱不支持售卖");
		return;
	end
