...
function DomainBattle:CheckRandItemAuctionPrice( tbAwardSetting )
	local tbChechDed= {}
	for _,v in ipairs(tbAwardSetting) do
		for _,v2 in ipairs(v[2]) do
			local nItemId = v2[1]
			if not tbChechDed[nItemId] and v2[4] then
				tbChechDed[nItemId] = true
				...
				if szClass == "RandomItemByTimeFrame" then
					nRandParamId = Item:GetClass(szClass):GetRandomKindId(nParamId);
				elseif szClass == "RandomItemByLevel" then
					nRandParamId = self:GetRandomKindId(GetMaxLevel(), nParamId);
				elseif szClass == "RandomItem" then
					nRandParamId = nParamId
				end
				if nRandParamId then
					local tbAllAward = Item:GetClass("RandomItem"):GetAllAward(nRandParamId)
					if tbAllAward then
						for _,tbItem in ipairs(tbAllAward) do
							if tbItem.nItem and tbItem.nItem > 0 and tbItem.szOtherType == "" then		
								--有对应的拍卖
								assert(Kin:GetAuctionItemPrice(tbItem.nItem), tbItem.nItem)
							end
						end
...
function DomainBattle:CheckDataSetting()
	self:CheckRandItemAuctionPrice(DomainBattle.define.tbActOwnerItemSetting)
	self:CheckRandItemAuctionPrice(DomainBattle.define.tbFlagAwardSetting)
end
...
function DomainBattle:SetMasterStatue()
	local tbOldnStatueNpcId = self.tbStatueNpcId
	if tbOldnStatueNpcId then
		for i,nOldNpcId in ipairs(tbOldnStatueNpcId) do
			local pOldNpc = KNpc.GetById(nOldNpcId)
			if pOldNpc then
				pOldNpc.Delete();
			end
		end
		self.tbStatueNpcId = nil;
	end
	...
	if nMasterId and nMasterId ~= 0 then
		...
		local tbStatusId = define.tbMasterStatueId[pRole.nFaction]
		if not tbStatusId then
			Log(debug.traceback())
			return
		end
		local nStatusId = tbStatusId[pRole.nSex]
		self.tbStatueNpcId = {}
		for i,v in ipairs(define.tbMasterStatuePos) do
			local nMapId, nX, nY, nDir = unpack(v)
			...
			table.insert(self.tbStatueNpcId, pNpc.nId)
		end
	end
end
...
function DomainBattle:StartDeclareWar()
	...
	KPlayer.SendWorldNotify(self.define.nMinLevel, 999, "今日攻城战已开始接受宣战！请谨慎宣战，开战後只能攻打宣战目标。", 1, 1)
...
function DomainBattle:NotifyKinDomainWar()
	...
	local fnNotifyKin = function (kinData, timeVal)
		...
		if not self:GetKinWarDeclare()[nKinId] then
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("攻城战将在%d分钟後开始，本帮派尚未宣战，请堂主或副堂主尽快去领地介面宣战！", math.ceil(timeVal / 60) ), nKinId);
		end
	end
...
function DomainBattle:NotifyEndDomainWar()
	KPlayer.SendWorldNotify(self.define.nMinLevel, 999, "今日攻城战将于10分钟後开始，请未宣战的帮派尽快宣战，开战後只能攻打宣战目标。", 1, 1)
end
...
function DomainBattle:StartActivity()
	...
	local tbMapAttakKin = {};
	self.tbOldOwnerMap = Lib:CopyTB(tbOwnerMap)
		...
		for i, v in ipairs(tbRankAward) do
			...
			for nRank = nLastRankEnd + 1, v.nRankEnd do
				...
				Mail:SendSystemMail(
				{
					To 	  		= dwRoleId,
					Title 		= "攻城战奖励",
					Text        = string.format("今日攻城战结束了，你的征战积分帮派排名第%d，奖励请领取附件", nRank),
					tbAttach 	= v.Award,
...
function DomainBattle:SendKinAward()
	...
	local tbGateAddScore = {}; --宣战这个地图且有领地的， 门加的积分
	local tbKinAddScore = {}; --都是有领地且没有打下自动宣战领地的家族  打下龙柱或门额外加的分
	local tbFlagAddScore = self.define.tbFlagAddScore
	for nMapTemplateId, nMapId in pairs(self.tbAllFifhtMapId) do --tbFifhtMapId 已经修改了
		local tbInst = self.tbMapInst[nMapId]
		local nMapLevel = DomainBattle:GetMapLevel(nMapTemplateId)
		local nGateFullScore = DomainBattle.define.tbGateScore[nMapLevel]	
		if nGateFullScore then
			local nGateAddScore = 0
			local tbCackeAttackInfo = tbInst:GetCacheAttackInfo(true)
			for nDoorIndex, nGateHpPercent in ipairs(tbCackeAttackInfo.tbGateState) do
				nGateAddScore = nGateAddScore + (1 - nGateHpPercent) * nGateFullScore	
			end
			tbGateAddScore[nMapTemplateId] = nGateAddScore
			Log("DomainBattle nGateAddScore", nMapTemplateId, nGateAddScore)
		end
		local tbFlogNpcPos = self.tbMapPosSetting[nMapTemplateId].tbFlogNpcPos
		for nIndex, v in ipairs(tbInst.tbFlagState) do
			local dwKinId = v[1]
			local nNpcTemplate, nFlagLevel = unpack(tbFlogNpcPos[nIndex]) 
			if self.tbOldOwnerMap[dwKinId] and tbOwnerMap[dwKinId] ~= nMapTemplateId and nMapTemplateId ~= self.tbOldOwnerMap[dwKinId] then
				local nAddScore = tbFlagAddScore[nMapLevel][nFlagLevel]
				tbKinAddScore[dwKinId] = (tbKinAddScore[dwKinId] or 0) + nAddScore 
			end
		end
	...
	--将原来所有自动宣战的家族加上地图的门分	
	for dwKinId, nMapTemplateId in pairs(self.tbOldOwnerMap) do
		local nAutoFightMap = self:GetFatherLinkMap(nMapTemplateId)
		if nAutoFightMap then
			local nNowHaveMapTemplateId = tbOwnerMap[dwKinId]	
			if nAutoFightMap ~= nNowHaveMapTemplateId then
				local nGateAddScore = tbGateAddScore[nAutoFightMap]
				if nGateAddScore then
					tbKinAddScore[dwKinId] = (tbKinAddScore[dwKinId] or 0) + nGateAddScore
				end
			end
		end
	end
	...
	for dwKinId, v in pairs(tbKinMapScore) do
		...
		local nOldMaxScore = nMaxScore
		local nAddScore = tbKinAddScore[dwKinId]
		if nAddScore then --都是没有占领目标领地情况下的
			local nOldOwnerMapId = self.tbOldOwnerMap[dwKinId]
			if nOldOwnerMapId then
				local nBaseScore = DomainBattle:GetBaseMapScore(nOldOwnerMapId)
				nMaxScore = math.floor(nMaxScore * (1 + nAddScore / nBaseScore) ) 
			end
		end
		Log("DomainBattle:SendKinAward", dwKinId, nOldMaxScore, nMaxScore, nMaxScoreMapId)
		local nMapLevel;
		...
		--针对max分进行奖励分配
		local tbItems = DomainBattle:GetActionAwardFromParam(tbAwardSet, nMaxScore, dwKinId)
...
function DomainBattle:SendCityMasterAward()
	...
	if nLeaderId and nLeaderId ~= 0 then
		...
		Mail:SendSystemMail(
		{
			To 	  		= nLeaderId,
			Title 		= "恭喜阁下成为襄阳城主",
			Text        = "在阁下英明神武的领导下，你的帮派同仇敌忾、浴血奋战，终於夺下了襄阳城，获得无上荣耀，你也成为了襄阳城新的主人，特赠与阁下贺礼，聊表心意。",
...
function DomainBattle:StopActivity(nMapTemplateId)
	...
	self.tbOldOwnerMap = nil;

	local szNews = "今日攻城战已结束";
	local tbMapOwner = self:GetDomainMapOwner()
	...
	if tbKinInfo then
		szNews = szNews .. string.format("， 恭喜「%s」帮派占据了襄阳城！", tbKinInfo.szName) 
	end

	KPlayer.SendWorldNotify(1, 999, szNews, 1, 1)
	...
	for nMapTemplateId, dwKinId in pairs(tbMapOwner) do
		...
		if tbKinInfo then
			...
			DomainBattle.tbCross:AddLocalScore(DomainBattle:GetMapLevel(nMapTemplateId), dwKinId)
		end
	end
	...
	DomainBattle.tbCross:OnLocalBattleEnd()
end
...
function DomainBattle:KinSignUp(pPlayer, nMapTemplateId)
	...
	if not define.tbCanDeclareCareer[nCareer] then
		pPlayer.CenterMsg("您不是帮派堂主或副堂主，无法宣战")
		return
	end
	...
	if nHasOwnerMapId then
		...
		if nAutoFightMap then
			pPlayer.CenterMsg("本帮派已经自动宣战" .. Map:GetMapName(nAutoFightMap))
			return
		else
			pPlayer.CenterMsg("本帮派现在只需防守")
			return
		end
	end
	...
	if self:GetKinWarDeclare()[dwKinId] == nMapTemplateId then
		pPlayer.CenterMsg("已对该地图宣战")
		return
	end

	if nMapTemplateId == self.tbMapSetting.nMapTemplateId then
		pPlayer.CenterMsg("只有占据村镇的帮派有资格攻打主城", true)
		return
	end

	self:SetData(dwKinId, self:GetKinWarDeclare(), dwKinId, nMapTemplateId)
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("本帮派当前宣战目标变更为「%s」，开战後只能攻打宣战目标。",Map:GetMapName(nMapTemplateId)), dwKinId);
...
function DomainBattle:PlayerSignUp(pPlayer)
	...
	if not tbFifhtMapId or not self.nActivyStartTime then
		pPlayer.CenterMsg("目前还不可以参战")
		return
	end

	if pPlayer.nLevel < self.define.nMinLevel then
		pPlayer.CenterMsg(string.format("%d级才可参加", self.define.nMinLevel))
		return
	end
	...
	if not self.tbPlayerSignTime[pPlayer.dwID] then
...
function DomainBattle:RequestLeave(pPlayer)
	...
	if nSchedulePos ~= 3 then
		if pPlayer.nFightMode  == 1 then
			GeneralProcess:StartProcess(pPlayer, 5 * Env.GAME_FPS, "传送中...", self.SwitchMapDirectly, self);
			return
		end
...
function DomainBattle:GotoAttackMap(pPlayer)
	...
	if self:GetDomainMapOwner()[nMapTemplateId] ~= pPlayer.dwKinId then
		pPlayer.CenterMsg( string.format("只有%s的占领者才有传送许可权", Map:GetMapName(nMapTemplateId)) )
		return
	end
	if not self.tbFifhtMapId then
		return
...
function DomainBattle:GotoDefendMap(pPlayer, nCampIndex)
	...
	if self:GetDomainMapOwner()[nGotoMapTemplateId] ~= pPlayer.dwKinId then
		pPlayer.CenterMsg(string.format("只有%s的占领者才有传送许可权", Map:GetMapName(nGotoMapTemplateId)) )
		return
	end
	if not self.tbFifhtMapId then
		return
...
function DomainBattle:SelectCamp(pPlayer, nIndex)
	...
	pPlayer.CenterMsg(string.format("帮派当前进攻的征战营地已经更换为「%s」！", tbInfo.tbAtackPos[nIndex][4]), true)
end

function DomainBattle:RequestAttackInfo(pPlayer)
	local dwKinId = pPlayer.dwKinId
	local tbOwnerMap = self:GetDomainOwnerMap()
	local nMapTemplateId = tbOwnerMap[dwKinId]
	if not nMapTemplateId then
		return
	end
	if not self:GetFatherLinkMap(nMapTemplateId) then
		return
	end
	local tbFifhtMapId = self.tbFifhtMapId
	if not tbFifhtMapId then
		return
	end
	local nAttackMapTmp = self:GetKinWarDeclare()[dwKinId] 
	local nAttackMapId = tbFifhtMapId[nAttackMapTmp]
	local tbInst = self.tbMapInst[nAttackMapId]
	tbInst:RequestAttackInfo(pPlayer)
end
...
function DomainBattle:AddOnwenrAcutionAward()
	...
	for nMapTemplateId, dwKinId in pairs(tbMapOwner) do
		...
		if kinData then
			...
			local tbItems = DomainBattle:GetActionAwardFromParam(tbFlagAwardSetting, nSocre, dwKinId)
			if next(tbItems) then
...
function DomainBattle:GetActionAwardFromParam(tbFlagAwardSetting, nSocre, dwKinId)
	local tbItems = {}
	local fnGetRandomAward = function (nItemId, nCount, tbItems)
		for i=1,nCount do
			...
			local tbAllAward;
			if szClass == "RandomItemByTimeFrame" then
				local nRet, szMsg, tbTempAllAward = Item:GetClass(szClass):GetAwardListByTimeFrame(nParamId, "领地行商")
				tbAllAward = tbTempAllAward;
			elseif szClass == "RandomItemByLevel" then
				local nRet, szMsg, tbTempAllAward = Item:GetClass(szClass):GetAwardListByLevel(GetMaxLevel(), nParamId, "领地行商")
				tbAllAward = tbTempAllAward;
			else
	...
	for _, v2 in ipairs(tbFlagAwardSetting) do
		...
		local szGuaranteeKey = define.tbGuaranteeItemKey[nItemId]
		local nAddCount = 0;
		if szGuaranteeKey then
			nAddCount = RandomAward:GetKinGuaranteeAwardCount(dwKinId, nLeftScore, nScorePrice, szGuaranteeKey)
		else
			local nRandVal = MathRandom(nScorePrice)
			Log("DomainBattle GetActionAwardFromParam", dwKinId, nItemId, nRandVal, nScorePrice, nLeftScore)
			if  nRandVal < nLeftScore then
				nAddCount = 1;
			end			
		end
		nCount = nCount + nAddCount;
...
function DomainBattle:NotifyBattleOpen()
	KPlayer.SendWorldNotify(self.define.nMinLevel, 999, "今日攻城战已经开始，请已宣战帮派成员尽快进入战场，为夺取帮派领地而战！", 1, 1)
...
function DomainBattle:OnCreateChatRoom(dwKinId, uRoomHighId, uRoomLowId)
	if DomainBattle.tbCross:OnCreateChatRoom(dwKinId, uRoomHighId, uRoomLowId) then
		return true
	end
