		...
		if fChangeFlag <= fP then
			if fP * nMaxLife < nOldHp then
				...
				if dwOldKinId and dwOldKinId ~= -1 then
					ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("本帮派的%s——%s%s！", Map:GetMapName(self.nMapTemplateId), pNpc.szName, szDmgInfo), dwOldKinId);	
				end	
			end
			break;
		end
...
function tbBase:StopFight()
	...
	local szMsg = dwWinId and string.format("恭喜%s帮派占据了%s", szWinName, Map:GetMapName(self.nMapTemplateId)) or "本场无战果" 
	local fnNofiy = function (pPlayer)
...
function tbBase:OnKillPlayer(pKiller, pDeader)
	...
	if nDomainBattleCombo % 10 == 0 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("「%s」在攻城战中，连斩%s人！", pKiller.szName, nDomainBattleCombo), pKiller.dwKinId);	
	end
...
function tbBase:CreateCampNpc(nFlagIndex, dwKinId, nDelay)
	...
	local szTitle = string.format("「%s」占据", tbKin.szName)
	...
	self.tbMapShowInfo[szNpcIndex] = string.format("[00FFFF]%s[-]", tbKin.szName)
...
function tbBase:OnFlagDeath(him, pKiller, nFlagIndex)
	...
	local function fnDamageCmp(a, b)
	    ...
	    for i,v in ipairs(tbSortDmg) do
		TLog("DomainBattleFlagFlow", szGameAppid, nPlat, nServerIdentity, self.nMapTemplateId, self.nOwnerKin, v[1], v[2], math.floor(v[2] / nMaxLife * 10000) , i, him.nTemplateId, him.nId  )
	    end
    end
	...
	DomainBattle:AddMapKinScore(self.nMapId, dwKillKinId, define.nKillFlagAddScore, string.format("摧毁%s获得了%d积分！", him.szName, define.nKillFlagAddScore))

	local tbFlagState = self.tbFlagState[nFlagIndex]
	local dwOldKinId = tbFlagState[1]
	if dwOldKinId and dwOldKinId ~= -1 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("本帮派的%s——%s已被摧毁！", Map:GetMapName(self.nMapTemplateId), him.szName ), dwOldKinId);	
	end

	self:CreateCampNpc(nFlagIndex, dwKillKinId, Env.GAME_FPS * 5)
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("本帮派成功占据了%s——%s！", Map:GetMapName(self.nMapTemplateId), him.szName ), dwKillKinId);
end
	...
	if nKillNpcAddScore then
		if pKiller then
			...
			if pPlayer then
				...
				pPlayer.CenterMsg(string.format("摧毁%s获得了%d积分！", him.szName, nKillNpcAddScore))
			end
		end
	end
...
function tbBase:UseItemCallDialogNpc(pPlayer, nNpcId, nDir, bIsVehicles, szItemName)
	...
	if bIsVehicles then --如果是攻城车
		if DomainBattle:GetMapLevel(self.nMapTemplateId) == 3 then
			return false, "攻城车不能在野外领地使用"
		end
		if pPlayer.dwKinId == self.nOwnerKin then
			return false, "守方不能用攻城车"
		end
	end
...
function tbBase:GetCacheAttackInfo(bReset)
	if bReset then
		self.nLastRequestAttackTime = nil;
	end
	local nNow = GetTime()
	if not self.nLastRequestAttackTime or self.nLastRequestAttackTime + 2 < nNow then
		self.nLastRequestAttackTime = nNow
		self.tbCackeAttackInfo = { };
		local tbGateState = {};
		for nNpcId, nDoorIndex in pairs(self.tbDoorNpcs) do
			local pNpc = KNpc.GetById(nNpcId)
			local nPercent = 0;
			if pNpc then
				nPercent = pNpc.nCurLife / pNpc.nMaxLife
			end
			tbGateState[nDoorIndex] = nPercent
		end
		self.tbCackeAttackInfo.tbGateState = tbGateState
		self.tbCackeAttackInfo.tbFlagState = self.tbFlagState
		self.tbCackeAttackInfo.nAttackMap = self.nMapTemplateId
	end
	return self.tbCackeAttackInfo
end

function tbBase:RequestAttackInfo( pPlayer )
	pPlayer.CallClientScript("DomainBattle:OnSynAttackAwardInfo", self:GetCacheAttackInfo() )
end
