...
function MapExplore:RequestExplore(pPlayer, nMapTemplateId)
	...
	if pPlayer.dwTeamID > 0 then
		...
		pPlayer.MsgBox("组队状态无法进入探索，是否退出目前队伍？",
			{
				{"退出并进入", fnQuitTeamAndRequestExplore},
				{"取消"},
			})
		return;
	end
