				...
				for _,nId in ipairs(tbIds) do
					...
					if tbInfo then
						...
						if nNow-nJoinTime>=24*3600*nRewardMinJoinDay then
							...
							local szText = string.format("    恭喜你所在的帮派在百大帮派活动中排名[FFFE0D]第%d名[-]", i)
							local szRankDesc = self:_OpenSrvGetRankDesc(i)
							if szRankDesc then
								szText = string.format("%s，被评选为%s。", szText, szRankDesc)
							end
							Mail:SendSystemMail({
								To = nId,
							    Title = "百大帮派评选",
...
function Kin:_OpenSrvDontQuitMail()
	...
	for nKinId in pairs(self.KinData) do
		Mail:SendKinMail({
			KinId = nKinId,
			Text = string.format("    诸位侠士，百大帮派评选将於[FFFE0D]%s（%d天后）[-]截止！\n    [FFFE0D]注意：此时脱离帮派将会错失丰厚的活动奖励[-]", szEndDate, nDaysLeft),
			From = "帮派总管",
		})
	end
end
