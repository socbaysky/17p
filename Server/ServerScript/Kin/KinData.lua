...
function _KinData:TryPushMail()
	...
	local szAndroidUrl = "http://jxqy.qq.com/webplat/info/news_version3/22500/23887/23888/23889/23891/m15035/201702/553075.shtml"
	local szIosUrl = "http://jxqy.qq.com/webplat/info/news_version3/22500/23887/23888/23889/23891/m15035/201702/553086.shtml"
	...
	for nId in pairs(tbValidIds) do
		--混服时同服会存在2种平台的玩家
		local pStayInfo = KPlayer.GetRoleStayInfo(nId)
		if pStayInfo then
			local szUrl = (pStayInfo.nPlatId == 1 and szAndroidUrl) or szIosUrl;
			Mail:SendSystemMail({
				To = nId,
				Title = "帮派全员一呼百应",
				From = "帮派总管",
				Text = string.format("    为方便帮派管理对帮派大小事宜的统一指挥行动，现特对帮派邮件增加手机系统提醒功能，帮派邮件不仅在游戏内发送给每个帮派成员，同时也将通过手机系统提示直接发送到已开通“剑侠情缘”手游系统提示许可权的帮派成员手机，帮派大事分秒即达，点击[FFFE0D][url=openinnerurl:<查看教程>,%s][-]，可查看手机系统提示开启方法。", szUrl),
			})
...
function _KinData:CacheChatMsg(nChannelId, nSenderId, szSenderName, nFaction, nPortrait, nLevel, nHeadBg, nChatBg, nSex, nNamePrefix, szMsg, tbLinkInfo)
...
function _KinData:DeleteMember(nMemberId)
	if nMemberId==self.nLeaderId then
		...
		Mail:SendKinMail({
			KinId = self.nKinId,
			Text = "    由於现任总堂主离开了帮派，新一任【总堂主】将于3天後评选，累积贡献最高的成员将成为新的帮派总堂主。",
			From = "帮派总管",
		})
	...
	Activity:OnGlobalEvent("Act_OnKinMemberQuit", self.nKinId, nMemberId)
...
function _KinData:ChangePublicDeclare(szPublicDeclare)
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, me.szName .. "更新了帮派公告！", self.nKinId);
...
function _KinData:AddFound(nPlayerId, nValue)
	...
	local nMax = self:GetMaxFound()
	if self.nFound + nValue > nMax then
		nValue = nMax-self.nFound
	end
...
function _KinData:GetMaxFound()
	return Kin:GetMaxFound(self:GetBuildingLevel(Kin.Def.Building_Main));
end
...
--获取指定职位玩家列表
function _KinData:GetCareerList(tbCrareer)
	local tbList = {}
	for nPlayerId, nCareer in pairs(self.tbMembers) do
		if tbCrareer[nCareer] then
			tbList[nPlayerId] = true
		end
	end
...
function _KinData:AddPrestige(nValue, nLogReason)
	...
	if nValue > 0 then
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("帮派於江湖中的威望值提升了%d", nValue), self.nKinId);
	end
end
...
function _KinData:SetLeaderId(nLeaderId, bSendMail)
	...
	if bSendMail and nOldLeaderId~=nLeaderId then
		...
		if tbMember then
			...
			Mail:SendKinMail({
				KinId = self.nKinId,
				Text = string.format("    帮派的诸位弟兄！大侠「%s」在帮派中的累计贡献已达%d，远超他人，成为新一任帮派【总堂主】乃实至名归，族中皆兄弟，还望各位弟兄日後能够鼎力相助，我等一同奋战，齐心协力，书写帮派辉煌！", szName, nContribution),
				From = "帮派总管",
			})
		end
	end
...
function _KinData:AppointLeader(nCandidateLeaderId)
	...
	if nNow-self.nAppointLeaderTime<=24*3600 then
		...
		return false, string.format("需%d小时後才可再进行总堂主任命操作", nHours)
	end
	...
	if pLeader and pCandidate then
		local szMsg = string.format("「%s」将「%s」任命为【候选总堂主】，将在72小时後正式成为【总堂主】", pLeader.szName, pCandidate.szName)
...
function _KinData:OnLeaderLogin()
	if self.nCandidateLeaderId>0 then
		...
		Mail:SendSystemMail({
	 		To = self.nLeaderId,
	        Title = "总堂主任命",
	        Text = string.format("    「%s」被你任命为【候选总堂主】，将在%s後正式成为【总堂主】，期间可以通过职位管理介面取消任命。", tbCandidate:GetName(), szTimeStr),
	        From = "系统",
	        tbAttach = {},
		})
	elseif self.nChangeLeaderTime>0 then
		...
		Mail:SendKinMail({
			KinId = self.nKinId,
			Text = "    由於现任总堂主重出江湖，自动取消正在进行的总堂主评选。（评选期间现任总堂主上线，自动取消总堂主评选）",
			From = "帮派总管",
		})
	end
end
...
function _KinData:CheckLeaderOff()
	...
	if nLeaderOffSeconds and nLeaderOffSeconds>=7*24*3600 then
		...
		Mail:SendKinMail({
			KinId = self.nKinId,
			Text = "    由於现任总堂主隐退江湖（离线超7天），新一任【总堂主】将于3天后评选，累积贡献最高的成员将成为新的帮派总堂主。",
			From = "帮派总管",
		})
	end
end
...
function _KinData:_ChangeLeader()
	...
	if self.nCandidateLeaderId==nLeaderId then
		...
		if pLeader and pCandidate then
			Mail:SendKinMail({
				KinId = self.nKinId,
				Text = string.format("    「%s」将「%s」任命为新一任【总堂主】", pLeader.szName, pCandidate.szName),
				From = "帮派总管",
			})
			bSendMail = false
		end
...
function _KinData:CheckAddBattleApplysLimit(nItemId, nNum)
	...
	if (self.tbAddBattleSupplyLimit[nItemId] or 0) + nNum > nLimitNum then
		...
		return false, string.format("每天最多买%d个%s", nLimitNum, tbItembase.szName)
	end
	return true
end
--todo 战争坊增加物资
...
function _KinData:ChangeName(szKinName, bNotNotify)
	...
	if not bNotNotify then
		Mail:SendKinMail({
			KinId = self.nKinId,
			Title = "帮派改名",
			Text = string.format("帮派正式更名为【%s】", szKinName),
			From = "帮派总管",
		})

		local szWorldNotifyMsg = string.format("诸位江湖侠士，经帮中弟兄商议，帮派【%s】现正式更名为【%s】，在此告知武林同道。", szOldName, szKinName)
...
function _KinMember:JoinKin(kinData)
	...
	if nMasterId ~= self.nMemberId then
		local szInfo = string.format("欢迎「%s」成为本帮派的一员！#49", tbRoleStayInfo.szName);
	...
	if nCareer==Kin.Def.Career_New and nMasterId~=self.nMemberId then
		Mail:SendSystemMail({
			...
			Title = "帮派信件",
			From = "帮派总管",
			Text = "    欢迎侠士加入本帮派！由於侠士等级较低，将暂时作为帮派中的小弟！小弟在帮派中享有特权，可被帮派其他成员[FFFE0D]【额外传功两次】[-]，帮助你早日提升自身实力，早日成为族中栋梁！",
		})
	end
	...
	DomainBattle.tbCross:OnJoinKin(self.nMemberId, self.nKinId)

	TLog("KinFlow", Env.LogWay_JoinKin, self.nKinId, kinData.szName, self.nMemberId, tbRoleStayInfo.nLevel, nCareer);
	Sdk:TLogQQInfo(tbRoleStayInfo, Env.QQTLog_Page_Kin, Env.QQTLog_Obj_JoinKin, Env.QQTLog_Operat_JoinKin, "", self.nKinId);
	return true;
end
...
function _KinMember:SendSetCareerMail(nCareer)
	...
	local szMailContent = string.format("恭喜您被任命为【%s】\n", Kin.Def.Career_Name[nCareer]);
	...
	if tbAuthority and next(tbAuthority) then
		szMailContent = szMailContent .. "拥有许可权：";
		local szDot = "";
	...
	if nCareer==Kin.Def.Career_Mascot then
		szMailContent = string.format("%s拥有特权：可在帮派频道发送30秒语音资讯，帮派吉祥物专属头像框", szMailContent)
	end
...
function _KinMember:Quite()
	...
	if tbRoleStayInfo then
		Sdk:TLogQQInfo(tbRoleStayInfo, Env.QQTLog_Page_Kin, Env.QQTLog_Obj_QuitKin, Env.QQTLog_Operat_QuitKin, "", self.nKinId);
	end
