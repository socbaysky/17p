...
Kin.tbRedBagNoKinDelayEvents = {
	...
	[Kin.tbRedBagEvents.valentines_day_1] = true,
	[Kin.tbRedBagEvents.valentines_day_2] = true,
	[Kin.tbRedBagEvents.valentines_day_3] = true,
	[Kin.tbRedBagEvents.valentines_day_4] = true,
	[Kin.tbRedBagEvents.wedding_1] = true,
	[Kin.tbRedBagEvents.wedding_2] = true,
	[Kin.tbRedBagEvents.wedding_3] = true,

	[Kin.tbRedBagEvents.GoodVoice_hx_1] = true,
	[Kin.tbRedBagEvents.GoodVoice_hx_10] = true,
	[Kin.tbRedBagEvents.GoodVoice_hx_vote] = true,
	[Kin.tbRedBagEvents.GoodVoice_SemiFinal_1] = true,
	[Kin.tbRedBagEvents.GoodVoice_SemiFinal_vote] = true,
	[Kin.tbRedBagEvents.GoodVoice_final_1] = true,
	[Kin.tbRedBagEvents.GoodVoice_final_10] = true,
	[Kin.tbRedBagEvents.GoodVoice_SemiFinalArea] = true,
	[Kin.tbRedBagEvents.GoodVoice_SemiFinalArea_normal] = true,
	[Kin.tbRedBagEvents.GoodVoice_SemiFinalFaction] = true,
	[Kin.tbRedBagEvents.GoodVoice_SemiFinalFaction_normal] = true,
...
function Kin:_RedBagDoSendNotifyClient(tbRedBag, tbPlayers, bOnline)
	local _, _, nMulti = Kin:RedBagGetMultiInfo(tbRedBag.nEventId, tbRedBag.nGold)
...
function Kin:_RedBagDoSendChatLinks(tbRedBag, nKinId, bOnline, bGlobal)
	...
	if bOnline then
		...
		local szMsg = string.format("#971%s，分享一个%s红包<查看红包>", szContent, szMulti)
		if bGlobal then
			szMsg = string.format("#940%s，分享一个%s世界红包<查看红包>", szContent, szMulti)
		end
		ChatMgr:SendPlayerMsg(bGlobal and ChatMgr.ChannelType.Public or ChatMgr.ChannelType.Kin,
			tbOwner.nId, pOwner.szName, pOwner.nFaction, pOwner.nPortrait, pOwner.nSex, pOwner.nLevel, szMsg, {
			nLinkType = ChatMgr.LinkType.KinRedBag,
			szId = tbRedBag.szId,
		})
	else
		local szSysMsg = string.format("帮派总管：#971「%s」的红包逾期由老夫代为发放 <查看红包>", pOwner.szName)
		if bGlobal then
			szSysMsg = string.format("#940「%s」的世界红包逾期由系统自动发放 <查看红包>", pOwner.szName)
		end
...
function Kin:_RedBagGlobalDoSend(tbRedBag, nKinId, bOnline)
	...
	self:_RedBagDoSendNotifyClient(tbRedBag, tbAllPlayer, bOnline)
...
function Kin:_RedBagDoSend(tbRedBag, tbKinData, bOnline)
	...
	self:_RedBagDoSendNotifyClient(tbRedBag, tbPlayers, bOnline)
...
function Kin:_RedBagCheckDead(tbRedBag, nKinId, bGlobal)
	if tbRedBag.nMaxReceiver<=#tbRedBag.tbReceivers then
		...
		if pLuckyMan and pOwner then
			local szMsg = string.format("「%s」的%d个红包共%d#999%s被抢完。「%s」手气最佳，抢到了%d#999",
