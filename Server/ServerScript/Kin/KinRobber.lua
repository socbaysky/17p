...
function _KinData:OpenKinRobber(nRobRound)
	...
	ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, "佳节将至，帮派内却忽现贼匪，诸位弟兄，收拾他们！（需组成2人以上队伍）", self.nKinId);
	...
	Timer:Register(Env.GAME_FPS * 10 * 60, function ()
		...
		if nCount < nMaxCount then
			...
			if nLeftCount > 0 then
				local szInfo = string.format("帮派中还有%d个盗贼在捣乱，诸位弟兄，剿灭他们！（需组成2人以上队伍）", nLeftCount)
...
function _KinData:OnRobberDeath(pNpc, nKillerPlayerId)
	...
	if Lib:CountTB(tbRobberData.tbNpcInfo) == 0 then
		...
		ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, "成功清扫所有的贼匪，区区小贼，竟敢来帮派中捣乱，劳烦诸位弟兄辛苦了！", self.nKinId);
		self:TraverseMembers(function (memberData)
			...
			if member and member.nMapTemplateId == Kin.Def.nKinMapTemplateId then
				member.SendBlackBoardMsg("成功将所有入侵的盗贼击退！");
			end
			return true;
		end);
