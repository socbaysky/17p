...
function StoneMgr:OnInset(pPlayer, nEquipId, nStoneTemplateId, nInsetPos)
	...
	if nOrgInsetId and  nOrgInsetId ~= 0 then
		...
		if pPlayer.GetMoney("Coin") < nCost + (nCombineCost or 0) then
			pPlayer.CenterMsg("银两不足")
			return
		end
		if not pPlayer.CostMoney("Coin", nCost, Env.LogWay_RemoveInset, nOrgInsetId) then
			pPlayer.CenterMsg("剥离银两不足")
			return
		end
	end
...
function StoneMgr:RemoveInset(pPlayer, nEquipId, nInsetPos)
	local bRet, szMsg = self:CanUnLoadStone(pPlayer, nEquipId, nInsetPos)
	if not bRet then
		return bRet, szMsg
	end
	local pEquip = pPlayer.GetItemInBag(nEquipId);
	...
	if not pPlayer.CostMoney(self.COST_MONEY_TYPE, self.COST_MONEY_COUNT, Env.LogWay_RemoveInset, nOrgInsetId) then
		local szMoneyName = Shop:GetMoneyName(self.COST_MONEY_TYPE)
		return false, string.format("剥离%s不足", szMoneyName) 
	end
	...
	StoneMgr:SetUnloadStoneTime(pPlayer);
	pPlayer.AddItem(nOrgInsetId, 1, nil, Env.LogWay_InsetGetOrg);
...
function StoneMgr:SetUnloadStoneTime(pPlayer)
	pPlayer.SetUserValue(self.USER_VALUE_GROUP_2, self.USER_VALUE_UNLOAD, GetTime())
end