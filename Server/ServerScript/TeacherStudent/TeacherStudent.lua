...
function TeacherStudent:ReqChuanGong(nTargetId)
	...
	local szTip = bOtherTeacher and "你向「%s」发起了传功请求，等待对方确认" or "你向「%s」发起了传功，等待对方确认"
...
function TeacherStudent:AssignCustomTasks(nStudentId, tbTasks)
	...
	if Lib:CountTB(tbTaskProgress)~=self.Def.nCustomTaskCount then
		return false, string.format("布置的任务数量必须是%d个", self.Def.nCustomTaskCount)
	end
	...
	me.CenterMsg(string.format("你给徒弟「%s」布置了任务", pStudent.szName))
...
function TeacherStudent:ReportCustomTasks(nTeacherId)
	...
	if #tbFinished<self.Def.nCustomTaskReportMin then
		return false, string.format("至少要完成%d个才能上交任务", self.Def.nCustomTaskReportMin)
	end
...
function TeacherStudent:_CustomTargetSendTeacherReward(nTeacherId, pStudent, nFinishedCount, szStatus, nTeacherReward)
	...
	if szStatus=="not_satisfy" then
		szMailContent = string.format([[    你的徒弟 [FFFE0D]%s[-] 完成了你布置的 [FFFE0D]%d[-] 个任务，任务完成数在本周所有徒弟中[FFFE0D]排前两位之後[-]，本次师徒任务师父无奖励。
    
小提示：
·徒弟完成任务数量越多，双方的奖励越高
·师父奖励每周以完成任务数最多的两个徒弟计算]], szStudentName, nFinishedCount)
	elseif szStatus=="dont_need_payback" then
		szMailContent = string.format([[    你的徒弟 [FFFE0D]%s[-] 完成了你布置的 [FFFE0D]%d[-] 个任务，附件是你的奖励！
    
小提示：
·徒弟完成任务数量越多，双方的奖励越高
·师父奖励每周以完成任务数最多的两个徒弟计算]], szStudentName, nFinishedCount)
	elseif szStatus=="need_payback" then
		szMailContent = string.format([[    你的徒弟 [FFFE0D]%s[-] 完成了你布置的 [FFFE0D]%d[-] 个任务，任务完成数在本周所有徒弟中[FFFE0D]排前两位[-]，你名望奖励差额请通过附件领取！
...
function TeacherStudent:ReportTargets(nTeacherId)
	...
	me.CenterMsg(string.format("你向师父「%s」汇报了师徒目标，请等待对方确认", pTeacher.szName))
end
...
function TeacherStudent:ReqDismiss(nTargetId)
	...
	if bGraduate then
		...
		me.CostGold(self.Def.nGraduateDismissCost, Env.LogWay_TS_Dismiss, 0, function(nPlayerId, bSuccess)
			...
			if not pPlayer then
				return false, "解除师徒关系时, 您掉线了"
			end
			if not bSuccess then
				return false, "消耗元宝失败"
			end
			DoAfterPay(pPlayer)
			Dialog:SendBlackBoardMsg(pPlayer, string.format("消耗了%d元宝[FFFE0D]（取消申请後自动退还）[-]", self.Def.nGraduateDismissCost));
			return true
		end)
	else
		DoAfterPay(me)
...
function TeacherStudent:CancelDismiss(nTargetId)
	...
	if bGraduate then
		...
		Mail:SendSystemMail({
			To = me.dwID,
			Title = "取消解除师徒关系",
			Text = string.format("    与%s [FFFE0D]%s[-] 解除师徒关系的申请已被你取消！附件退还申请时所花费的 [FFFE0D]%d元宝[-]，请及时领取。",
				bOtherTeacher and "师父" or "徒弟", pStay.szName, self.Def.nGraduateDismissCost),
			From = "上官飞龙",
			tbAttach = {
...
function TeacherStudent:_CanAddStudent(pPlayer)
	...
	local tbSetting = self:GetCurrentTimeFrameSettings()
	if not tbSetting then
		return false, "师徒系统尚未开放"
	end

	if pPlayer.nLevel<tbSetting.nTeaLvMin then
		return false, bImPlayer and "你的等级不满足收徒条件" or "对方的等级不满足收徒条件"
	end
	...
	if nPunishCd>0 or nAcceptCd>0 then
		...
		if nPunishCd>nAcceptCd then
			return false, bImPlayer and string.format("你处於解除师徒关系惩罚，%s後可收徒", Lib:TimeDesc5(nPunishCd)) or "对方处於解除师徒关系惩罚，无法拜其为师"
		end
		return false, bImPlayer and string.format("你处於收徒间隔期，%s後可收徒", Lib:TimeDesc5(nAcceptCd)) or "对方处於收徒间隔期，无法拜其为师"
	end
	...
	if nUngraduateCount>=self.Def.nMaxUndergraduate then
		self:_RemoveFromSearchList(pPlayer.dwID)
		return false, bImPlayer and "你未出师徒弟数量已达上限" or "对方未出师徒弟数量已达上限"
	end
...
function TeacherStudent:_CanAddTeacher(pPlayer)
	...
	if pPlayer.nLevel<tbSetting.nStuLvMin then
		return false, bImPlayer and "你的等级不满足拜师条件" or "对方的等级不满足拜师条件"
	end

	local tbPlayer = self:GetPlayerScriptTable(pPlayer)
	if self:_IsPunishing(tbPlayer) then
		return false, bImPlayer and string.format("你处於解除师徒关系惩罚，%s後拜师", Lib:TimeDesc5(tbPlayer.nPunishDeadline-GetTime())) or "对方处於解除师徒关系惩罚，无法收其为徒"
	end

	local nTeacherCount = Lib:CountTB(tbPlayer.tbTeachers)
	if nTeacherCount>=self.Def.nMaxTeachers then
		self:_RemoveFromSearchList(pPlayer.dwID)
		return false, bImPlayer and "你的师父数量已达上限" or "对方的师父数量已达上限"
	end
...
function TeacherStudent:_IsLevelValidToAdd(pTeacher, pStudent)
	...
	if (pTeacher.nLevel-pStudent.nLevel)<nLvDiff then
		return false, string.format("等级差不足%d级", nLvDiff)
	end
...
function TeacherStudent:_SendConnectApplyNotify(pMe, nOtherId, bTeacher)
	...
	local szTip = bTeacher and "你向「%s」发起了收徒请求，请等待对方确认" or "你向「%s」发起了拜师请求，请等待对方确认"
...
function TeacherStudent:_DoAddAfterCheck(pTeacher, pStudent)
	...
	local szTeacherNotice = string.format("已成功收「%s」为徒", pStudent.szName)
	...
	local szStudentNotice = string.format("已成功拜「%s」为师", pTeacher.szName)
...
function TeacherStudent:_AddTitleWith(pMe, bImTeacher, nOtherId, szOtherName)
	...
	local szTitle = bImTeacher and string.format("%s的师父", szOtherName) or string.format("%s的徒弟", szOtherName)
...
function TeacherStudent:_CheckBeforeChuanGong(pTeacher, pStudent)
	...
	if not bOk then
		if pStudent.nLevel >= pTeacher.nLevel then
			szErr = pStudent==me and "你的等级≥师傅等级，无法进行传功" or "只能给比自己等级低的徒弟传功"
		end
		return false, szErr
	end
...
function TeacherStudent:_CheckBeforeReport(pTeacher, pStudent)
	...
	local tbStudent = self:GetPlayerScriptTable(pStudent)
	if tbStudent.tbTeachers[pTeacher.dwID].bGraduate then
		return false, bImTeacher and "对方已出师，不可汇报" or "你已出师，不可汇报"
	end

	local tbTargets = self:_GetCanReportTargets(pTeacher, pStudent)
	if #tbTargets<=0 then
		return false, "没有可汇报的目标"
	end
...
function TeacherStudent:_DoConfirmReportAfterCheck(pTeacher, pStudent, tbTargets)
	...
	if nTotalExp>0 then
		...
		local szStudentNotice = string.format("成功汇报目标，本次汇报目标获得%d经验", nTotalExp)
		pStudent.CenterMsg(szStudentNotice)
		pStudent.Msg(szStudentNotice)
	end
	if bIgnoreImityReward then
		pStudent.Msg("因您的亲密度目标累计汇报次数已达上限，本次汇报亲密度目标奖励超出部分不发放")
	end

	if nTotalRenown>0 then
		pTeacher.AddMoney("Renown", nTotalRenown, Env.LogWay_TS_Report)
		local szTeacherNotice = string.format("成功确认目标，本次汇报目标获得%d名望", nTotalRenown)
		pTeacher.CenterMsg(szTeacherNotice)
		pTeacher.Msg(szTeacherNotice)
	end
	if bIgnoreImityReward then
		pTeacher.Msg("因徒弟的亲密度目标累计汇报次数已达上限，本次汇报亲密度目标奖励超出部分不发放")
	end
...
function TeacherStudent:_GiveGraduateRewards(pPlayer, nOtherId, bDontGiveOther)
	...
	if tbTeacherReward and not(bDontGiveOther and nTeacherId==nOtherId) then
		Mail:SendSystemMail({
			To = nTeacherId,
			Title = "出师奖励",
			Text = string.format("    你的徒弟「%s」已成功出师。附件为出师奖励，请查收！\n    出师评价：%s（达成%d条以上师徒目标）\n    出师奖励：%s", bOtherStudent and szOtherName or pPlayer.szName, tbTeacherReward.szJudgement, tbTeacherReward.nMin, tbTeacherReward.szJudgement2),
			From = "上官飞龙",
	...
	if tbStudentReward and not(bDontGiveOther and nStudentId==nOtherId) then
		Mail:SendSystemMail({
			To = nStudentId,
			Title = "出师奖励",
			Text = string.format("    你已成功从师父「%s」门下出师，以後可以独自闯荡江湖了。附件为出师奖励，请查收！\n    出师评价：%s（达成%d条以上师徒目标）\n    出师奖励：%s", bOtherStudent and pPlayer.szName or szOtherName, tbStudentReward.szJudgement, tbStudentReward.nMin, tbStudentReward.szJudgement2),
			From = "上官飞龙",
...
function TeacherStudent:RefundForceDismissGold(nId1, nId2, bTeacher)
	...
	if bGraduate then
		...
		Mail:SendSystemMail({
			To = nId1,
			Title = "取消解除师徒关系",
			Text = string.format("    与%s [FFFE0D]%s[-] 解除师徒关系的申请已被自动取消！附件退还申请时所花费的 [FFFE0D]%d元宝[-]，请及时领取。",
				bTeacher and "师父" or "徒弟", pStay.szName, self.Def.nGraduateDismissCost),
			From = "上官飞龙",
			tbAttach = {
				{"Gold", self.Def.nGraduateDismissCost},
			},
...
function TeacherStudent:_DoForceGraduate(pMe, nOtherId)
	...
	if bAmIStudent then
		local szStudentNotice = "你已经成功出师，以後可以独自闯荡江湖了（奖励已经通过邮件发放）"
		pMe.CenterMsg(szStudentNotice)
		pMe.Msg(szStudentNotice)
	else
		local pStay = KPlayer.GetRoleStayInfo(nOtherId)
		local szTeacherNotice = tbTeacherReward and string.format("你的徒弟「%s」已成功出师（奖励已经通过邮件发放）", pStay.szName) or string.format("你的徒弟「%s」已成功出师", pStay.szName)
...
function TeacherStudent:_CheckBeforeGraduate(pTeacher, pStudent)
	...
	if nTargetsCount<self.Def.nGraduateTargetMin then
		return false, string.format("至少完成%d个师徒目标", self.Def.nGraduateTargetMin)
	end
...
function TeacherStudent:_DoGraduateAfterCheck(pTeacher, pStudent)
	...
	local szTeacherNotice = string.format("你的徒弟「%s」已成功出师（奖励已经通过邮件发放）", pStudent.szName)
	pTeacher.CenterMsg(szTeacherNotice)
	pTeacher.Msg(szTeacherNotice)
...
function TeacherStudent:_CheckShowDismissNotice(pPlayer)
	...
	for nOtherId, tbInfo in pairs(tbMyDismissing) do
		local szOtherRole = tbInfo.bOtherTeacher and "师父" or "徒弟"
		...
		if pOtherStayInfo then
			...
			if tbInfo.bMyReq then
				szMsg = string.format("你正在申请与%s「%s」解除师徒关系，将在%s後生效", szOtherRole, pOtherStayInfo.szName, szDeadline)
			else
				szMsg = string.format("你的%s「%s」正在申请与你解除师徒关系，将在%s後生效", szOtherRole, pOtherStayInfo.szName, szDeadline)
			end
			pPlayer.Msg(szMsg)
		end
	end
...
function TeacherStudent:_DoDismissWith(pMe, nOtherId, bForce, bReq, bGraduate)
	...
	Mail:SendSystemMail({
		To = pMe.dwID,
		Title = "解除师徒关系",
		Text = string.format("    你的%s「%s」已经与你正式解除师徒关系。", szOtherRole, pOtherStayInfo.szName),
		From = "上官飞龙",
	})
...
function TeacherStudent:_GiveGraduateRewardAfterCheck(pTeacher, pStudent, nItemId, szMsg)
	...
	szMsg = szMsg or ""
	if szMsg~="" then
		szMsg = string.format('    师父留言："%s"', szMsg)
	end
	Mail:SendSystemMail({
		To = pStudent.dwID,
		Title = "师父赠送",
		Text = string.format("    你的师父「%s」给你赠送了出师礼物，请查收附件。\n\n%s", pTeacher.szName, szMsg),
		From = "上官飞龙",
		tbAttach = {
			{"item", nItemId, 1},
		},
