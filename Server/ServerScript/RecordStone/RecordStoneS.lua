...
function RecordStone:AddRecordCount(pPlayer, szType, nCount)
    ...
    if type(pPlayer) == "number" then
        local nPlayerId = pPlayer
        pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
        if not pPlayer then
            local pStayInfo = KPlayer.GetRoleStayInfo(nPlayerId)
            if pStayInfo then
                KPlayer.AddDelayCmd(nPlayerId,
                    string.format("RecordStone:AddRecordCount(me, '%s', %d)", szType, nCount),
                    string.format("RecordStone:AddRecordCount By DelayCmd: %d, %s, %d", nPlayerId, szType, nCount))
            end
            return
        end
    end
...
--新开时间轴容易漏配置可铭刻石的装备
function RecordStone:CheckDataSetting()
    local nMaxLevel = TimeFrame.tbOpenNewMaxLevelTimeFrame[#TimeFrame.tbOpenNewMaxLevelTimeFrame].nMaxLevel;
    local nUseWeaponLevel = nMaxLevel - 9;
    local tbTypeCount = {};
    for ItemId,v in pairs(self.tbRecordEquipInfo) do
        local tbItembase = KItem.GetItemBaseProp(ItemId)
        if tbItembase.nRequireLevel >= nUseWeaponLevel then
            tbTypeCount[tbItembase.nDetailType]  = true;
        end
    end 
    --最大时间轴下 需要要对应稀有和黄金武器的铭刻石配置
    assert(tbTypeCount[Item.DetailType_Rare])
    assert(tbTypeCount[Item.DetailType_Gold])
end