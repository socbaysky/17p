...
--    self.tbCrossBossAllInfo.tbTimeFrame = {};
    ...
    self.tbCrossBossAllInfo.szCurTimeFrame = nil;
...
function BossLeader:GetCrossBossTimeFrame()
    ...
    if MODULE_ZONESERVER then
        return self.tbCrossBossAllInfo.szCurTimeFrame;
    end

    return self.tbCrossBossDataInfo.szTimeFrame;
end

function BossLeader:UpdateCrossBossTimeFrame(szTimeFrame)
    if not self.tbCrossBossAllInfo.szCurTimeFrame then
        self.tbCrossBossAllInfo.szCurTimeFrame = szTimeFrame;
    else
        local nCurOpenTime = TimeFrame:CalcTimeFrameOpenTime(self.tbCrossBossAllInfo.szCurTimeFrame);
        local nNewOpenTime = TimeFrame:CalcTimeFrameOpenTime(szTimeFrame);
        if nNewOpenTime > nCurOpenTime then
            self.tbCrossBossAllInfo.szCurTimeFrame = szTimeFrame;
        end
    end
    Log("UpdateCrossBossTimeFrame", self.tbCrossBossAllInfo.szCurTimeFrame, szTimeFrame);
end
...
function BossLeader:PlayerCreateCrossTeam(pPlayer)
    ...
    local nTeamId = nil;
    for nOrgPlayerID, _ in pairs(tbTeamInfo.tbMember) do
        local nPlayerID = KPlayer.GetZonePlayerIdByOrgId(nOrgPlayerID, pPlayer.nZoneServerId);
    ...
    local _, nOrgCaptainId = KPlayer.GetOrgPlayerIdByZoneId(nCaptainId);
    if nOrgCaptainId ~= tbTeamInfo.nCaptainId and tbTeamInfo.nCaptainId == pPlayer.dwOrgPlayerId then
...
function BossLeader:GetKinConnectIndex(nKinId)
    ...
    local _, nOrgKinId = KPlayer.GetOrgKinIdByZoneId(nKinId);
    if nOrgKinId <= 0 then
        Log(debug.traceback());
    end

    local tbKinInfo = self:GetCrossBossKinInfo(nKinId);
    return tbKinInfo.nZoneIndex, nOrgKinId;
end
...
function BossLeader:SetServerCrossBossInfoZ(szTimeFrame, tbAllKinInfo)
    ...
    self:UpdateCrossBossTimeFrame(szTimeFrame);

    local nServerId = Server:GetServerId(nConnectIdx);
    for nOrgKinId, tbInfo in pairs(tbAllKinInfo) do
        local nKinId = KPlayer.ForceGetZoneKinIdByOrgId(nOrgKinId, nServerId);
        local tbKinInfo = self:GetCrossBossKinInfo(nKinId);
...
function BossLeader:PreStartCrossBossZ()
    ...
    local szCrossBossTimeFrame = self:GetCrossBossTimeFrame();
    for nConnectIdx, _ in pairs(tbServerInfo) do
        CallZoneClientScript(nConnectIdx, "BossLeader:StartCrossBossServer", self.tbCreateNpcMap, szCrossBossTimeFrame);
    end
...
function BossLeader:StartCrossBossServer(tbMapDate, szCrossBossTimeFrame)
    for nMapTID, tbInfo in pairs(tbMapDate) do
        self.tbCrossBossMapInfo[nMapTID] = tbInfo.nMapId;
        Log("BossLeader SetCanCrossServer Map Info", nMapTID, tbInfo.nMapId);
    ...
    self.tbCrossBossDataInfo.szTimeFrame = szCrossBossTimeFrame;
    self.tbCrossBossDataInfo.nTime = GetTime();
...
function BossLeader:SendCrossPlayerInfoZ(nOrgPlayerId, tbSendInfo)
    local nConnectIdx = Server.nCurConnectIdx;
    local nServerId = Server:GetServerId(nConnectIdx);
    local nPlayerID = KPlayer.ForceGetZonePlayerIdByOrgId(nOrgPlayerId, nServerId);
    local tbPlayerInfo = self:GetCrossBossPlayerInfo(nPlayerID);
