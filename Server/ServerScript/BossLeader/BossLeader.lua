...
function BossLeader:AddAchievement(pPlayer, szType, tbAchievementType)
    ...
    Lib:CallZ2SOrLocalScript(pPlayer.nZoneIndex, "BossLeader:AddPlayerAchievement", pPlayer.dwOrgPlayerId, tbAchievement.szKey, tbAchievement.nValue);  
end
...
function BossLeader:CheckEnterFuben(pPlayer, szType, nMapTID)
    ...
    if self:HaveCrossBossServer(pPlayer) then
        ...
        if not tbMapInfo then
            local szShowInfo = "尚未开启!";
            if self.tbCrossBossMapInfo[nMapTID] then
                szShowInfo = "没有资格进入跨服地图";
            end
...
function BossLeader:ApplyEnterFuben(pPlayer, szType, nMapTID)
    ...
    if pPlayer.nFightMode == 1 then -- 野外传送读条
        GeneralProcess:StartProcess(pPlayer, 5 * Env.GAME_FPS, "传送中...", self.SwitchMapDirectly, self, nMapID, nMapTID);
    else
...
function BossLeader:CloseActivity(szType)
    ...
    if szType == "Boss" then
        ...
        if not MODULE_ZONESERVER then
            ...
            BossLeader:OnEndLeaveKinChatRoom()
        end
    end
    ...
    tbTypeData.bKinJiFen = false;
    tbTypeData.tbKinAuctionItem = {};
    tbTypeData.bAddJoinCount = false;
...
function BossLeader:AddKinJoinCount(szType, pPlayer)
    local tbTypeData = self:GetTypeData(szType);
    if not tbTypeData.bAddJoinCount then
        return;
    end
    ...
    if tbKinPlayer[pPlayer.dwOrgPlayerId] then
        return;
    end

    tbKinPlayer[pPlayer.dwOrgPlayerId] = true; 
    local nPrestige = BossLeader.tbJoinPrestige[szType];
    Lib:CallZ2SOrLocalScript(pPlayer.nZoneIndex, "BossLeader:AddKinPrestige", pPlayer.dwOrgKinId, nPrestige, szType);   

    if szType == "Boss" then
        Dialog:SendBlackBoardMsg(pPlayer, "成功参与挑战本次名将，可享受帮派分红");
        pPlayer.Msg("成功参与挑战本次名将，可享受帮派分红");
    end

    Log("BossLeader AddKinJoinCount", szType, pPlayer.dwID);    
end
...
function BossLeader:SendKinNotify(nKindID, bWorldNotice, bKinNotice, szNpcName, szInfoMsg)
    ...
    if bKinNotice then
        local szShowMsg = string.format("恭喜本帮派挑战%s, 获得了%s", szNpcName or "", szInfoMsg);
    ...
    if bWorldNotice then
        ...
        local szShowMsg = string.format("恭喜%s帮派挑战%s, 获得了%s", szKinName or "-", szNpcName or "", szInfoMsg);
...
function BossLeader:CalcKinDmgRankAward(szType, tbKinDmg, pBossNpc, nMapID, nNpcTotalDamage)
    ...
    if nLastDmgPlayerID > 0 then
        ...
        if tbNpcInfo.LastAwardID > 0 then
            ...
            local nOrgPlayerId = KPlayer.GetOrgPlayerId(nLastDmgPlayerID);
            Lib:CallZ2SOrLocalScript(nConnect, "BossLeader:SendLastDmgPlayerAward", szType, nOrgPlayerId, tbNpcInfo.LastAwardID);   
        end
    ...
    for nRank, tbKinInfo in ipairs(tbSortKinDmg) do
        ...
        if not MODULE_ZONESERVER then
            ...
            local nConnectIdx, nOrgKinId = self:GetKinConnectIndex(tbKinInfo.nKinId);
            local tbOrgKinInfo = {nKinId = nOrgKinId, nHPPercent = tbKinInfo.nHPPercent};
            CallZoneClientScript(nConnectIdx, "BossLeader:CrossCalcKinDmgTotalValue", szType, tbOrgKinInfo, tbNpcExtInfo, nJoinCount, nRank);
        end
...
function BossLeader:CalcKinDmgTotalValue(szType, tbKinInfo, tbNpcExtInfo, nJoinCount, nRank) 
    ...
    TLog("KinActFlow", szGameAppid, nPlat, nServerIdentity, Env.LogWay_FieldBoss, tbKinInfo.nKinId, tbNpcInfo.NpcID, math.floor(tbKinInfo.nHPPercent * 100));
    ...
    for _, tbItemInfo in pairs(tbValueAward) do
        ...
        if nCurValue > 0 and tbItemInfo.ItemID > 0 then
            ...
            if nItemCount > 0 then
                ...
                local szMsg = string.format("%s个%s", nItemCount, szItemName);
                local szTrueNpc = "(真身)";
    ...
    for nIndex, nItemID in pairs(tbNpcInfo.tbExtRandomItemID) do
        ...
        if nRandomValue > 0 and nItemID > 0 then
            ...
            if nExtItemCount > 0 then
                ...
                local szMsg = string.format("%s个%s", nExtItemCount, szItemName);
                local szTrueNpc = "(真身)";
                if tbNpcInfo and tbNpcInfo.IsFalse == 1 then
                    szTrueNpc = "(分身)";
                end
...
function BossLeader:CalcKinDmgMingJiangHonor(szType, nKinId, nRankValue, tbNpcInfo, tbTimeFrameInfo)
    ...
    if nCount > 0 then
        ...
        local szMsg = string.format("%s个%s", nCount, szItemName);
        local szTrueNpc = "(真身)";
        if tbNpcInfo and tbNpcInfo.IsFalse == 1 then
            szTrueNpc = "(分身)";
        end
...
function BossLeader:CalcKinDmgSoul(szType, nKinId, nRankValue, tbNpcInfo, tbTimeFrameInfo)
    ...
    if nStoneCount > 0 then
        ...
        local szMsg = string.format("%s个%s", nStoneCount, szItemName);
        local szTrueNpc = "(真身)";
        if tbNpcInfo and tbNpcInfo.IsFalse == 1 then
            szTrueNpc = "(分身)";
        end
...
function BossLeader:CalKinDmgItem(szType, nKinId, nRankValue, tbNpcInfo)
    ...
    for nIndex, nCount in pairs(tbItemCount) do
        ...
        if tbAwardInfo.WorldNotice == 1 or tbAwardInfo.KinNotice == 1 then
            ...
            szInfoMsg = szInfoMsg .. string.format("%s个%s", nCount, szName);
        end     
    end
...
function BossLeader:AllocKinAward(szType)
    ...
    for nKinId, tbPlayerIds in pairs(tbJoinKinCount) do
        if not MODULE_ZONESERVER then
            ...
            local nConnectIdx, nOrgKinId = self:GetKinConnectIndex(nKinId);
            CallZoneClientScript(nConnectIdx, "BossLeader:CrossSendKinAward", szType, nOrgKinId, tbPlayerIds);
        end    
    end  
...
function BossLeader:AuctionKinAward(szType, nKinId, tbAllAward, tbPlayerIds)
    Kin:AddAuction(nKinId, "BossLeader_"..szType, tbPlayerIds, tbAllAward);
...
function BossLeader:CreateKinChatRoom()
    if MODULE_ZONESERVER then
        return;
    end

    --只会触发一次
    if Calendar:IsActivityInOpenState("FieldBoss") and Calendar:IsActivityInOpenState("FieldCrossBoss") then
        return
    end

    Kin:TraverseKinInDiffTime(1, function (kinData)
        if not ChatMgr:IsKinHaveChatRoom(kinData.nKinId) then
            ChatMgr:CreateKinChatRoom(kinData.nKinId)
        else
            BossLeader:OnCreateChatRoom(kinData.nKinId)
        end
    end);
end

function BossLeader:CheckJoinChatRoom(pPlayer)
    --实时语音
    local dwKinId = pPlayer.dwKinId
    if dwKinId > 0 then
        if not ChatMgr:IsKinHaveChatRoom(dwKinId) then
            ChatMgr:CreateKinChatRoom(dwKinId, pPlayer.nZoneIndex)
        else
...
function BossLeader:OnCreateChatRoom(dwKinId)
    if MODULE_ZONESERVER then
        return;
    end
    ...
    if (not tbTypeData or not tbTypeData.bStartEnter) and not self:IsOpenCrossBoss() then
        return false
    end

    --将家族里在线玩家都加入
    local tbMember = Kin:GetKinMembers(dwKinId, true)
...
function BossLeader:OnPlayerLogin(pPlayer)
    if not (Calendar:IsActivityInOpenState("FieldBoss") or Calendar:IsActivityInOpenState("FieldCrossBoss")) then
        return
    end
    --把在活动期间登录的玩家加入家族实时语音频道
    BossLeader:CheckJoinChatRoom(pPlayer)
end

function BossLeader:OnEndLeaveKinChatRoom()
    --判断如果本服名将和跨服名将都结束了再退出家族实时语音
    if Calendar:IsActivityInOpenState("FieldBoss") or Calendar:IsActivityInOpenState("FieldCrossBoss") then
        return
    end
