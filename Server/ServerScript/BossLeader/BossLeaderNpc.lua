        ...
        for nI = 1, nTotalTeam - 1 do
            for nJ = nI + 1, nTotalTeam do
                ...
                if nImitity then
                    ...
                    local nOrgPlayerId1 = KPlayer.GetOrgPlayerId(nPlayer1);
                    local nOrgPlayerId2 = KPlayer.GetOrgPlayerId(nPlayer2);
                    Lib:CallZ2SOrLocalScript(nConnect, "BossLeader:FriendShipAddImitity", nOrgPlayerId1, nOrgPlayerId2, nImitity, LogType);
                end
            end
        end
    end
...
function tbNpc:AddCaptainAward(tbBLInfo, tbKinSort, nCaptainId, nRank, tbTeam, tbDmgInfo)
    ...
    if tbPrestige then
        ...
        local nConnect, nOrgKinId = BossLeader:GetKinConnectIndex(nKinId);
        Lib:CallZ2SOrLocalScript(nConnect, "BossLeader:AddKinPrestige", nOrgKinId, nPrestige, tbBLInfo.szType);
    end
...
function tbNpc:OnDeathNotic(pNpc, pKillNpc, nFirstCaptainId, szType, tbNpcInfo)
    ...
    if szDeathNotic and tbNpcInfo.IsFalse ~= 1 then
        ...
        elseif nFirstCaptainId > 0 then
            local tbStayInfo = KPlayer.GetRoleStayInfo(nFirstCaptainId) or {};
...
function tbNpc:OnByAttack()
    ...
    if tbBLInfo.szType == "Leader" then
        ...
        if me.nPkMode == Player.MODE_PEACE or me.nPkMode == Player.MODE_EXCERCISE then
            ...
            me.SendBlackBoardMsg("攻击野外首领，强制切为帮派模式");
        end
    end
    ...
    if me.dwTeamID > 0 and not tbTouchImitityPlayer[me.dwID] and nImitity then
        ...
        for _, nPlayerID in pairs(tbMember) do
            if nPlayerID ~= me.dwID then
                ...
                local nOrgPlayerId2 = KPlayer.GetOrgPlayerId(nPlayerID);
                Lib:CallZ2SOrLocalScript(me.nZoneIndex, "BossLeader:FriendShipAddImitity", me.dwOrgPlayerId, nOrgPlayerId2, nImitity, LogType);
            end
        end
    end

    Lib:CallZ2SOrLocalScript(me.nZoneIndex, "BossLeader:TeacherOnJoinBossLeader", me.dwOrgPlayerId, tbBLInfo.szType);

    if not bTouchNpc and him.tbBossLeaderInfo and him.tbBossLeaderInfo.bIsFirstDmg then
        Lib:CallZ2SOrLocalScript(me.nZoneIndex, "BossLeader:DoAttackFirstDmgAward", me.dwOrgPlayerId, tbBLInfo.szType);
    end
end
