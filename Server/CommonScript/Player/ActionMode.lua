...
function ActionMode:CheckDoChangeActionMode(pPlayer, nActMode)
    ...
    if Map:IsForbidRide(nMapTID) then
        return false, string.format("目前地图不能%s", tbActModeInfo.szName);
    end
    ...
    if pNpc.nShapeShiftNpcTID > 0 then
        return false, string.format("处於变身状态时不能%s", tbActModeInfo.szName);
    end
...
function ActionMode:OnEnterMap(pPlayer, nMapTemplateId)
    ...
    ActionMode:DoForceNoneActMode(pPlayer, "目前地图不能骑马！");
end
...
function ActionMode:CallDoActionMode(nActionMode, bMsg)
    ...
    if not Toy:CanJump() and nActionMode ~= Npc.NpcActionModeType.act_mode_none then
