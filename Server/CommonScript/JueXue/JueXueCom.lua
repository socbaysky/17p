...
function JueXue:GetBookActivateArea(nBookTemplateId)
	...
	if not nType then
		return false, "此秘笈无法启动任何区域"
	end

	local nAreaId = self:GetItemActivateAreaId(nType)
	if not nAreaId then
		return false, "此秘笈无法启动该区域"
	end
	return true, "", nAreaId
end
...
function JueXue:CheckCanXiuLian(pPlayer, nAreaId)
