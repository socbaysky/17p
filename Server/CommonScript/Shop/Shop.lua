...
Shop.tbMarketable =  LoadTabFile("Setting/Shop/MarketableThing.tab",
    "ddsds", "TemplateId",
    {"TemplateId", "Price", "MoneyType", "PlayerLevel", "TimeFrameLimit"});
...
function Shop:_CanBuyWare(pPlayer, tbWare, nBuyCount)
	...
	------paulf-------paulf20181228
	 if nTemplateId==3238 or nTemplateId==2877  or nTemplateId==2876 then
      if nBuyCount>200 then 
        return false, "该物品单次购买数量200"
	  end 
    end
	--------------
    ...
    if tbWare.nMinLevel and tbWare.nMinLevel > pPlayer.nLevel then
        return false, string.format("角色%s级後可购买", tbWare.nMinLevel)
    end
...
Shop.tbItemClassSellFunc = {
    ...
    ["Stone"] = function (self, nTemplateId, nCount, tbItemInfo)
        ...
        if GetTimeFrameState("OpenLevel129") == 1 then
            table.insert(tbAllowQuality, 4)
        end
