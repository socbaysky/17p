...
function tbZhenYuan:TrimSetting()
	...
	if MODULE_GAMESERVER then
		...
		self.tbIdentifyValue = LoadTabFile(
		"Setting/Item/ZhenYuan/IdentifyValue.tab", 
		"dd", "nLevel", {"nLevel", "nValue"} );	
...
function tbZhenYuan:GetAndChangeOldIndentifyToVal(pPlayer)
	local nNowVal = pPlayer.GetUserValue(self.SAVE_GROUP, self.SAVE_KEY_IDENTI_XX_VAL)
	if nNowVal ~= 0 then
		return nNowVal
	end
	...
	for nLevel = 1, #self.tbMakerLevelSetting do
		...
		if nXYFails > 0 then
			nNowVal = nNowVal + nXYFails * self.tbIdentifyValue[nLevel].nValue
			szFailLevelsVal = string.format("%s; %d|%d ", szFailLevelsVal, nLevel, nXYFails)
			pPlayer.SetUserValue(self.SAVE_GROUP, nSaveKeyXYIndentify, 0)
		end
	end
	Log("GetAndChangeOldIndentifyToVal", pPlayer.dwID, szFailLevelsVal, nNowVal)
	return nNowVal
end
	...
	if pEquip.nDetailType == Item.DetailType_Rare and me then
		...
		if not bForbitSkill then
			...
			if bDirectGetSkill then
				...
				local nOldTotalVal = self:GetAndChangeOldIndentifyToVal(me)
				local nCurVal = self.tbIdentifyValue[nLevel].nValue
				local nNowVal = nOldTotalVal + nCurVal
				local nRand = MathRandom(self.RAND_SKILL_VAL)
				bGetSkill = nRand <= nNowVal;
				Log("tbZhenYuan:OnGenerateXY", pEquip.dwTemplateId,me.dwID, nRand, nNowVal, nLevel)
				if bGetSkill then
					nNowVal = nNowVal - self.RAND_SKILL_VAL
				end
				me.SetUserValue(self.SAVE_GROUP, self.SAVE_KEY_IDENTI_XX_VAL, nNowVal)
			end
		
			if bGetSkill then
				local nRandSkill, bReadyFull = self:GetRandSkillId(me)
				local nSkillLevel = 1;
				local nSave = tbRefinement:AttribToSaveData(nRandSkill, nSkillLevel);
				pEquip.SetIntValue(self.nItemKeySKillInfo, nSave)	
				if bReadyFull and me then
					Achievement:AddCount(me, "Mystic_ZhenYuan5Skill", 1)
				end
			end
		end
		...
		for i,v in ipairs(self.tbRandSkillId) do
			if not tbForbid[v.SkillId] then
				...
				if nRate <= nSumRate then
					return v.SkillId, nForbitConut+1 == #self.tbRandSkillId
				end	
			end
		end
	end
...
if MODULE_GAMECLIENT then
	...
	function tbZhenYuan:RequestRefineSkill(nItemId)
		...
		local szMsg = string.format("您确定要花费%s将已装备的真元上的技能[FFFE0D]%s[-]，替换为[FFFE0D]%s[-]吗？（替换後，技能等级将会继承保留）",szMoneyUseDesc, szSkillNameCur, szSkillNameSrc)
