...
function Item:Init()
	...
	KItem.RegisterPos2Part(Item.EQUIPPOS_WAI_BACK, 5, 2);
...
--	end
	...
	if pItem.nUseLevel > me.nLevel then
		return 0, string.format("%d级之後可使用", pItem.nUseLevel);
	end
	-- TODO cd
...
function Item:UseEquip(nId, bSlient, nEquipPos)
	...
	if nResult == 1 then
		...
		if nEquipPos == Item.EQUIPPOS_HORSE then
			local szEquipName = KItem.GetItemShowInfo(pEquip.dwTemplateId, me.nFaction, me.nSex);
...
function Item:NotifyTimeOut(dwTemplateId, nCount)
	local szItemName = KItem.GetItemShowInfo(dwTemplateId, me.nFaction, me.nSex)
