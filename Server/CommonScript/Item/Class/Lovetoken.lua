...
function tbItem:ConfirmUseItem(nPlayerId, nOtherPlayerId, nItemId, szTitle1, szTitle2, bResult)
	...
	elseif nType == BiWuZhaoQin.TYPE_KIN then
		if pPlayer.dwKinId > 0 then
			ChatMgr:SendSystemMsg(ChatMgr.SystemMsgType.Kin, string.format("恭喜帮派成员[FFFE0D]%s[-]与[FFFE0D]%s[-]结成了情缘关系，从此仗剑江湖，风雨同行！", pPlayer.szName, pOther.szName), pPlayer.dwKinId);
		end
	end
	...
	for _, szTitle in pairs(tbTitle) do
		if version_vn then
			...
			if nVNLen > BiWuZhaoQin.nVNTitleNameMax or nVNLen < BiWuZhaoQin.nVNTitleNameMin then
				return false, string.format("自订称号需在%d~%d字之间", BiWuZhaoQin.nVNTitleNameMin, BiWuZhaoQin.nVNTitleNameMax);
			end
		elseif version_th then
			...
			if nNameLen > BiWuZhaoQin.nTHTitleNameMax or nNameLen < BiWuZhaoQin.nTHTitleNameMin then
				return false, string.format("自订称号需在%d~%d字之间", BiWuZhaoQin.nTHTitleNameMin, BiWuZhaoQin.nTHTitleNameMax);
			end
		else
			local nNameLen = Lib:Utf8Len(szTitle);
			if nNameLen > BiWuZhaoQin.nTitleNameMax or nNameLen < BiWuZhaoQin.nTitleNameMin then
				return false, string.format("自订称号需在%d~%d字之间", BiWuZhaoQin.nTitleNameMin, BiWuZhaoQin.nTitleNameMax);
			end
		end

		if not CheckNameAvailable(szTitle) then
			return false, "不允许的自订称号！";
		end
	end
