...
function tbRandomItem:AddOtherAward(szType, SubType, nCount, nItem)
	...
	elseif szType == "LabaMatrial" then
...
function tbRandomItem:AddAward(pPlayer, tbItem, szFromItemName, nLogReazon, nLogReazon2)
	...
	if tbItem.nItem and tbItem.nItem > 0 and tbItem.szOtherType == "" then
		...
		if tbItem.nTimeLimit and tbItem.nTimeLimit > 0 then
			varTimeOut = os.date("%Y-%m-%d %H:%M:%S", GetTime() + tbItem.nTimeLimit);
		elseif tbItem.nDateLimit and tbItem.nDateLimit > 0 then
			varTimeOut = tbItem.nDateLimit
		end
		if tbItem.nAdd2Auction == 1 then
			bAdd2Auction = true
		end
		tbGetAward = {"item", tbItem.nItem, tbItem.nAmount, varTimeOut, tbItem.bForbidStall}
...
function tbRandomItem:OnUse(it)
	...
	local nShowTip = tonumber(it.nRandomItemKindId or KItem.GetItemExtParam(it.dwTemplateId, 2));
	if nShowTip == -1 then
		bShowTip = false
	end
