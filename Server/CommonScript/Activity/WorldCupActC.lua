...
tbAct.szMailText = "您在[FFFE0D]2018世界盃徽章收集活动[-]中位列第[FFFE0D]%d[-]名，附件为奖励，请查收！"
tbAct.tbRankAward = {
...
if MODULE_GAMECLIENT then
	...
	function tbAct:Transfer(bNormal)
		...
		local szMsg = bNormal and string.format("确定将[FFFE0D]%s[-]转换成除本徽章之外随机一个国家队徽章吗？转换成功之後将消耗[FFFE0D]普通的徽章转换符[-]！", szFromName) or 
			string.format("确定将[FFFE0D]%s[-]转换成[FFFE0D]%s[-]吗？转换成功後将消耗[FFFE0D]高级的徽章转换符[-]！", szFromName, KItem.GetItemShowInfo(self.nTransToItemId, me.nFaction, me.nSex))
