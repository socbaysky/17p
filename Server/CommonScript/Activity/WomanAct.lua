...
tbAct.szMailText =  "    光阴似箭,日月如梭，侠士经历了如此之久的江湖生活，想必已认识不少挚友，不知其中是否有印象深刻的侠士，尤其是女侠，若是有的话，就快打开印象册，为那些与你一同闯荡江湖的同行者写上你对他们的印象吧！";
tbAct.szMailFrom =  "";
...
-- 可赠送的截止时间
tbAct.szSendLabelEndTime = "2018-3-11-3-59-59"
--[[
师徒关系模式:
	去掉旧活动领取活跃度奖励时给的奖励
	去掉升到一定等级邮件发送印象签的设定
...
function tbAct:CheckCommon(pPlayer, nAcceptId, nType, szLabel, bNotCheckGold)
	...
	if nType == self.FreeLabel then
		if pPlayer.GetItemCountInAllPos(self.nImpressionLabelItemID) < self.nNeedConsumeImpressionLabel then
			return false, string.format("您拥有的%s不足", KItem.GetItemShowInfo(self.nImpressionLabelItemID, pPlayer.nFaction, pPlayer.nSex) or "印象签")
		end
