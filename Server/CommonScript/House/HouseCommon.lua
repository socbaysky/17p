...
House.MUSE_TIME = 5;	-------paulf20181228					-- 冥想时间
...
function House:GetFurnitureMakeOpenTips()
	...
	if bMax then
		return string.format("当前已开放至 [FFFE0D]%s级[-] 家俱制作", tbOpenLevelInfo[#tbOpenLevelInfo]);
	end

	local nRefDay = Lib:GetLocalDay(CalcTimeFrameOpenTime(szNextTimeFrame)) - Lib:GetLocalDay()
	return string.format("[FFFE0D]%s天[-] 後开放 [FFFE0D]%s[-] 家俱制作", nRefDay, szOpenInfo);
end
