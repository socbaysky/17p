		...
		if tbTmp.nQuotiety ~= nQuotiety or tbTmp.szJiuName ~= szJiuName then
			...
			if szJiuName and nQuotiety and nJiuMax then
				if nTeamId ~= 0 then
					...
					szLastGuohuoTipContent = string.format("目前处於饮酒状态，获得%d%%的篝火经验", nQuotiety)
					if pOwnner.szLastGuohuoTipContent ~= szLastGuohuoTipContent then
...
function tbGouhuoNpc:AddExp2Player(nPlayerId, nNpcId, nAnnouce, bKinGather)
	...
	if tbTmp.nCanUseXiuLianZhu == 1 then
		...
		if bXiuLian then
			Lib:CallBack({self.OnAddXiuLianExp, self, pPlayer, pNpc});
		end
	else
...
function tbGouhuoNpc:OnAddXiuLianExp(pPlayer, pNpc)
	if Achievement:IsKindCompleted(pPlayer, "Mystic_8Fire") then
		return
	end
	pPlayer.tbXiuLianGouHuo = pPlayer.tbXiuLianGouHuo or {}
	pPlayer.tbXiuLianGouHuo[pNpc.nId] = true
	local nCount = 0
	for nId in pairs(pPlayer.tbXiuLianGouHuo) do
		local pNpc = KNpc.GetById(nId);
		if pNpc then
			nCount = nCount + 1
		else
			pPlayer.tbXiuLianGouHuo[nId] = nil
		end
	end
	if nCount >= 8 then
		Achievement:AddCount(pPlayer, "Mystic_8Fire", 1)
	end
end
