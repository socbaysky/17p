...
function tbNpc:OnDialog()
	...
	local tbOptList = {
		{ Text = "今日情缘", Callback = self.TodayLove, Param = {self, me.dwID} };
	};
	...
	if szTaskStep then
		...
		table.insert(tbOptList, { Text = "[缘]情缘任务", Callback = self.TaskLove, Param = {self, me.dwID} })
	end
...
function tbNpc:RemoveLover(bConfirm)
	...
	Mail:SendSystemMail({
		To = nOtherId,
		Title = "情缘解除",
		Text = string.format("大侠，很遗憾的告诉您，侠士[FFFE0D]%s[-]於%s解除了与您的情缘关系！", me.szName, os.date("%Y年%m月%d日", GetTime())),
		From = "燕若雪",
	});
