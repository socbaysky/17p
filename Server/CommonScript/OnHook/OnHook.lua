...
function OnHook:PayTips(pPlayer,nPayType)
	...
	if nExpTime > nBaiJuWanTime then
		...
		if nHaveNum > 0 then
			szTip = szTip ..string.format("已拥有[FFFE0D]%d个[-]%s",nHaveNum,KItem.GetItemShowInfo(nItemId, pPlayer.nFaction, pPlayer.nSex))
		end
		...
		if nLackNum > 0 then																	-- 需要用元宝的情况
			...
			local nNeedPay = nLackNum * nPerBaiJuWanPrice;
			 ...
			 if nHaveNum <= 0 then
			 	szNeedBuy = szNeedBuy .. KItem.GetItemShowInfo(nItemId, pPlayer.nFaction, pPlayer.nSex)
			 end
...
function OnHook:CheckRequestOnlineOH(pPlayer)
	if pPlayer.nRequestStartOnlineOHTime and GetTime() < pPlayer.nRequestStartOnlineOHTime then
		...
		return false,string.format("托管操作太频繁了,请%d秒之後再尝试",nSecond)
	end
