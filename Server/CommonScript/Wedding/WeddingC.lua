...
Wedding.tbBubbleMsg = 						-- npc模板ID对应的祝贺词
{
	...
	--赵丽颖
	[2279] = "前些日子红娘在家俱坊定制了一批上等家俱，说是要等侠士的成婚纪念日时给个惊喜！#43",
}
...
Wedding.tbTrigger =
{
	...
	[12] = {
		nTime = 15,
		tbExe = {
			{szType = "NpcBubbleTalk", tbParam = {nNpcTId = 2386, szBubble = "新娘好漂亮啊，等我长大了我也要这麽美美哒！#13#13", szBubbleTime = "10"}};
			{szType = "NpcBubbleTalk", tbParam = {nNpcTId = 2387, szBubble = "新娘好漂亮啊，等我长大了我也要这麽美美哒！#13#13", szBubbleTime = "10"}};
		}
	};
	...
	[204] = {
		nTime = 8,
		tbExe = {
			{szType = "TempNpcBubbleTalk", tbParam = {szGroup = "xiaoyinfang", nNpcTId = 2626, szBubble = "新郎凭什麽能娶到这麽漂亮的新娘？哼！可敢与我上比武台比划比划？#119", szBubbleTime = "8"}};
		};
	};
...
Wedding.nProposeImitity = 1 																		-- 需要亲密度
...
Wedding.szProposeSuccessNotify = "相遇于剑侠，相识于江湖，缘起於情缘，缘定於今生。恭喜[FFFE0D]「%s」[-]向[FFFE0D]「%s」[-]求婚成功，祝有情人终成眷属！"					-- 求婚成功滚屏公告
Wedding.szProposeFailTip = "「%s」也许还未做好准备，拒绝了你的求婚"									-- 求婚者拒绝提示
Wedding.szBeProposeFailTip = "你拒绝了「%s」的求婚" 												-- 被求婚者拒绝求婚提示
Wedding.szProposeBreakTip = "求婚中断啦，有些耐心，过程中要静静等待对方答覆哦！"								-- 求婚过程中主动打断的提示（切地图，离线，重登等）
...
Wedding.nMarryOpenStart = "00:50" 																	-- 可开启婚礼开始时间点
...
Wedding.tbWeddingLevelMapSetting = {
	[Wedding.Level_1] = {
		...
		tbSchdule = 
		{
			{nProcess = Wedding.PROCESS_WELCOME, szProcess = "迎宾"};
			{nProcess = Wedding.PROCESS_STARTWEDDING, szProcess = "婚礼准备"};
			{nProcess = Wedding.PROCESS_PROMISE, szProcess = "山盟海誓"};
			{nProcess = Wedding.PROCESS_MARRYCEREMONY, szProcess = "拜堂"};
			{nProcess = Wedding.PROCESS_FIRECRACKER, szProcess = "开心爆竹"};
			{nProcess = Wedding.PROCESS_WEDDINGTABLE, szProcess = "宴席"};
			{nProcess = Wedding.PROCESS_CANDY, szProcess = "派发喜糖"};
		};
	};
	[Wedding.Level_2] = {
		nMapTID = 5001;
		...
		nStartDay = Lib:ParseDateTime("nil"); 												-- 预定开始的时间(从某个时间点开始可以预定，为nil则从当前开始)
		...
		tbSchdule = 
		{
			...
			{nProcess = Wedding.PROCESS_MARRYCEREMONY, szProcess = "拜堂"};
			{nProcess = Wedding.PROCESS_FIRECRACKER, szProcess = "开心爆竹"};
			{nProcess = Wedding.PROCESS_CONCENTRIC, szProcess = "同食同心果"};
			{nProcess = Wedding.PROCESS_WEDDINGTABLE, szProcess = "宴席"};
			{nProcess = Wedding.PROCESS_CANDY, szProcess = "派发喜糖"};
		};
		nContinueTime = 27;
	...
	[Wedding.Level_3] = {
		...
		--end;
		nOverdueClearDay = 60;
		nCompleteClearDay = 1;	 																			-- 按周算，7天，为了防止玩家再次预订已经预约过的日期
		...
		nStartDay = Lib:ParseDateTime("nil");
...
Wedding.tbMemorialMonthRewards = { -- N个月纪念
	...
	[12] = {
		...
		tbRedBags = {
			Kin.tbRedBagEvents.marry_month_12,
		},
	},
}
...
function Wedding:CheckProposeC(pPlayer, nPlayerId)
	...
	if not Wedding:IsSingle(pPlayer) then
        return false, string.format("[FFFE0D]「%s」[-]已经成婚或已有订婚物件", pPlayer.szName)
    end
	 ...
	 if not Map:IsCityMap(pPlayer.nMapTemplateId) and not Map:IsKinMap(pPlayer.nMapTemplateId) and not Map:IsHouseMap(pPlayer.nMapTemplateId) then
        return false, string.format("[FFFE0D]「%s」[-]所在场景不允许求婚，请在[FFFE0D]忘忧岛、主城、家园、帮派属地[-]进行", pPlayer.szName)
    end
    if nPlayerId then
...
function Wedding:CheckPromiseValid(szMsg)
	if version_kor then
		...
		if nKorLen > Wedding.nKorPromiseMax or nKorLen < Wedding.nKorPromiseMin then
			return false, string.format("誓言长度需要在%d~%d个汉字内", Wedding.nKorPromiseMin, Wedding.nKorPromiseMax)
		end
	elseif version_vn then
		local nVNLen = string.len(szMsg);
		if nVNLen > Wedding.nVNPromiseMax or nVNLen < Wedding.nVNPromiseMin then
			return false, string.format("誓言长度需要在%d~%d个汉字内", Wedding.nVNPromiseMin, Wedding.nVNPromiseMax)
		end
	elseif version_th then
		local nNameLen = Lib:Utf8Len(szMsg);
		if nNameLen > Wedding.nTHPromiseMax or nNameLen < Wedding.nTHPromiseMin then
			return false, string.format("誓言长度需要在%d~%d个汉字内", Wedding.nTHPromiseMin, Wedding.nTHPromiseMax)
		end
	else
		local nNameLen = Lib:Utf8Len(szMsg);
		if nNameLen > Wedding.nPromiseMax or nNameLen < Wedding.nPromiseMin then
			return false, string.format("誓言长度需要在%d~%d个汉字内", Wedding.nPromiseMin, Wedding.nPromiseMax)
		end
		if Lib:HasNonChineseChars(szMsg) then
	        return false, "誓言只能使用中文字元，请修改后重试！"
	    end
	end
	if ReplaceLimitWords(szMsg) then
		return false, "誓言含有敏感字元，请修改后重试！"
	end
	return true
end
