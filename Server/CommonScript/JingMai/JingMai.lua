...
JingMai.nJingMaiLevelUpTime = 7 * 24 * 60 * 60
...
-- 每日穴位升级次数（成功才算次数）
JingMai.nMaxLevelupTimes = 50;-------paulf20181228
...
JingMai.szJingMaiLevelTimeDes = "周天运转中，%s後可启动" 					-- 经脉升级中倒计时文本描述
...
function JingMai:CheckJingMaiOpen(nJingMaiId)
	...
	if self.tbOpenInfo then
		if nJingMaiId >= self.tbOpenInfo.nNextOpenJingMaiId and nTimeNow < self.tbOpenInfo.nOpenTime then
			return false, string.format("将在%s後开放此经脉", Lib:TimeDesc2(self.tbOpenInfo.nOpenTime - nTimeNow));
		end
	end

	local nOpenTime = TimeFrame:CalcTimeFrameOpenTime(tbJingMai.szOpenTimeFrame);
	if nTimeNow < nOpenTime then
		return false, string.format("将於%s後开放此经脉", Lib:TimeDesc2(nOpenTime - nTimeNow));
	end
...
function JingMai:OnFinishTask(nTaskId)
	...
	me.SendBlackBoardMsg("阁下已领悟了打通经脉的法门，可通过“[FFFE0D]同伴——经脉[-]”介面查看！", true);
	...
	if nZhenQi > 0 then
		local tbMail = {
			To = me.dwID;
			Title = "真气运行";
			Text = "      小友天资不错，这麽快就领悟了[FFFE0D]经脉[-]中真气运行的法门，在此之前你的同伴使用资质丹已经在丹田中凝结了很多[FFFE0D]真气[-]，老朽就帮助你获得这些真气以便随意使用吧。";
			From = "玄天道人";
