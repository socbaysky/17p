...
PartnerCard.DEFAULT_TALK = "吾浪荡江湖多年，终於找到了个落脚的地方，此处静谧，吾甚是喜欢。" 				-- 默认泡泡
...
PartnerCard.tbDevilMasterAward = {					-- 主人奖励
	{1,{{"Energy", 280}},{{"BasicExp", 5}}};
	{2,{{"Energy", 360}},{{"BasicExp", 10}}};
	{3,{{"Energy", 440}},{{"BasicExp", 15}}};
	{4,{{"Energy", 520}},{{"BasicExp", 20}}};
	{5,{{"Energy", 600}},{{"BasicExp", 25}}};
	{6,{{"Energy", 680}},{{"BasicExp", 30}}};
	{7,{{"Energy", 760}},{{"BasicExp", 35}}};
	{8,{{"Energy", 840}},{{"BasicExp", 40}}};
	{9,{{"Energy", 920}},{{"BasicExp", 45}}};
	{10,{{"Energy", 1000}},{{"BasicExp", 50}}};								-- n次（包括n次）以上的奖励
} 	
PartnerCard.tbDevilAssistAward	= {					-- 协助奖励
	{1,{{"Energy", 80}},{{"BasicExp", 5}}};
	{2,{{"Energy", 160}},{{"BasicExp", 10}}};
	{3,{{"Energy", 240}},{{"BasicExp", 15}}};
	{4,{{"Energy", 320}},{{"BasicExp", 20}}};
	{5,{{"Energy", 400}},{{"BasicExp", 25}}};
	{6,{{"Energy", 480}},{{"BasicExp", 30}}};
	{7,{{"Energy", 560}},{{"BasicExp", 35}}};
	{8,{{"Energy", 640}},{{"BasicExp", 40}}};
	{9,{{"Energy", 720}},{{"BasicExp", 45}}};
	{10,{{"Energy", 800}},{{"BasicExp", 50}}};									-- n次（包括n次）以上的奖励
} 	
...
function PartnerCard:CheckBubble(szMsg)
	...
	elseif version_th then
		...
		if Lib:HasNonChineseChars(szMsg) then
	        return false, "喊话内容只能使用中文字元，请修改後重试！"
	    end
	end
	if ReplaceLimitWords(szMsg) then
		return false, "喊话内容含有敏感字元，请修改後重试！"
	end
	return true
end
...
function PartnerCard:CheckLeaveHouse(pPlayer, nCardId)
	...
	if nActState ~= PartnerCard.CARD_ACT_STATE_NONE then
		return false, "您的门客有派遣奖励待领取，请先至派遣介面领取奖励"
	end
	return true
end
		...
		for _, tbInfo in pairs(tbCost) do
			...
			elseif nType == Player.award_type_item then
				...
				if nCount < tbInfo[3] then
					local szItemName = KItem.GetItemShowInfo(tbInfo[2], pPlayer.nFaction, pPlayer.nSex)
