...
-- PartnerCard.szFuncOpenTimeFrame = "OpenLevel109" 													-- 功能开放时间轴
PartnerCard.szFuncOpenTimeFrame = "OpenLevel999" 													-- 功能开放时间轴
PartnerCard.nMaxGiftSendTimes = 100 																	-- 每个门客每天可被赠送礼物的次数
...
function PartnerCard:LoadSetting()
	...
	local tbFile = LoadTabFile("Setting/PartnerCard/PartnerCardSetting.tab", "dddddssdssds", nil, {"nCardId", "nPartnerTempleteId", "nNpcTempleteId", "nExtAttribId", "nExtPartnerSkillId", "szUniquePartnerSkillId", "szCost", "nSuitIdx", "szName", "szTask", "nDialogId", "szRepeat"});
	local nMaxCardId = 0
	...
	tbFile = LoadTabFile("Setting/PartnerCard/PartnerCardPosSetting.tab", "ddssss", nil, {"nPartnerPos", "nCardPos", "szOpenTimeFrame", "szCost", "szLockTip", "szConsumeTip"});
	for _, v in ipairs(tbFile) do
...
function PartnerCard:IsCardPosOpen(pPlayer, nCardPos)
	...
	local szOpenTimeFrame = tbCardPosInfo.szOpenTimeFrame
	if szOpenTimeFrame then
...
function PartnerCard:CanSendGift(pPlayer, nCardId, nItemId, nCount)
	...
	return true, nil, tbGiftInfo.nAddExp * nCount
end
